/**
 */
Ext.define('lookup.popup.item.ItemClassPopup', { extend: 'Axt.popup.Search',
    alias: 'widget.lookup-item-class-popup',
    store: 'lookup.popup.item.store.ItemClassPopup',

    title: Language.get('itm_clss_popup','품목 분류 찾기'),
    closable: true,
    autoShow: true,
    width: 700,
    height: 500,
    			...
    			items : [
    		 		{
    		 			...
    		 			columns    : [{ text: Language.get('class_1','대분류') , dataIndex: 'clss_nm',  flex : 1  }],
    		 			listeners  : {
    		 				...
    		 				selectionchange : function(grid, records ) {
    		 					...
    		 					if (record) {
    		 						...
    		 						var param = Ext.merge({ hq_id : record.get('hq_id'), prnt_id : record.get('clss_id') }, me.popup.params);
    		 						class2.load({
    		 			...
    		 			viewConfig : { loadMask: new Ext.LoadMask( me , { msg: Const.SELECT.mask }) },
    		 			store      : Ext.create( me.store ),
    		 			columns    : [{ text: Language.get('class_2','중분류') , dataIndex: 'clss_nm',  flex : 1  }],
    		 			listeners  : {
    		 				itemclick : function (dv, record, item, index, e) {
    		 					me.selectRecord( record);
    		 				},
    		 			...
    		 			viewConfig : { loadMask: new Ext.LoadMask( me , { msg: Const.SELECT.mask }) },
    		 			store      : Ext.create( me.store ),
    		 			columns    : [{ text: Language.get('class_3','소분류') , dataIndex: 'clss_nm',  flex : 1  }],
    		 			listeners  : {
    		 				itemclick : function (dv, record, item, index, e) {
    		 					me.selectRecord( record);
    		 				},
    		 			...
    		 			viewConfig : { loadMask: new Ext.LoadMask( me , { msg: Const.SELECT.mask }) },
    		 			store      : Ext.create( me.store ),
    		 			columns    : [{ text: Language.get('class_4','여유분류') , dataIndex: 'clss_nm',  flex : 1  }],
    		 			listeners  : {
    		 				itemclick : function (dv, record, item, index, e) {
    		 					me.selectRecord( record);
    		 				},
    	...
    	var me = this,
    		tool = {
    			...
    			items	: [
    			 	...
    			 	{xtype: 'textfield' , name : 'clss_id', hidden : true , readOnly : true  },
    ...
    selectRecord : function( record ) {
		...
		if (record) {
			...
			base.down('[name=clss_desct]').setValue(record.get('clss_dect'));
		}
    },
    /**
     * 확인 버튼 이벤트
