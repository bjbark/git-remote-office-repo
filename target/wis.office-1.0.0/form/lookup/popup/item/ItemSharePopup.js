/**
 */
Ext.define('lookup.popup.item.ItemSharePopup', { extend: 'Axt.popup.Search',

	alias   : 'widget.lookup-item-share-popup',
	store   : 'lookup.popup.item.store.ItemSharePopup',

	requires:
	[
	],
	title   : Language.get('hq_itm_copy','본사 품목 가져오기'),
	closable: true,
	autoShow: true,
     ...
     createGrid: function(){
        var  me = this,
        	grid = {
        		...
        		columns: [
        		 	{	dataIndex: 'using_yn'  	, width:  40, text: Language.get('copyed','받음')  	,  xtype : 'lookupcolumn' , lookupValue : resource.getList('use_yn')
    		 		},{ dataIndex: 'shr_yn'     , width:  40, text: Language.get('common','공통')   ,  xtype : 'lookupcolumn' , lookupValue : resource.getList('shr_yn')
		 			},{	dataIndex: 'itm_sts'   	, width:  60, text: Language.get('itm_sts','판매상태')  , xtype : 'lookupcolumn' , lookupValue : resource.getList('itm_sale_sts') },
        		 	{   dataIndex: 'item_code'		, width: 110, text: Language.get('item_code',     '품목코드')},
        		 	{   dataIndex: 'item_name'		, width: 180, text: Language.get('item_name',       '품명'  )},
