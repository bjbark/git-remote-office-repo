Ext.define('lookup.popup.view.HntopItemClorPopup', { extend: 'Axt.popup.Search',

	alias	: 'widget.lookup-clor-popup',
	store	: 'lookup.popup.store.HntopItemClorPopup',
	requires: [
	...
	title	: Language.get('item_popup','자재 찾기'),
	closable: true,
	autoShow: true,
	 ...
	 createGrid: function(){
			...
			grid = {
				...
				columns: [
					...
					},{	dataIndex:	'colr_name'	, width: 100, align : 'left'   , text: Language.get( 'colr_name'      , '색상'	),
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					},{	dataIndex:	'stnd_pric'	, width:  80, align : 'right'  , text: Language.get( 'stnd_pric'      , '구매단가'	), xtype: 'numericcolumn' , format: '#,##0',
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					},{	dataIndex:	'cont_date'	, width:  80, align : 'center' , text: Language.get( 'cont_date'      , '계약일자'	),
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					},{	dataIndex:	'colr_idcd'	, width:  80, align : 'left'   , text: Language.get( 'colr_idcd'      , '색상id'	), hidden : true
					}
				],
				listeners: {
