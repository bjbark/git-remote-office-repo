/**
 */
Ext.define('lookup.popup.view.DeptPopup', { extend: 'Axt.popup.Search',
	id: 'lookup-dept-popup',
	alias: 'widget.lookup-dept-popup',
	store: 'lookup.popup.store.DeptPopup',
	title: Language.get('dept_popup','부서 코드 선택'),
	closable: true,
	autoShow: true,
	width: 480,
	height: 500,
	...
	createGrid: function(){
		var me = this,
			...
			columns:[
				{ text: Language.get('dept_code','부서코드')	, dataIndex: 'dept_code',  width: 80,
				...
				{ text: Language.get('dept_name','부서명' )	, dataIndex: 'dept_name',  width: 150,
					renderer: function(value, meta){
						if(me.popup.params.tema){
							this.columns[1].width = 190;
							meta.style = 'font-size:2em !important;height:30px;line-height:30px;'; // applied style for DIV element
						}
						return value;
					}
				},
				{ text: Language.get('user_memo','메모'  )	, dataIndex: 'user_memo',  width: 150,
					renderer: function(value, meta){
						if(me.popup.params.tema){
				...
				//{ text: '사업장'  , dataIndex: 'stor_nm', width: 80  },
				//{ text: '사업부'  , dataIndex: 'saup_nm',  width: 80  },
				//{ text: '부(팀)' , dataIndex: 'team_nm',  width: 80  }
		...
		if (selects.length === 0) {
			...
			if (me.popup.apiurl && me.popup.apiurl.master) {
				...
				Ext.each( selects , function( eachrow ){
					request.push({
						dept_idcd : eachrow.get('dept_idcd')
					});
				});
