Ext.define('lookup.popup.view.DelyLcalPopup', { extend: 'Axt.popup.Search',

	alias: 'widget.lookup-delylcal-popup',
	store: 'lookup.popup.store.DelyLcalPopup',
	title: Language.get( '' , '지역코드 찾기') ,
	...
	createGrid: function(tema){
		var	me		= this,
			grid	= {
				...
				columns		:[
					{	text	: Language.get('lcal_code'    , '지역코드')	, dataIndex: 'lcal_code'	, width: 100,
					...
					{	text		: Language.get('lcal_name'    , '지역명')	, dataIndex: 'lcal_name'	, width: 120,
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					{	text	: Language.get('trnt_cost_1fst'    , '운송비#1'  )	, dataIndex: 'trnt_cost_1fst'	, width: 120 ,
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					{	text	: Language.get('trnt_cost_2snd'    , '운송비#2'  )	, dataIndex: 'trnt_cost_2snd'	, width: 120 ,
						renderer: function(value, meta){
							if(me.popup.params.tema){
								this.columns[2].width = 160;
								meta.style = 'font-size:2em !important;height:30px;line-height:30px;'; // applied style for DIV element
					...
					{	text	: Language.get('trnt_cost_3trd'    , '운송비#3'  )	, dataIndex: 'trnt_cost_3trd'	, width: 120 ,
						renderer: function(value, meta){
							if(me.popup.params.tema){
								this.columns[2].width = 160;
								meta.style = 'font-size:2em !important;height:30px;line-height:30px;'; // applied style for DIV element
		...
		if (selects.length === 0) {
			...
			if (me.popup.apiurl && me.popup.apiurl.master) {
				Ext.each( selects , function( eachrow ){
					request.push({
						lcal_idcd : eachrow.get('lcal_idcd')
					});
				});
