/**
 */
Ext.define('lookup.popup.view.WkrnCodePopup', { extend: 'Axt.popup.Search',
	id 		: 'lookup-wkrn-code-popup',
	alias	: 'widget.lookup-wkrn-code-popup',
	store	:'lookup.popup.store.WkrnCodePopup',

	title	: Language.get('wkrn_popup','임율 코드 선택'),
	closable: true,
	autoShow: true,
	width	: 700,
	height	: 500,
	...
	createGrid: function(){
		...
		var me = this,
			grid = {
				...
				columns: [
					{	text: Language.get('wkrn_code','코드')	, dataIndex: 'wkrn_code',  width: 150 },
					{	text: Language.get('wkrn_name','코드명')	, dataIndex: 'wkrn_name',  flex  : 1  }
				],
				listeners: {
		...
		if (selects.length === 0) {
			...
			if (me.popup.apiurl && me.popup.apiurl.master) {
				...
				Ext.each( selects , function( eachrow ){
					request.push({
						wkrn_idcd : eachrow.get('wkrn_idcd')
					});
				});
