Ext.define('lookup.popup.view.SaleOrderStokPopup', { extend: 'Axt.popup.Search',
	id		: 'lookup-saleorder-stok-popup',
	alias: 'widget.lookup-saleorder-stok-popup',
	store		: 'lookup.popup.store.SaleOrderStokLister',

	title		: Language.get( 'stok_qntt' ,'재고수량'),
	closable	: true,
	autoShow	: true,
	width		: 400 ,
	height		: 500,
	...
	createGrid : function () {
		...
		grid = 		{
				...
				columns		:[
					{	text		: Language.get('prod_code'  , '제품코드')	, dataIndex: 'prod_code'	, width: 100,
					},{	text	: Language.get('prod_name'    , '제품명')	, dataIndex: 'prod_name'	, width: 145,
					},{	text		: Language.get('base_qntt'  , '발주수량')	, dataIndex: 'base_qntt'	, width: 55,
					},{	text	: Language.get('stok_qntt'    , '재고수량'  )	, dataIndex: 'stok_qntt'	, width: 55 ,
					}
				],
				listeners	: {
	...
	finishAction: function(){
		...
		if (selects.length === 0) {
			...
			if (me.popup.apiurl && me.popup.apiurl.master) {
				...
				param = Ext.merge( me.popup.params,item_idcd , {
					records : request
				});
