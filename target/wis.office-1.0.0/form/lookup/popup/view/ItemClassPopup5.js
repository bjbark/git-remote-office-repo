Ext.define('lookup.popup.view.ItemClassPopup5', { extend: 'Axt.popup.Search',
	alias	: 'widget.lookup-item-clss-popup5',
	store	: 'lookup.popup.store.ItemClassPopup2',
					...
					{	xtype		: 'grid-panel',
						...
						listeners	: {
							itemclick : function (dv, record, item, index, e) {
								...
								me.down('[name=lcls_name]').setValue(record.get('clss_name')); // A-ONE 품목코드
								me.selectRecord( record);
							},
							itemdblclick: function(dataview, index, item, e) {
								if (me.popup.choice && me.popup.choice == 'free') {
									me.down('[name=lcls_idcd]').setValue(record.get('clss_idcd'));
									me.down('[name=lcls_code]').setValue(record.get('lcls_code')); // A-ONE 품목코드
									me.down('[name=lcls_name]').setValue(record.get('clss_name')); // A-ONE 품목코드
									me.finishAction();
								}
							},
						...
						listeners	: {
							itemclick : function (dv, record, item, index, e) {
								...
								me.down('[name=mcls_name]').setValue(record.get('clss_name')); // A-ONE 품목코드
								me.selectRecord( record);
							},
							itemdblclick: function(dataview, index, item, e) {
								if (me.popup.choice && me.popup.choice == 'free') {
									me.down('[name=mcls_idcd]').setValue(record.get('clss_idcd'));
									me.down('[name=mcls_idcd]').setValue(record.get('clss_code')); // A-ONE 품목코드
									me.down('[name=mcls_name]').setValue(record.get('clss_name')); // A-ONE 품목코드
									me.finishAction();
								}
							},
...
//								me.down('[name=mcls_idcd]').setValue(record.get('clss_idcd'));
						...
						listeners	: {
							itemclick : function (dv, record, item, index, e) {
								...
								me.down('[name=scls_name]').setValue(record.get('clss_name')); // A-ONE 품목코드
								me.selectRecord( record);
							},
							itemdblclick: function(dataview, index, item, e) {
								if ((me.popup.choice && me.popup.choice == 'free') || me.down('#itemclass4').hidden) {
									...
									me.down('[name=scls_code]').setValue(index.get('clss_code')); // A-ONE 품목코드
									me.down('[name=scls_name]').setValue(index.get('clss_name')); // A-ONE 품목코드
									me.finishAction();
								}
							},
							selectionchange : function(grid, records ) {
	...
	finishAction: function(){
		...
		if ((me.popup.choice && me.popup.choice == 'free') || panel.hidden) {
			if (me.choiceRecord) {
				...
				if(me.choiceRecord.get('mcls_idcd', base.down('[name=mcls_idcd]')) == '') {
					resource.showError( '중분류를 선택 하여 주시기 바랍니다.'  );
					return false;
				}
				if(me.choiceRecord.get('scls_idcd', base.down('[name=scls_idcd]')) == '') {
					resource.showError( '소분류를 선택 하여 주시기 바랍니다.'  );
					return false;
				}
