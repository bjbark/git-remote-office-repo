/**
 */
Ext.define('lookup.popup.view.WkfwPopup', { extend: 'Axt.popup.Search',
	id 		: 'lookup-wkfw-popup',
	alias	: 'widget.lookup-wkfw-popup',
	store	: 'lookup.popup.store.WkfwPopup',

	title	: Language.get('wkfw_popup','공정 흐름 선택'),
	closable: true,
	autoShow: true,
	width	: 600,
	height	: 500,
								...
								{	xtype	: 'fieldset',
									...
									items	: [
										{	xtype	: 'label',
											...
											margin	: '3 10 0 0',
											flex	: 1,
											emptyText	: '공정흐름코드 또는 공정 흐름명을 입력하세요....',
	...
	createGrid: function(){
		var me = this,
			grid = {
				...
				columns: [
					{	text: Language.get('wkfw_idcd','공정id')	, dataIndex: 'wkfw_idcd',  width : 150, hidden	: true
					},{	text: Language.get('wkfw_code','코드')	, dataIndex: 'wkfw_code',  width :  80
					},{	text: Language.get('wkfw_name','흐름명')	, dataIndex: 'wkfw_name',  flex  : 1
					},{	text: Language.get('remk_text','비고')	, dataIndex: 'remk_text',  width : 120
					}
				],
				listeners: {
		...
		if (selects.length === 0) {
			...
			if (me.popup.apiurl && me.popup.apiurl.master) {
				...
				Ext.each( selects , function( eachrow ){
					request.push({
						wkfw_idcd : eachrow.get('wkfw_idcd')
					});
				});
