/**
 */
Ext.define('lookup.popup.view.ItemPopupNbolt', { extend: 'Axt.popup.Search',

	alias	: 'widget.lookup-item-popup-nbolt',
	store	: 'lookup.popup.store.ItemPopupNbolt',
	requires: [
...
//		}
	...
	createGrid: function(){
			...
			grid = {
				...
				columns: [
					...
					},{	dataIndex:	'item_name'	, width: 145, align : 'left'   , text: Language.get( 'item_name'      , '품명'	),
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					},{	dataIndex:	'cont_pric'	, width: 75, align : 'right'  , text: Language.get( 'cont_pric'      , '계약단가'	),
						renderer: function(value, meta){
							if(me.popup.params.tema){
		...
		var me = this,
			grid = {
				...
				columns: [
					...
					},{	dataIndex:	'item_spec'	, flex : 1, align : 'left'   , text: Language.get( 'item_spec'      , '품목규격'),
						renderer: function(value, meta){
							if(me.popup.params.tema){
								this.columns[5].width = 260;
								meta.style = 'font-size:2em !important;height:30px;line-height:30px'; // applied style for DIV element
	...
	insert:function(){
		var	me = this,
			owner	= Ext.ComponentQuery.query('lookup-item-popup-nbolt')[0],
