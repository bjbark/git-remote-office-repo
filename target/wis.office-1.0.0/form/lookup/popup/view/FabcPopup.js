/**
 */
Ext.define('lookup.popup.view.FabcPopup', { extend: 'Axt.popup.Search',

	alias	: 'widget.lookup-fabc-popup',
	store	: 'lookup.popup.store.FabcPopup',

	title	: Language.get('fabc_popup','원단 찾기'),
	closable: true,
	autoShow: true,
			...
			grid = {
				...
				columns: [
					{	dataIndex:	'line_stat'	, width:  50, align : 'center', text: Language.get( 'line_stat'	, '상태'		) , xtype: 'lookupcolumn' , lookupValue: resource.lookup( 'line_stat' ),
						hidden	: true
					},{	dataIndex:	'fabc_code'	, width: 100, align : 'center', text: Language.get( 'fabc_code'	, '원단코드'	),
					},{	dataIndex:	'fabc_name'	, flex :   1, align : 'left'  , text: Language.get( 'fabc_name'	, '원단명'		),
					...
					},{	dataIndex:	'ppkd_dvcd'	, width:  70, align : 'center', text: Language.get( 'ppkd_dvcd'	, '종별'		), xtype: 'lookupcolumn' , lookupValue: resource.lookup( 'ppkd_dvcd' ),
					},{	dataIndex:	'ppsg_dvcd'	, width:  70, align : 'center', text: Language.get( 'ppsg_dvcd'	, '단종'		), xtype: 'lookupcolumn' , lookupValue: resource.lookup( 'ppsg_dvcd' ),
					},{	dataIndex:	'fabc_idcd'	, width: 100, align : 'left'  , text: Language.get( 'fabc_idcd'	, '원단ID'	),
						hidden : true
					}
				],
				listeners: {
		...
		if (selects.length === 0) {
			...
			if (me.popup.apiurl && me.popup.apiurl.master) {
				...
				Ext.each( selects , function( eachrow ){
					request.push({
						fabc_idcd : eachrow.get('fabc_idcd')
					});
				});
