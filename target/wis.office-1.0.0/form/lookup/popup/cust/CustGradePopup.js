Ext.define('lookup.popup.cust.CustGradePopup', { extend: 'Axt.popup.Search',

	alias   : 'widget.lookup-custgrade-popup',
	id      : 'lookup-custgrade-popup',
	requires: [
	 	'Axt.form.Layout',
	 	'Axt.form.Search',
	 	'Axt.grid.Panel'
	],
	title   : Language.get('cust_grade_popup','고객 등급 정보'),
	closable: true,
	autoShow: true,
	width   : 700,
	height  : 500,
     ...
     createGrid: function(){
        var  me = this,
        	 grid = {
        		...
        		store    : Ext.create('lookup.popup.custinfo.store.CustGradePopup' ),
        		paging   : {
        			xtype: 'grid-paging',
        			items: [
        		 		 '->',
        		 ...
        		 columns:
        		[
	 	         	{ text : Language.get('grade_cd','등급코드')		, dataIndex: 'grade_cd'  , width   : 100},
		         	{ text : Language.get('grade_nm','등급명')		, dataIndex: 'grade_nm'  , width   : 200},
		         	{ text : Language.get('user_memo','메모사항')		, dataIndex: 'user_memo' , flex  : 1},
     ...
     finishAction: function(){
        ...
        if (selected.length === 0) {
        	...
        	if (me.popup.apiurl && me.popup.apiurl.master) {
        		...
        		var store = Ext.create('lookup.popup.custinfo.store.CustGradePopup' );
