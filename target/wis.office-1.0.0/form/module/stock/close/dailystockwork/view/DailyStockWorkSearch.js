Ext.define('module.stock.close.dailystockwork.view.DailyStockWorkSearch', { extend: 'Axt.form.Search',

	alias: 'widget.module-dailystockwork-search',
	initComponent: function(){
		var me = this;
		var wkctLookup = new Array();

		Ext.Ajax.request({
			url		: _global.location.http() + '/stock/close/dailystockwork/get/wkctsearch.do',
			params	: {
		...
		me.items =[ me.addonSearch(wkctLookup)];
		me.callParent();
	},

	addonSearch : function(wkctLookup){
		var me = this,
			line = {
				xtype	: 'fieldset'
					,border	: 0
					...
					,items	: [
						{	xtype: 'fieldset',
							...
							height	: 50,
							items: [
							{	fieldLabel	: Language.get('stok_date','재고일자'),
								xtype		: 'datefield',
								name		: 'invc_date',
								width		: 300,
								height		: 50,
								...
								style		: 'text-align:center',listeners	: {
									change:function(field,value,a){
										var lister		= Ext.ComponentQuery.query('module-dailystockwork-lister')[0],
											layout		= Ext.ComponentQuery.query('module-dailystockwork-layout')[0],
										...
										lister.select({
											callback:function(records, operation, success) {
												...
												stor_id   : _global.stor_id,
												invc_date : Ext.util.Format.date(value,'Ymd')
											})
										);
										mask.hide();
									}
								...
								listConfig	:{
									...
									render : function(field) {
										var lister		= Ext.ComponentQuery.query('module-dailystockwork-lister')[0],
											invc_date	= Ext.util.Format.date(me.down('[name=invc_date]').getValue(), "Ymd"),
											mask		= new Ext.LoadMask(Ext.getBody(), {msg: Const.SELECT.mask }),
											value		= field.value
										;
										mask.show();
									...
									change:function(field,value,a){
										var lister		= Ext.ComponentQuery.query('module-dailystockwork-lister')[0],
											invc_date	= Ext.util.Format.date(me.down('[name=invc_date]').getValue(), "Ymd"),
											layout		= Ext.ComponentQuery.query('module-dailystockworky-layout')[0],
											mask		= new Ext.LoadMask(Ext.getBody(), {msg: Const.SELECT.mask })
										;
										mask.show();
