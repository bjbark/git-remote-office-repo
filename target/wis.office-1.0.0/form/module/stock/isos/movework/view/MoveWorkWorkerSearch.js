Ext.define('module.stock.isos.movework.view.MoveWorkWorkerSearch', { extend: 'Axt.form.Search',

	alias   : 'widget.module-movework-worker-search',
							...
							{	text		: '품명' , xtype : 'label', style : 'text-align:center;'
								...
								popup		: {
									...
									result	: function(records, nameField, pairField) {
										...
										me.selectItem( { records : records } );
										setTimeout(function(){
											panel.down('[name=move_qntt]').focus(true , 10);
										},200);
									},
									create : function (self ) {
										Ext.merge( self.popup.values , {
						...
						items	: [
							{	text	: '단가', xtype : 'label',style:'text-align:center;'
								...
								listeners : {
									change : function(self, value) {
										var panel = self.up('form')
											inv_amt = Number(value) * Number(panel.down('[name=move_qntt]').getValue())
										;
										panel.down('[name=amnt]').setValue(inv_amt)
									},
									keydown : function(self, e) {
										if (e.keyCode == e.ENTER) {
											...
											if(pri > 10000000){
												Ext.Msg.show({ title: '알림', msg: "단가는 10,000,000원 이상 입력하실수 없습니다.", icon: Ext.Msg.INFO, buttons: Ext.Msg.YES, defaultFocus: 'yes', scope: me,
													fn : function (button) {
														self.up('form').down('[name=amnt]').focus(true, 10);
													}
												});
												return;
											}
...
//			me.down('[name=brcd]').focus(true, 10);
			...
			me.down('[name=move_qntt]'	).setValue('1');
	...
	appendItem : function (config) {
		...
		record = Ext.create( store.model.modelName , {
			...
			move_qntt		: me.down('[name=move_qntt]').getValue(),
		...
		store.each(function(findrecord) {
			if (   findrecord.get('item_idcd') == record.get('item_idcd')
				...
				is_equal = true;
				// 상품의 수량을 추가
				findrecord.set("move_qntt", findrecord.get('move_qntt') + record.get('move_qntt'));
			}
		});
		// 상품을 추가
		if (!is_equal) {
