Ext.define('module.custom.dhtec.prod.bomwork.view.BomWorkTree', { extend: 'Ext.tree.Panel',
	alias		: 'widget.module-bomwork-tree',
	store		: 'module.custom.dhtec.prod.bomwork.store.BomWorkTree',
	border	: 0  ,
		...
		var me = this,
			item = {
				...
				items	: [
					...
					{	text : '<span class="write-button">BOM작성</span>'	, action : 'bomInsert'		, cls: 'button-style', width: 80	} ,
					{	text : '<span class="write-button">작업지시</span>'		, action : 'prorAction'		, cls: 'button1-style', width: 80	} ,
					'->',
					'-',
					...
					{	text : '<span class="write-button">설계자료 업로드</span>'	, action : 'testExcel'	, cls: 'button-style', width: 100	} ,
					'-',
				]
			};
		return item ;
	...
	columnItem : function () {
		...
		item = {
				...
				items		: [
					...
					},{ text : '투입공정'	, dataIndex: 'wkct_name'		, width : 100	, hidden		: _global.hq_id.toUpperCase()=='N1000WONTC',
					},{ text : '생산라인'	, dataIndex: 'wkfw_name'		, width : 100	, hidden		: _global.hq_id.toUpperCase()!='N1000WONTC',
		...
		if(select){
			...
			Ext.Msg.show({ title: '확인', msg: '삭제하시겠습니까?', icon: Ext.Msg.QUESTION, buttons: Ext.Msg.YESNO, defaultFocus: 'no', scope: me,
				fn: function (button) {
					if (button=='yes') {
						Ext.Ajax.request({
							url		: _global.location.http() + '/design/bomwork/set/setBomDelete.do',
							params	: {
	...
	popup:function(){
		var	me		= this,
			tree	= me.up('panel'),
			search	= Ext.ComponentQuery.query('module-bomwork-search')[0],
			select	= tree.getSelectionModel().getSelection()[0],
		...
		if(select){
			...
			if(me.itemId == 'insertBom'){
				...
				wkfw_idcd		= select.get('ivst_wkct_idcd');
				wkfw_name		= select.get('ivst_wkct_name');
			...
			var	form = Ext.widget('form', {
				...
				items:[
					{	fieldLabel	: Language.get('item_name','품목'),
						...
						hidden		: _global.hq_id.toUpperCase()=='N1000WONTC',
						popup		: {
							select	: 'SINGLE',
					...
					},{	fieldLabel	: Language.get('wkfw_name','투입공정'),
						xtype		: 'popupfield',
						name		: 'wkfw_name',
						pair		: 'wkfw_idcd',
						labelWidth	: 60,
					...
					},{	xtype		: 'hiddenfield', name : 'wkfw_idcd', value	: wkfw_idcd,
						...
						handler: function() {
							...
							if(_global.hq_id.toUpperCase()=='N1000WONTC'){
								...
								if(set=='insert'){
									...
									ivst_wkct_idcd	= param.wkfw_idcd		;
