Ext.define('module.custom.dhtec.prod.workentry.view.WorkEntryWkctPopup', { extend: 'Axt.popup.Search',
	id 		: 'lookup-dhtec-workenty-wkct-popup',
	alias	: 'widget.lookup-dhtec-workenty-wkct-popup',
	store	: 'module.custom.dhtec.prod.workentry.store.WorkEntryWkctPopup',
	...
	createGrid: function(){
		var me = this,
			grid = {
				...
				columns: [
					{	dataIndex:	'wkct_code'      	, width:  150, align : 'center' , text: Language.get( 'wkct_code'      , '공정코드'           ),
						renderer: function(value, meta){
							meta.style = 'font-size:3em !important;height:50px;line-height:50px;'; // applied style for DIV element
							return value;
						},
					},{	dataIndex:	'wkct_idcd'      	, flex: 100, align : 'left'   , text: Language.get( 'wkct_idcd'      , '설비ID'             ),hidden : true
					},{	dataIndex:	'wkct_name'      	, flex: 100, align : 'left'   , text: Language.get( 'wkct_name'      , '공정명'             ),
						renderer: function(value, meta){
							meta.style = 'font-size:3em !important;height:50px;line-height:50px;'; // applied style for DIV element
							return value;
						},
					},{	dataIndex:	'dept_name'      	, width: 180, align : 'left'   , text: Language.get( 'dept_name'      , '부서명'             ),
						renderer: function(value, meta){
							meta.style = 'font-size:3em !important;height:50px;line-height:50px;'; // applied style for DIV element
							return value;
						},
		...
		if (selects.length === 0) {
			...
			if (me.popup.apiurl && me.popup.apiurl.master) {
				...
				Ext.each( selects , function( eachrow ){
					request.push({
						wkct_idcd : eachrow.get('wkct_idcd')
					});
				});
