Ext.define('module.custom.hantop.mtrl.po.purcisttwork.view.PurcIsttWorkLister', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-purcisttwork-lister',
	store		: 'module.custom.hantop.mtrl.po.purcisttwork.store.PurcIsttWork',
	split		: true,
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{	dataIndex: 'make_cmpy_name'	, text : Language.get('make_cmpy_name'	,'제조회사'		) , width : 120  , align : 'left' ,
					...
					},{	dataIndex: 'make_date'		, text : Language.get('make_date'		,'제조일자'		) , width : 95  , align : 'center' ,
						hidden	: (_global.options.haccp_item_yorn == 0),
						tdCls	: 'editingcolumn',
						editor	: {
							xtype		:'datefield',
								...
								keydown : function(self, e) {
									var index = self.up('grid').view.getSelectionModel().getCurrentPosition().row;
										if(e.keyCode == e.ENTER){
											self.up("grid").plugins[0].startEdit(index+1 , 12);
											...
											var selection = self.up('grid').view.getSelectionModel().getCurrentPosition();
												if(index == (me.getStore().data.length-1) && selection.column == 16){
													...
													self.up("grid").plugins[0].startEdit(index , 12);
												}
										}
								}
							}
