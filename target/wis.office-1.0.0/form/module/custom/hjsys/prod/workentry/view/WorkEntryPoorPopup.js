Ext.define('module.custom.hjsys.prod.workentry.view.WorkEntryPoorPopup', { extend: 'Axt.popup.Search',
	...
	initComponent: function(config){
		...
		//불량 조회
		var store = Ext.ComponentQuery.query('module-workentry-poor')[0].getStore(),
		...
		var me = this,
			form = {
				...
				items			: [
					...
					},{	xtype : 'module-workentry-poor', region:'center' , flex : 1, height:500 ,margin: '0 0 0 0', itemId : 'grid1'
					}
				],
				buttons: [
		...
		var form = Ext.widget('form', {
			...
			items:[
				{	xtype		: 'label',
					...
					listeners:{
						render:function(field ){
							field.getEl().on('click', function( event, el ) {
								var trigger1 = Ext.dom.Query.select('.trigger1qcHjsys')[0];
					...
					trigger1Cls : 'hideCls trigger1qcHjsys',
				},
			],
			buttons: [
	...
	poorupdate : function (rec, rec2, param) {
		...
		if(poor_qntt==0||poor_qntt==''||poor_qntt==null){
			...
			//line_seqn count
			Ext.Ajax.request({
				url		: _global.location.http() + '/custom/hjsys/prod/workentry/get/poorseqn.do',
				params	: {
					token : _global.token_id,
			...
			Ext.Ajax.request({
				url		: _global.location.http() + '/custom/hjsys/prod/workentry/set/poor.do',
				params	: {
					token : _global.token_id,
					param : JSON.stringify({
						_set			: 'insert',
