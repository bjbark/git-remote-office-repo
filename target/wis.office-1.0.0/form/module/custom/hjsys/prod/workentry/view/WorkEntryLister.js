Ext.define('module.custom.hjsys.prod.workentry.view.WorkEntryLister', { extend: 'Axt.grid.Panel',
	id			: 'module-workentry-lister',
	alias		: 'widget.module-workentry-lister',
	store		: 'module.custom.hjsys.prod.workentry.store.WorkEntryLister',
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{	dataIndex: 'wkct_1'		, flex :  1, align : 'center'	,
						renderer: function(value, meta, record){
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_1]').setText(record.get('wkct_1'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
								;
								if(wkct_idcd != ''){
									Ext.defer(function() {
										Ext.widget('button', {
											...
											cls		: 'btn btnTemp '+value.split(',')[1]+' '+_global.options.work_book_tema+'button',
											width	: 70,
											height	: 29,
					...
					},{	dataIndex: 'wkct_2'		, flex :  1, align : 'center'	,
						renderer: function(value, meta,record){
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_2]').setText(record.get('wkct_2'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
					...
					},{	dataIndex: 'wkct_3'		, flex :  1, align : 'center'	,
						renderer: function(value, meta,record){
							var code;
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_3]').setText(record.get('wkct_3'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
					...
					},{	dataIndex: 'wkct_4'		, flex :  1, align : 'center'	,
						renderer: function(value, meta,record){
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_4]').setText(record.get('wkct_4'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
					...
					},{	dataIndex: 'wkct_5'		, flex :  1, align : 'center'	,
						renderer: function(value, meta,record){
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_5]').setText(record.get('wkct_5'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
					...
					},{	dataIndex: 'wkct_6'		, flex :  1, align : 'center'	,
						renderer: function(value, meta,record){
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_6]').setText(record.get('wkct_6'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
					...
					},{	dataIndex: 'wkct_7'		, flex :  1, align : 'center'	,
						renderer: function(value, meta,record){
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_7]').setText(record.get('wkct_7'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
					...
					},{	dataIndex: 'wkct_8'		, flex :  1, align : 'center'	,
						renderer: function(value, meta,record){
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_8]').setText(record.get('wkct_8'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
					...
					},{	dataIndex: 'wkct_9'		, flex :  1, align : 'center'	,
						renderer: function(value, meta,record){
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_9]').setText(record.get('wkct_9'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
					...
					},{	dataIndex: 'wkct_10'		, flex :  1, align : 'center'	,
						renderer: function(value, meta,record){
							if(record.get('wkct') == 'wkct_name'){
								me.down('[dataIndex=wkct_10]').setText(record.get('wkct_10'));
							}
							if(record.get('wkct') == '작업보고'){
								var id = Ext.id(),
									wkct_idcd = value.split(',')[0]
	...
	popup : function (wkct_idcd) {
		var editor = Ext.ComponentQuery.query('module-workentry-editor')[0],
		...
		if(_global.hqof_idcd.toUpperCase() == 'N1000HNSUN'){
			popupAl = 'lookup-user-popup';
		}else{
			popupAl = 'lookup-wkct-popup'
			Ext.Ajax.request({
								...
								{	fieldLabel	: Language.get('cstm_name','수주처'),
									...
									items	: [
										{	fieldLabel	: Language.get('invc_qntt','수주량'),
											...
											fieldCls	: 'textTemp '+_global.options.work_book_tema+'field',
											labelWidth	: 105,
											value		: editor.down('[name=unit_name]').getValue()
										}
									]
								}
							]
								...
								{	fieldLabel	: Language.get('item_name','품명'),
									...
									items	: [
										{	fieldLabel	: Language.get('indn_qntt','지시수량'),
											...
											margin		: '0 0 5 0',
											labelWidth	: 80,
										},{	text : '<span class="btnTemp" style="font-size:2em; font-weight:bold;">검사</span>'  ,
											xtype	: 'button',
											...
											hidden	: true,
											handler	: function(){
//												var form = Ext.getCmp('module-workentry-lister');
//												form.workmtrl(wkct_idcd);
									...
									items	: [
										...
										,{	fieldLabel	: Language.get('good_qntt','양품수량'),
											...
											popup		: {
												select	: 'SINGLE',
												widget	: popupAl,
												params	: { stor_grp : _global.stor_grp , row_sts : '0',tema : _global.options.work_book_tema, find : wkct_code,dept_idcd : '3000'},
												result	: function(records, nameField, pairField) {
													if(popupAl=='lookup-wkct-popup'){
														resource.loadPopup({
			...
			buttons: [
				{	text	: '<span class="btnTemp" style="font-size:2em; font-weight:bold;">재작업</span>',
					...
					handler: function() {
						Ext.Ajax.request({
							url		: _global.location.http() + '/custom/hjsys/prod/workentry/set/poordelete2.do',
							params	: {
								token : _global.token_id,
								param : JSON.stringify({
		...
		if(pror_numb=='' || pror_numb==null){
			...
			Ext.Ajax.request({
				url		: _global.location.http() + '/custom/hjsys/prod/workentry/get/work.do',
				params	: {
					token : _global.token_id,
					param : JSON.stringify({
						stor_id		: _global.stor_id,
...
//	},
	...
	commit : function(wkct_idcd, dvcd){
		var me = this,
			...
			lister2= Ext.ComponentQuery.query('module-workentry-lister2')[0],
			editor = Ext.ComponentQuery.query('module-workentry-editor')[0],
			form   = Ext.getCmp('form1'),
			store  = me.getStore(),
			good_qntt, poor_qntt,invc_date,wker_idcd,prog_stat_dvcd,line_seqn,
