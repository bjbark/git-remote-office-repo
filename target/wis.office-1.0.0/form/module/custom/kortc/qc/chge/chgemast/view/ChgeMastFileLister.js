Ext.define('module.custom.kortc.qc.chge.chgemast.view.ChgeMastFileLister', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-chgemast-filelister'			,
	store		: 'module.custom.kortc.qc.chge.chgemast.store.ChgeMastFile',
					...
					{	fieldLabel	: Language.get('file_name','파일첨부'),					// temp
						...
						popup: {
							...
							result : function(records, nameField, pairField) {
								var store 	= Ext.ComponentQuery.query('module-chgemast-filelister')[0].getStore();
								store.reload();
								var store2 	= Ext.ComponentQuery.query('module-chgemast-lister')[0].getStore();
								store2.reload();
							},
						},
						listeners:{
							render:function(){
								this.popup.params.orgn_dvcd = 'chge_mast';											// table name이 필요하다
							}
						}
					},{	xtype:'button',
						text :'삭제',
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{ dataIndex: 'file_ttle'		, text : Language.get('file_ttle'		,'파일제목')		, width : 120 ,hidden:true
