...
Ext.define('module.custom.kortc.mtrl.po.purcordr2.PurcOrdr2', { extend : 'Axt.app.Controller',
	...
	models:[
		'module.custom.kortc.mtrl.po.purcordr.model.PurcOrdr2Invoice',
		'module.custom.kortc.mtrl.po.purcordr.model.PurcOrdr2Master',
		'module.custom.kortc.mtrl.po.purcordr.model.PurcOrdr2Detail',
		'module.custom.kortc.mtrl.po.purcordr2.model.PurcOrdr2WorkerLister2',
		'module.custom.kortc.mtrl.po.purcordr2.model.PurcOrdr2WorkerLister3',
		'module.custom.kortc.mtrl.po.purcordr2.model.PurcOrdr2Popup2'
	],
	stores:[
		'module.custom.kortc.mtrl.po.purcordr2.store.PurcOrdr2Invoice',
		'module.custom.kortc.mtrl.po.purcordr2.store.PurcOrdr2Master',
		'module.custom.kortc.mtrl.po.purcordr2.store.PurcOrdr2Detail',
		'module.custom.kortc.mtrl.po.purcordr2.store.PurcOrdr2WorkerLister2',
		'module.custom.kortc.mtrl.po.purcordr2.store.PurcOrdr2WorkerLister3',
		'module.custom.kortc.mtrl.po.purcordr2.store.PurcOrdr2Popup2'

	],
	views : [
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2Layout',
		/* 현황 */
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2Search',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2ListerMaster',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2ListerDetail',
		/* 작업 */
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2WorkerEditor',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2WorkerEditor2',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2WorkerSearch',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2WorkerSearch2',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2WorkerLister',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2WorkerLister2',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2WorkerLister3',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2Popup',
		'module.custom.kortc.mtrl.po.purcordr2.view.PurcOrdr2Popup2'
...
//		}
			...
			Ext.Msg.show({ title: '확인', msg: '선택하신 발주를 승인하시겠습니까?', icon: Ext.Msg.QUESTION, buttons: Ext.Msg.YESNO, defaultFocus: 'no', scope: me,
				fn: function (button) {
					if (button=='yes') {
						...
						Ext.each(select, function(record) {
							Ext.Ajax.request({
								url		: _global.location.http() + '/custom/kortc/mtrl/po/purcordr2/set/ok.do',
								params	: {
	...
	okCancelAction:function(callbackFn) {
		...
		if (select) {
			...
			Ext.each(select, function(record) {
				Ext.Ajax.request({
					url			: _global.api_host_info + '/' + _global.app_site + '/custom/kortc/mtrl/po/purcordr2/chk/cancel.do',
					method		: "POST",
					params		: {
						token	: _global.token_id,
						param	: Ext.encode({
...
//			mask.show();
	...
	modifyAction:function() {
		...
		if (select && select.length != 0) {
			Ext.each(select, function(record) {
				if (record.get("acpt_numb") == null || records[0].get("acpt_numb") == '') {
					err_msg = "수주로 등록된 발주는 수정이 불가능합니다.";
				}
			});
...
//						lister.getSelectionModel().select(0)
	...
	updateAction2:function() {
		var me = this,
			...
			chk,
			chk2,
			records	= lister3.getSelectionModel().getSelection()[0],
		...
		for (var i = 0; i < changes; i++) {
			if(lister3.getStore().getUpdatedRecords()[i].data.cstm_name == null){
				chk2 = 1;
				break;
			}
		}
		...
		if(changes != 0){
			...
			if(chk == 2){
				Ext.Msg.alert("알림","거래처명을 반드시입력해주십시오.");
				return;
			}
			...
			for (var a = 0; a < changes; a++) {
				...
				lister3.getStore().sync({
					success : function(operation){
						Ext.Ajax.request({
							url			: _global.location.http() + '/custom/kortc/mtrl/po/purcordr2/set/itemset.do',
							params		: {
								token	: _global.token_id ,
								param	: JSON.stringify({
									hqof_idcd : _global.hqof_idcd,
									invc_numb : record2[0].data.invc_numb,
									purc_date : param.invc_date,
	...
	deleteAction:function() {
		...
		Ext.Msg.confirm("확인", "삭제하시겠습니까?", function(button) {
			if (button == 'yes') {
				...
				Ext.Ajax.request({
					url			: _global.api_host_info + '/' + _global.app_site + '/custom/kortc/mtrl/po/purcordr2/set/del_yn.do',
					method		: "POST",
					params		: {
					 	token	: _global.token_id,
						param	: Ext.encode({
			...
			fn : function (button) {
				if (button==='yes') {
					for (var i = 0; i < records.length; i++) {
						Ext.Ajax.request({
							url			: _global.api_host_info + '/' + _global.app_site + '/custom/kortc/mtrl/po/purcordr2/set/del_yn2.do',
							method		: "POST",
							params		: {
								token	: _global.token_id,
								param	: Ext.encode({
	...
	isttAction:function() {
		...
		if(!select){
			...
			Ext.Msg.confirm("확인", "입고접수 하시겠습니까?", function(button) {
				if (button == 'yes') {
					Ext.Ajax.request({
						url			: _global.api_host_info + '/' + _global.app_site + '/custom/kortc/mtrl/po/purcordr2/set/istt.do',
						method		: "POST",
						params		: {
							token	: _global.token_id,
							param	: Ext.encode({
