Ext.define('module.custom.kortc.sale.order.sordermast.view.SorderMastWorkerSearch', { extend: 'Axt.form.Search',
	store	: 'module.custom.kortc.sale.order.sordermast.store.SorderMastInvoice',
	alias	: 'widget.module-sordermast-worker-search',
								...
								popup		: {
									...
									result	: function(records, nameField, pairField) {
										...
										var panel = nameField.up('form'),
											searchForm = Ext.ComponentQuery.query('module-sordermast-worker-search')[0];
...
//										panel.down('[name=ttsm_amnt]').setValue(inv_tot);
	...
	attachItem : function (config) {
		...
		if (config.clear || !config.record) {
			...
			console.log(record);
			var deli = Ext.Date.format(Ext.ComponentQuery.query('module-sordermast-worker-editor')[0].down('[name=deli_date]').getValue(),'Y-m-d');
			console.log(deli);
...
//			me.down('[name=ttsm_amnt]'	).setValue(record.get('ttsm_amnt' ));
	...
	appendItem : function (config) {
		...
		var amnd_degr = Ext.ComponentQuery.query('module-sordermast-worker-editor')[0].down('[name=amnd_degr]').getValue();
