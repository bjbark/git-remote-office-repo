Ext.define('module.custom.komec.prod.workbook.view.WorkBookFailPopup', { extend: 'Axt.popup.Search',
	id 		: 'lookup-komec-workbook-fail-popup',
	alias	: 'widget.lookup-komec-workbook-fail-popup',
	...
	initComponent: function(config){
		...
		//유실 조회 param값 pror invc_numb로 변경해야함.
		var store = Ext.ComponentQuery.query('module-komec-workbook-fail')[0].getStore(),
		...
		var me = this,
			form = {
				...
				items			: [
					...
					},{	xtype : 'module-komec-workbook-fail', region:'center' , flex : 1, height:500 ,margin: '0 0 0 0', itemId : 'grid1'
					}
				],
				buttons: [
...
//							});
	...
	failupdate : function (val, param) {
		var me = this,
			...
			sttm      = sttm1.replace(':',''),
			edtm      = edtm1.replace(':',''),
			fail      = Ext.ComponentQuery.query('module-komec-workbook-fail')[0],
			store     = fail.getStore(),
			loss_time = 0.
		...
		if(sttm1==null||edtm1==null||sttm1==''||edtm1==''||sttm>edtm){
			...
			Ext.Ajax.request({
				url		: _global.location.http() + '/custom/komec/prod/workbook/set/fail.do',
				params	: {
					token : _global.token_id,
