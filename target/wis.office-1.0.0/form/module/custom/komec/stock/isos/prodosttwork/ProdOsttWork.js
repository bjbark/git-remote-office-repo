...
Ext.define('module.custom.komec.stock.isos.prodosttwork.ProdOsttWork', { extend : 'Axt.app.Controller',
	...
	models:[
		'module.custom.komec.stock.isos.prodosttwork.model.ProdOsttWorkInvoice',
		'module.custom.komec.stock.isos.prodosttwork.model.ProdOsttWorkMaster',
		'module.custom.komec.stock.isos.prodosttwork.model.ProdOsttWorkDetail'
	],
	stores:[
		'module.custom.komec.stock.isos.prodosttwork.store.ProdOsttWorkInvoice',
		'module.custom.komec.stock.isos.prodosttwork.store.ProdOsttWorkMaster',
		'module.custom.komec.stock.isos.prodosttwork.store.ProdOsttWorkDetail'
	],
	views : [
		'module.custom.komec.stock.isos.prodosttwork.view.ProdOsttWorkLayout',
		/* 현황 */
		'module.custom.komec.stock.isos.prodosttwork.view.ProdOsttWorkSearch',
		'module.custom.komec.stock.isos.prodosttwork.view.ProdOsttWorkListerMaster',
		'module.custom.komec.stock.isos.prodosttwork.view.ProdOsttWorkListerDetail',
		/* 작업 */
		'module.custom.komec.stock.isos.prodosttwork.view.ProdOsttWorkWorkerEditor',
		'module.custom.komec.stock.isos.prodosttwork.view.ProdOsttWorkWorkerSearch',
		'module.custom.komec.stock.isos.prodosttwork.view.ProdOsttWorkWorkerLister'
	...
	init: function() {
		...
		me.control({
			...
			'module-prodosttwork-worker-search button[action=selectAction2]'		: { click : me.selectAction2      }, /* 바코드조회 */
	...
	pocket : {
		...
		worker : {
			...
			search : function () { return Ext.ComponentQuery.query('module-prodosttwork-worker-search')[0] }
		},
		lister : {
	...
	selectAction2 : function() {
		...
		Ext.Ajax.request({
			url		: _global.location.http() + '/custom/komec/stock/isos/prodosttwork/get/lotchk.do',
			params	: {
				token : _global.token_id,
				param : JSON.stringify({
					lott_numb		: param.barcode_pono,
				})
			},
			async	: false,
			method	: 'POST',
			success	: function(response, request) {
				...
				if	(!result.success ){
					...
					if(result.records[0].count == 0){
						resource.showError( '입고되어있지않는 lot번호입니다. 확인 후 진행 하세요.'  );
						return;
					}
				}
				Ext.Ajax.request({
					url			: _global.location.http() + '/custom/komec/stock/isos/prodosttwork/get/lot.do',
					method		: "POST",
					params		: {
						token	: _global.token_id,
						param	: Ext.encode({
...
//		}, Ext.merge( param, {stor_id : _global.stor_id, hq_id : _global.hq_id}) );
	...
	updateAction:function() {
		...
		if (editor.down('[name=ostt_wrhs_idcd]').getValue() == null || editor.down('[name=ostt_wrhs_idcd]').getValue() == '') {
			Ext.Msg.alert("알림", "출고창고를 반드시 선택해주세요.");
			return;
		}
	...
	deleteAction:function() {
		...
		Ext.Msg.confirm("확인", "삭제하시겠습니까?", function(button) {
			if (button == 'yes') {
				...
				Ext.Ajax.request({
					url : _global.api_host_info + '/' + _global.app_site + '/custom/komec/stock/isos/prodosttwork/set/del_yn.do',
					method : "POST",
					params : {
					 	token : _global.token_id,
						param : Ext.encode({
	...
	invReportAction:function(button) {
		...
		if(select.length === 1) {
			...
			resource.loadPrint({
				...
				requrl 			: {
					search : _global.api_host_info + '/' + _global.app_site +'/custom/komec/stock/isos/prodosttwork/get/printing.do',
				},
