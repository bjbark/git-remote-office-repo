 ...
 * 파일 업로드 공통 팝업
 */
Ext.define('module.custom.dehansol.sale.saleorder.view.FileUpload', { extend: 'Axt.popup.Upload',
...
//							hidden		: true
	...
	finishAction : function (button) {
		...
		form= me.down('[name=numb]').getValue(),
...
//		param.chk		= chk;
		...
		var a = Ext.ComponentQuery.query('#invc_date')[0].getValue(); //기준일자
		if	(a== null || a==''){
			Ext.Msg.show({ title: '알림', msg: '수주일자는 반드시 입력해야합니다.', icon: Ext.Msg.INFO, buttons: Ext.Msg.YES, defaultFocus: 'yes', scope: me });
			return;
		}
		// submit할 form가져오기
