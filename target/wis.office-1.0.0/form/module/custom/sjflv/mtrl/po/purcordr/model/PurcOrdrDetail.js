Ext.define('module.custom.sjflv.mtrl.po.purcordr.model.PurcOrdrDetail', { extend: 'Axt.data.Model',
	fields: [
		...
		},{	name: 'vatx_dvcd'			, type: 'string'	/* 단위명		*/
		},{	name: 'need_qntt'			, type: 'float'		/* 소요수량		*/
		},{	name: 'each_qntt'			, type: 'float'		/* Each수량	*/
		},{	name: 'pack_qntt'			, type: 'float'		/* Pacj수량	*/
		...
		},{	name: 'offr_vatx2'			, type: 'float'		/* 발주부가세		*/
	...
	recalculation: function(inv,context) {
		var row = this
...
//		if(field!="deli_date2"&&field!="user_memo"){
//			if(resId == 'SJFLV' ){
//				var offr_amnt;
//
//				if(editor.down('[name=supl_dvcd]').value == '6000' ){
//					if (_global.hq_id.toUpperCase() == 'N1000SJFLV') {
//						offr_amnt = Number((Number(row.get('offr_qntt')) * Number(row.get('offr_pric'))).toFixed(2));
//					} else {
//						offr_amnt = Number((Number(row.get('offr_qntt')) * Number(row.get('offr_pric'))).toFixed(3));
//					}
//
//					row.set('offr_amnt'	, offr_amnt);
//					row.set('offr_vatx'	, 0);
//					row.set('ttsm_amnt'	, offr_amnt) ;
//				}else{
//					offr_amnt = Number((Number(row.get('offr_qntt')) * Number(row.get('offr_pric'))).toFixed(0));
//					row.set('offr_amnt'	, offr_amnt);
//					if(field=="offr_vatx"){
//						row.set('offr_vatx'	, Number(context.value));
//					}else{
//						row.set('offr_vatx'	, offr_amnt/Number(_global.tax_rt));
//					}
//					row.set('ttsm_amnt'	, offr_amnt + Number(row.get('offr_vatx')));
//				}
//			} else{
...
//				if(editor.down('[name=supl_dvcd]').value == '6000' ){
//					row.set('offr_vatx'	, 0);
//					row.set('ttsm_amnt'	, row.get('offr_amnt')) ;
//				}else{
...
//
//		}
//		거래처 비과세 과세 여부로 부과세여부를 적용시켜 조달구분 조건과 겹치므로 임시로 주석처리함. 2023.06.30 양효선
	}
});