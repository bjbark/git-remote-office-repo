Ext.define('module.custom.sjflv.mtrl.isttcalc.purcrettmast.view.PurcRettMastWorkerLister', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-purcrettmast-worker-lister',
	split		: true,
	...
	initComponent: function () {
		var me = this;
		me.dockedItems = [{xtype: 'module-purcrettmast-worker-search'}];
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{	dataIndex: 'invc_date'	, text : Language.get('invc_date'		,'입고일자'	) , width :  80 , align : 'center'	
					},{	dataIndex: 'cstm_idcd'	, text : Language.get('cstm_idcd'		,'거래처id'	) , width : 250 , align : 'left' , hidden:true
					},{	dataIndex: 'cstm_name'	, text : Language.get('cstm_name'		,'거래처명'	) , width : 160 , align : 'left'
					},{	dataIndex: 'invc_numb'	, text : Language.get('invc_numb'		,'입고번호'	) , width :  95 , align : 'center'
					...
					},{ dataIndex: 'istt_qntt'	, text : Language.get('istt_qntt'		,'입고수량'	) , width :  80 , xtype : 'numericcolumn', summaryType: 'sum', format: _global.hq_id.toUpperCase()!='N1000SJUNG'?'#,##0.##':'#,##0.###',
					},{ dataIndex: 'unpaid'		, text : Language.get('unpaid'			,'미반품수량'	) , width :  80 , xtype : 'numericcolumn', format: _global.hq_id.toUpperCase()!='N1000SJUNG'?'#,##0.##':'#,##0.###',
					},{ dataIndex: 'rett_qntt'	, text : Language.get('rett_qntt'		,'반품수량'	) , width :  80 , xtype : 'numericcolumn', format: _global.hq_id.toUpperCase()!='N1000SJUNG'?'#,##0.##':'#,##0.###',
						 tdCls	: 'editingcolumn',
						editor	: {
							xtype		:'textfield',
							selectOnFocus: true,
...
//						}
//					}
					{ dataIndex	: 'rett_memo'		, text : Language.get('rett_memo'		,'반품메모'	) , width : 180,
						tdCls		: 'editingcolumn',
						editor		: {
							xtype		:'textfield',
							selectOnFocus: true,
					...
					},{ dataIndex: 'lott_numb'		, text : Language.get('lott_numb'		,'Batch 번호'	) , width : 100
//						, tdCls	: 'editingcolumn',
//						editor	: {
//							xtype		:'textfield',
//							selectOnFocus: true,
...
//							}
//						}
					},{ dataIndex: 'istt_pric'		, text : Language.get('istt_pric'		,'단가'		) , width : 80  , xtype : 'numericcolumn', hidden: false
					...
					},{	dataIndex: 'purc_invc_date'	, text : Language.get('purc_invc_date'	,'발주일자'	) , width : 80 , align : 'center'
					},{	dataIndex: 'purc_invc_numb'	, text : Language.get('purc_invc_numb'	,'발주번호'	) , width : 95 , align : 'center' , align : 'left', xtype : 'lookupcolumn', lookupValue : resource.lookup('acpt_dvcd'),
					}
				]
			}
		;
	...
	cellEditAfter  : function (editor, context,rowIndexNum) {
		...
		if(ostt_qntt<rett_qntt || rett_qntt<0){
			...
			pric		= me.getStore().getAt(rowIndex).get('istt_pric');		//개당단가
