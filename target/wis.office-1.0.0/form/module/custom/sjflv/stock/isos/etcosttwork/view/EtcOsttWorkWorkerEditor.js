...
Ext.define('module.custom.sjflv.stock.isos.etcosttwork.view.EtcOsttWorkWorkerEditor', { extend: 'Axt.form.Editor',

	alias: 'widget.module-etcosttwork-worker-editor',
	height : 230,
	header : false,
	getStore : function() {
		return Ext.getStore( 'module.custom.sjflv.stock.isos.etcosttwork.store.EtcOsttWorkInvoice' );
	},
		...
		var me	= this,
			...
			items		: [
				{	fieldLabel	: Language.get('', '출고유형' ),
					...
					// 2023.08.07 - 이강훈 - 출고처가 거래처인 경우 거래처 찾기 화면이 팝업되도록 처리, 기존 출고처명 팝업 기능  주석 처리.
					listeners	: {
						change:function(val){
								...
								this.up('form').down('[name=cstm_name]').popup = {
									...
									create : function (self ) {
										...
										if (cstm_dvcd == '' || cstm_dvcd == null ){
											Ext.Msg.alert("알림","출고처구분을 먼저 선택하여 주십시오.");
											self.close();
											return;
										} else {
