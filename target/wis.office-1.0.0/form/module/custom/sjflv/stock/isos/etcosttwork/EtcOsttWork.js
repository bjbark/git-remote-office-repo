...
Ext.define('module.custom.sjflv.stock.isos.etcosttwork.EtcOsttWork', { extend : 'Axt.app.Controller',
	...
	models:[
		'module.custom.sjflv.stock.isos.etcosttwork.model.EtcOsttWorkInvoice',
		'module.custom.sjflv.stock.isos.etcosttwork.model.EtcOsttWorkMaster',
		'module.custom.sjflv.stock.isos.etcosttwork.model.EtcOsttWorkDetail'
	],
	stores:[
		'module.custom.sjflv.stock.isos.etcosttwork.store.EtcOsttWorkInvoice',
		'module.custom.sjflv.stock.isos.etcosttwork.store.EtcOsttWorkMaster',
		'module.custom.sjflv.stock.isos.etcosttwork.store.EtcOsttWorkDetail'
	],
	views : [
		'module.custom.sjflv.stock.isos.etcosttwork.view.EtcOsttWorkLayout',
		/* 현황 */
		'module.custom.sjflv.stock.isos.etcosttwork.view.EtcOsttWorkSearch',
		'module.custom.sjflv.stock.isos.etcosttwork.view.EtcOsttWorkListerMaster',
		'module.custom.sjflv.stock.isos.etcosttwork.view.EtcOsttWorkListerDetail',
		/* 작업 */
		'module.custom.sjflv.stock.isos.etcosttwork.view.EtcOsttWorkWorkerEditor',
		'module.custom.sjflv.stock.isos.etcosttwork.view.EtcOsttWorkWorkerSearch',
		'module.custom.sjflv.stock.isos.etcosttwork.view.EtcOsttWorkWorkerLister'

	],
	initPermission: function(workspace, permission) {
		this.callParent([workspace, permission]);
		this.joinPermission(workspace.down('module-etcosttwork-lister-master button[action=etcPrintAction]'),  Const.PERMIT.EXPORT);
	},
	init: function() {
		var me = this;
		me.control({
			'module-etcosttwork-layout button[action=selectAction]'					: { click : me.selectAction      }, /* 조회 */

			'module-etcosttwork-lister-master menuitem[action=closeAction]'			: { click : me.closeAction       }, /* 마감 */
			'module-etcosttwork-lister-master menuitem[action=closecancelAction]'	: { click : me.closecancelAction }, /* 마감취소 */

			'module-etcosttwork-lister-master button[action=etcPrintAction]'		: { click : me.invReportAction   }, /* 명세서 발행 */
			'module-etcosttwork-lister-master button[action=insertAction]'			: { click : me.insertAction      }, /* 등록 */
			'module-etcosttwork-lister-master button[action=modifyAction]'			: { click : me.modifyAction      }, /* 수정 */
			'module-etcosttwork-lister-master button[action=exportAction]'			: { click : me.exportAction      }, /* 엑셀 */
			'module-etcosttwork-lister-master button[action=deleteAction]'			: { click : me.deleteAction      }, /* 삭제 */

			'module-etcosttwork-lister-detail button[action=deleteAction]'			: { click : me.deleteAction2     }, /* 디테일삭제 */
			'module-etcosttwork-lister-detail button[action=exportAction]'			: { click : me.exportDetailAction}, /* 엑셀 */

			'module-etcosttwork-worker-lister button[action=updateAction]'			: { click : me.updateAction      }, /* 저장 */
			'module-etcosttwork-worker-lister button[action=cancelAction]'			: { click : me.cancelAction      }, /* 취소 */
			'module-etcosttwork-lister-master' : {
	...
	pocket : {
		layout : function () { return Ext.ComponentQuery.query('module-etcosttwork-layout')[0] },
		search : function () { return Ext.ComponentQuery.query('module-etcosttwork-search')[0] },
		worker : {
			editor : function () { return Ext.ComponentQuery.query('module-etcosttwork-worker-editor')[0] },
			lister : function () { return Ext.ComponentQuery.query('module-etcosttwork-worker-lister')[0] }
		},
		lister : {
			master  : function () { return Ext.ComponentQuery.query('module-etcosttwork-lister-master')[0] },
			detail  : function () { return Ext.ComponentQuery.query('module-etcosttwork-lister-detail')[0] }
		}
	},
	...
	selectAction:function(callbackFn) {
		...
		mask.show();
		var proc_drtr_idcd = me.pocket.search().getValues().proc_drtr_idcd;
		lister.select({
		...
		}, Ext.merge(me.pocket.search().getValues(), { stor_grp : _global.stor_grp , proc_drtr_idcd : proc_drtr_idcd}));
	},
	...
	updateAction:function() {
		...
		editor.updateRecord({
			...
			Ext.ComponentQuery.query('module-etcosttwork-worker-search')[0].getForm().reset();
			}
		});
	},
	...
	deleteAction:function() {
		...
		Ext.Msg.confirm("확인", "삭제하시겠습니까?", function(button) {
			if (button == 'yes') {
				...
				Ext.Ajax.request({
					url : _global.api_host_info + '/' + _global.app_site + '/custom/sjflv/stock/etcosttwork/set/del_yn.do',
					method : "POST",
					params : {
					 	token : _global.token_id,
						param : Ext.encode({
	...
	deleteAction2:function() {
		...
		var err_msg = "";
		var records = master.getSelectionModel().getSelection();
		var records2 = detail.getSelectionModel().getSelection();

		console.log(records);

		if (!records2 || records2.length!=1) {
			Ext.Msg.alert("알림", "삭제 하시려는 1건을 선택 후 진행하십시오!");
			return;
		}
...
//
				Ext.Ajax.request({
					url : _global.api_host_info + '/' + _global.app_site + '/custom/sjflv/stock/etcosttwork/set/del_yn2.do',
					method : "POST",
					params : {
					 	token : _global.token_id,
						param : Ext.encode({
							invc_numb	: records2[0].get('invc_numb'),
							line_seqn	: records2[0].get('line_seqn'),
							lott_numb	: records2[0].get('lott_numb'),
							qty			: records2[0].get('ostt_qntt'),
							upt_usr_nm	: _global.login_pk
						})
					},
					success : function(response, request) {
	...
	closeAction:function(callbackFn) {
		...
		if (select && select.length != 0) {
			...
			Ext.Msg.alert("알림", "마감할 출고 목록을 선택하여 주시기 바랍니다.");
			return;
		}
			...
			Ext.Msg.show({ title: '확인', msg: '선택하신 내역을 마감하시겠습니까?', icon: Ext.Msg.QUESTION, buttons: Ext.Msg.YESNO, defaultFocus: 'no', scope: me,
				fn: function (button) {
					if (button=='yes') {
						...
						Ext.each(select, function(record) {
							Ext.Ajax.request({
								url		: _global.location.http() + '/custom/sjflv/stock/etcosttwork/set/close.do',
								params	: {
									token : _global.token_id,
									param : JSON.stringify({
										invc_numb		: record.get('invc_numb'),
	...
	closecancelAction:function(callbackFn) {
		...
		if (select && select.length != 0) {
			...
			Ext.Msg.alert("알림", "마감 해지할 출고내역을 선택하여 주시기 바랍니다.");
			return;
		}
	...
	reportAction : function(button) {
		...
		Ext.widget('popup-report',{
			title: '마감리포트',
			url: _global.api_host_info + '/system/recv/etcosttwork/get/report.do',
			params: { param :
				JSON.stringify({
	...
	invReportAction:function(button) {
		...
		if(select.length === 1) {
			...
			resource.loadPrint({
				...
				requrl 			: {
					search : _global.api_host_info + '/' + _global.app_site +'/custom/sjflv/stock/etcosttwork/get/printing.do',
				},
