Ext.define('module.custom.sjflv.stock.isos.goodsosttwork.view.GoodsOsttWorkWorkerLister', { extend: 'Axt.grid.Panel',
	...
	initComponent: function () {
		var me = this;
		me.dockedItems = [{xtype: 'module-goodsosttwork-worker-search'}];
...
//		var v = Math.floor(amnt*Number(_global.tax_rt)/1000)*10;				//부가세
		...
		if(d == a){
			models[rowIndex].set('new_sale_amnt',grid.getStore().getAt(rowIndex).get('sale_amnt'));
			models[rowIndex].set('new_vatx_amnt',grid.getStore().getAt(rowIndex).get('vatx_amnt'));
			models[rowIndex].set('new_ttsm_amnt',grid.getStore().getAt(rowIndex).get('ttsm_amnt'));
		} else if(d == 0){
