Ext.define('module.custom.sjflv.stock.isos.etcisttwork.view.EtcIsttWorkLabelPopup', { extend: 'Axt.popup.Search',
	id 		: 'module-etcisttwork-label-popup',
	alias	: 'widget.module-etcisttwork-label-popup',
...
//				store		: Ext.create(me.store),
				store		: Ext.getStore('module.custom.sjflv.stock.isos.etcisttwork.store.EtcIsttWorkLabelPopup'),
				defaults: {style: 'text-align:center'},
				listeners	:{
					afterrender:function(){
						var store = Ext.getStore('module.custom.sjflv.stock.isos.etcisttwork.store.EtcIsttWorkLabelPopup');
						store.clearData();
						store.removeAll();
						store.commitChanges();
				...
				columns: [
					...
					},{	dataIndex: 'istt_qntt'		, text : Language.get('istt_qntt'		,'입고수량'		) , width : 80  , align : 'right', xtype : 'numericcolumn'
					},{	dataIndex: 'qntt'			, text : Language.get('qntt'			,'분할수량'		) , width : 80  , align : 'right', xtype : 'numericcolumn',
						tdCls	: 'editingcolumn',
						editor	: {
							xtype		:'numericfield',
							selectOnFocus	: true,
	...
	finishAction: function(){
		...
		a+= "]";
		if(a != "[]"){
			var _param = '_param~{\'type\':\'P\' ,\'records\':'+a+'}~';
			var arg = _param;
