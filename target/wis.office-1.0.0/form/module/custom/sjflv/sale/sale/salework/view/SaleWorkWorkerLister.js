Ext.define('module.custom.sjflv.sale.sale.salework.view.SaleWorkWorkerLister', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-sjflv-salework-worker-lister',
	...
	initComponent: function () {
		var me = this;
		me.dockedItems = [{xtype: 'module-sjflv-salework-worker-search'}];
...
//										record.set('vatx',record.get('vatx2'));
										record.set('vatx',record.get('deff_amnt')*0.1);
										record.set('ttsm',(record.get('deff_amnt')+record.get('vatx')));
									}else{
										record.set('sply',record.get('deff_amnt'));
										record.set('amnt',record.get('deff_amnt'));
										record.set('vatx',0);
					...
					},{	dataIndex:	'sale_deff_amnt', width: 110, align : 'right'	, text: Language.get( ''			, '(매출)미발행금액'	), xtype : 'numericcolumn', summaryType: 'sum'
					},{	dataIndex:	'rett_deff_amnt', width: 110, align : 'right'	, text: Language.get( ''			, '(반품)미발행금액'	), xtype : 'numericcolumn', summaryType: 'sum'
					},{	dataIndex:	'deff_amnt'		, width: 110, align : 'right'	, text: Language.get( 'deff_amnt'	, '미발행금액'		), xtype : 'numericcolumn', summaryType: 'sum'
					},{	dataIndex:	'sply'			, width: 110, align : 'right'	, text: Language.get( 'sply'		, '발행금액'		), xtype : 'numericcolumn', summaryType: 'sum'
						, tdCls	: 'editingcolumn',
						editor	: {
							xtype		:'numericfield',
							selectOnFocus: true,
					...
					},{	dataIndex:	'amnt'			, width: 110, align : 'right'	, text: Language.get( ''	, '공급가'		), xtype : 'numericcolumn', summaryType: 'sum',
					},{	dataIndex:	'vatx'			, width: 100, align : 'right'	, text: Language.get( ''	, '부가세'		), xtype : 'numericcolumn', summaryType: 'sum',
//					},{	dataIndex:	'vatx2'			, width: 100, align : 'right'	, text: Language.get( ''	, '부가세2'		), xtype : 'numericcolumn', summaryType: 'sum',hidden:true
					},{	dataIndex:	'ttsm'			, width: 120, align : 'right'	, text: Language.get( ''	, '합계'		), xtype : 'numericcolumn', summaryType: 'sum',
	...
	cellEditAfter  : function (editor, context) {
		...
		if(context.field == "sply"){
			...
			if(search.down('[name=vatx_dvcd]').getValue() == '1'){
				...
				models[rowIndexNum].set('vatx',(context.value/10));
