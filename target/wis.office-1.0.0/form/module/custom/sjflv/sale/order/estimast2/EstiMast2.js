Ext.define('module.custom.sjflv.sale.order.estimast2.EstiMast2', { extend : 'Axt.app.Controller',
	...
	models:[
		'module.custom.sjflv.sale.order.estimast2.model.EstiMast2Invoice',
		'module.custom.sjflv.sale.order.estimast2.model.EstiMast2Master',
		'module.custom.sjflv.sale.order.estimast2.model.EstiMast2Detail',
		'module.custom.sjflv.sale.order.estimast2.model.EstiMast2File'
	],
	stores:[
		'module.custom.sjflv.sale.order.estimast2.store.EstiMast2Invoice',
		'module.custom.sjflv.sale.order.estimast2.store.EstiMast2Master',
		'module.custom.sjflv.sale.order.estimast2.store.EstiMast2Detail',
		'module.custom.sjflv.sale.order.estimast2.store.EstiMast2File'
	],
	views : [
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2Layout',
		/* 현황 */
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2Search',
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2ListerMaster',
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2ListerDetail',
		/* 작업 */
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2WorkerEditor',
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2WorkerSearch',
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2WorkerLister',
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2Lister2',
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2CopyPopup',
		'module.custom.sjflv.sale.order.estimast2.view.EstiMast2PrintPopup',
	],
	...
	init: function() {
		...
		me.control({
			...
			'module-estimast2-lister-master menuitem[action=closeCancelAction]'		: { click : me.closeCancelAction  }, /* 마감취소 */
			'module-estimast2-lister-master button[action=copyAction]'				: { click : me.copyAction         }, /* 견적복사 */
			'module-estimast2-lister-master button[action=orderAction]'				: { click : me.orderAction        }, /* 수주등록 */
			...
			'module-estimast2-worker-lister button[action=deleteAction]	'			: { click : me.deleteAction2       }, /* 디테일삭제 */
	...
	pocket : {
		...
		worker : {
			...
			search : function () { return Ext.ComponentQuery.query('module-estimast2-worker-search')[0] }
		},
		lister : {
...
//							record.store.commitChanges();
	...
	orderAction:function() {
		...
		if(select){
			...
			if  (Ext.isEmpty(select.get('cstm_idcd'))) {
			...
			Ext.Msg.confirm("확인", "수주등록을 하시겠습니까?", function(button) {
				if (button == 'yes') {
					Ext.Ajax.request({
						url			: _global.api_host_info + '/' + _global.app_site + '/custom/sjflv/sale/order/estimast2/set/acpt.do',
						method		: "POST",
						params		: {
							token	: _global.token_id,
							param	: Ext.encode({
	...
	printAction:function() {
		...
		if(resId == 'N1000SJUNG'){
			if (select) {
				...
				Ext.Msg.alert("알림", "발행할 견적서를 선택해주십시오.");
			}
		}else{
			var err_msg = "";
	...
	insertAction:function() {
		...
		editor.insertRecord({
			...
			callback	: function (results){
				if  (results.success){
					editor.down('[name=new_ordr]').setValue('신규');
...
//		});
	...
	modifyAction:function() {
		...
		if (select){
			...
			editor.modifyRecord({
				...
				callback: function( results ) {
					if (results.success){
						editor.down('[name=new_ordr]').setValue('수정');
						me.pocket.layout().getLayout().setActiveItem(1);
	...
	updateAction:function() {
		...
		if((editor.getValues().invc_numb != '' || editor.getValues().invc_numb) && editor.getValues().new_ordr === '신규'){
			//견적번호 중복체크
			Ext.Ajax.request({
				url			: _global.api_host_info + '/' + _global.app_site + '/custom/sjflv/sale/order/estimast2/set/invoiceCheck.do',
				method		: "POST",
				params		: {
				 	token	: _global.token_id,
					param	: Ext.encode({
...
//												detail.getStore().reload();
							...
							Ext.Msg.alert("알림", "견적번호가 이미 있습니다.");
						}
					} else {
						Ext.Msg.show({ msg : result.message, buttons : Ext.Msg.YES, icon : Ext.Msg.ERROR });
					}
		...
		if (select) {
			...
			Ext.Msg.confirm("확인", "삭제하시겠습니까?", function(button) {
				if (button == 'yes') {
					...
					Ext.Ajax.request({
						url			: _global.api_host_info + '/' + _global.app_site + '/custom/sjflv/sale/order/estimast2/get/checkLastSeqn.do',
						method		: "POST",
						params		: {
						 	token	: _global.token_id,
							param	: Ext.encode({
								hqof_idcd : _global.hq_id,
								last_idcd : select.data.invc_numb,
							})
						},
						success : function(response, request) {
							var object = response,
							...
							;
							if (result.success) {
								var last_yorn = result.records[0]['cnt'];

								var currentNumber = select.data.invc_numb;
								var prefix = currentNumber.split('-')[0]; // "NO"
								var numberPart = currentNumber.split('-')[1]; // "0002"
								var currentSeqn = parseInt(numberPart, 10); // 숫자 변환: 2

								var last_seqn = currentSeqn - 1;
								var last_idcd = prefix + '-' + Ext.String.leftPad(last_seqn, 4, '0');
								var code_last_seqn = last_seqn;
								var last_code =  Ext.String.leftPad(last_seqn, 4, '0');

								Ext.Ajax.request({
									url			: _global.api_host_info + '/' + _global.app_site + '/custom/sjflv/sale/order/estimast2/set/del_yn.do',
									method		: "POST",
									params		: {
									 	token	: _global.token_id,
										param	: Ext.encode({
