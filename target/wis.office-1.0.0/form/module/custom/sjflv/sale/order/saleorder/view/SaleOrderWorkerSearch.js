Ext.define('module.custom.sjflv.sale.order.saleorder.view.SaleOrderWorkerSearch', { extend: 'Axt.form.Search',
	store	: 'module.custom.sjflv.sale.order.saleorder.store.SaleOrderInvoice',
	alias	: 'widget.module-sjflv-saleorder-worker-search',
							...
							{	text		: '품목코드' , xtype : 'label',	style : 'text-align:center;',
								...
								popup		: {
									...
									result	: function(records, nameField, pairField) {
										...
										panel.down('[name=vatx_incl_yorn]').setValue(records[0].get('vatx_incl_yorn'));
									},
...
//								fieldCls	: 'readonlyfield',
								...
								listeners: {
									change : function(self, value) {
										...
										if (vatx_incl_yorn == "1") {
											inv_vat = Math.round( Number(panel.down('[name=sply_amnt]').getValue())/(Number(_global.tax_rt)))
										}
										panel.down('[name=vatx_amnt]').setValue(inv_vat);

										var	inv_tot = Number(panel.down('[name=sply_amnt]').getValue()) + inv_vat;
						...
						items	: [
							{	text		: '부가세', xtype : 'label',style:  'text-align:center;'
								...
								/* 23.07.05 - 부가세 변경 시 합계금액 계산 위치 변경, 금액변경 시 부가세 계산 후 합계금액 계산
								listeners	: {
									change	: function(self, value) {
										var panel = self.up('form')
						...
						width	: 60,
						items	: [
							{	text		: '부가세포함', xtype : 'label',style:  'text-align:center;',hidden		: _global.options.mes_system_type.toUpperCase() != 'SJFLV',
							},{	xtype		: 'lookupfield',
								name		: 'vatx_incl_yorn',
								margin		: '2 2 2 2',
								...
								listeners	: {
									change	: function(self, value) {
										...
										var	inv_tot = Number(panel.down('[name=sply_amnt]').getValue()) + inv_vatx;
										panel.down('[name=invc_amnt]').setValue(inv_tot);
									}
								}
							}
										...
										if (e.keyCode == e.ENTER) {
											...
											if ( Ext.isEmpty( self.up('form').down('[name=deli_date2]').getValue()) ){
												...
												if(_global.hq_id.toUpperCase() != 'N1000SJFLV'){
													self.up('form').down('[name=prod_trst_dvcd]').focus(true, 10);
												}else{
													if (self.up('form').down('[name=item_idcd]').getValue().trim() != '') {
														me.appendItem({});
													}
						...
						width	: 120,
						items	: [
							{	text		: '생산구분' , xtype : 'label', style : 'text-align:center;'
							},{
								xtype		: 'lookupfield',
								name		: 'prod_trst_dvcd',
								margin		: '2 2 2 2',
								lookupValue	: resource.lookup('prod_trst_dvcd'),
								readOnly	: _global.hq_id.toUpperCase() == 'N1000SJFLV'? true : false,
								listeners: {
										...
										self.el.on('keydown', function(e) {
											if (e.keyCode == e.ENTER) {
												...
												if(_global.hq_id.toUpperCase() == 'N1000SJUNG'){
													var edit = Ext.ComponentQuery.query('module-sjflv-saleorder-worker-editor')[0];
													console.log(edit.down('[name=acpt_dvcd]').getValue());
													if(edit.down('[name=acpt_dvcd]').getValue() === '1000'){
														me.down('[name=prod_trst_dvcd]').setValue('3000');
													}else{
														me.down('[name=prod_trst_dvcd]').setValue('1000');
													}
												}
											} else if (e.keyCode == e.ESC) {
												me.attachItem({ clear : true });
...
//				me.down('[name=invc_amnt]'		).setValue(record.get('invc_amnt'));
			...
			if(_global.hq_id.toUpperCase()=='N1000SJFLV'){
				...
				if(_global.hq_id.toUpperCase() == 'N1000SJUNG'){
					var editSearch = Ext.ComponentQuery.query('module-sjflv-saleorder-worker-editor')[0];
					if(editSearch.down('[name=acpt_dvcd]').getValue() == "1000"){
						me.down('[name=prod_trst_dvcd]').setValue('3000');
					}else{
						me.down('[name=prod_trst_dvcd]').setValue('1000');
					}
			...
			if (config.append) {
				...
				html = '<div>'
					 ...
					 + '  </div> '
					 + '  <div style = "width: 150;  float : left" >'
					 + '  	<div>재고창고</div><div>'+ record.get('stor_wrhs_name') +'</div> '
					 + '  </div> '
					 + '  <div style = "width: 150;  float : left" >'
...
//		var vatx_incl_yorn = '';
//		if(resId == 'SJFLV'){
//			if(me.down('[name=vatx_incl_yorn]').getValue() == false ) {
//				vatx_incl_yorn = '0';
//			}	else if(me.down('[name=vatx_incl_yorn]').getValue() == false){
//				vatx_incl_yorn = '1';
//			}
//		}
