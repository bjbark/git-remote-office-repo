Ext.define('module.custom.inkopack.sale.order.saleorder.view.SaleOrderFile', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-inkopack-saleorder-file'			,
	store		: 'module.custom.inkopack.sale.order.saleorder.store.SaleOrderFile',
			...
			item = {
				...
				fbar	: [
					{	fieldLabel	: Language.get('file_name','파일첨부'),		// temp
						...
						popup: {
							select : 'SINGLE',
							widget : 'lookup-board-upload2',
							params : { stor_grp : _global.stor_grp , line_stat : '0' ,invc_numb : '' , orgn_dvcd : '', line_seqn : 0 },
							result : function(records, nameField, pairField) {
								var store = Ext.ComponentQuery.query('module-inkopack-saleorder-file')[0].getStore();
								store.reload();
							}
						},listeners:{
							render:function(){
								this.popup.params.orgn_dvcd = 'saleorder';	// table name이 필요하다
							}
						},
					},{	xtype:'button',
						text :'삭제',
