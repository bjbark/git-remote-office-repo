Ext.define('module.custom.inkopack.prod.workentry.view.WorkEntryListerDetail4', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-workentry-detail4'			,
	...
	initComponent: function () {
		var me = this;
		me.dockedItems = [{xtype: 'module-workentry-detailSearch2'}];
		...
		var me = this,
			item = {
				...
				items : [
					{	dataIndex: 'prog_stat_dvcd'	, text : Language.get(''			,'상태'		) , width	: 70	, xtype : 'lookupcolumn', lookupValue : resource.lookup('prog_stat_dvcd'), align : 'center'
					},{ dataIndex: 'wkct_name'		, text : Language.get(''			,'공정'		) , width	: 125	, align : 'center'
					},{ dataIndex: 'cvic_name'		, text : Language.get(''			,'설비'		) , width	: 125	, align : 'center'
					},{ dataIndex: 'acpt_numb'		, text : Language.get(''			,'수주번호'	) , width	: 165	, align : 'center', hidden : true
					},{ dataIndex: 'wkod_numb'		, text : Language.get(''			,'지시번호'	) , flex	: 2		, align : 'center',hidden:true
					},{ dataIndex: 'wkod_seqn'		, text : Language.get(''			,'지시순번'	) , flex	: 2		, align : 'center',hidden:true
					},{ dataIndex: 'plan_strt_dttm'	, text : Language.get(''			,'시작예정'	) , width	: 100	, align : 'center',
					},{ dataIndex: 'work_strt_dttm'	, text : Language.get(''			,'시작시간'	) , width	: 100	, align : 'center',
					},{ dataIndex: 'work_endd_dttm'	, text : Language.get(''			,'종료시간'	) , width	: 100	, align : 'center'
					},{ dataIndex: 'wkod_numb'		, text : Language.get(''			,'지시번호'	) , width	: 220	, hidden : true
					...
					},{ dataIndex: 'total_qntt'		, text : Language.get(''			,'누적수량'	) , width	: 85	, xtype:'numericcolumn'
					},{ dataIndex: 'wker_name'		, text : Language.get(''			,'작업자'	) , width	: 115	, align  : 'center', hidden:true
						...
						renderer: function(val,meta,rec) {
							var id     = Ext.id(),
								search = Ext.ComponentQuery.query('module-workentry-detailSearch')[0]
	...
	insert : function (rec) {
		var search = Ext.ComponentQuery.query('module-workentry-detailSearch')[0],
			store = this.getStore(),
