Ext.define('module.custom.iypkg.sale.order.saleorder.view.SaleOrderEditorIypkg', { extend: 'Axt.form.Editor',

	alias: 'widget.module-saleorder-editor-iypkg',
	...
	createDock : function () {
		var me = this,
			item = {
				...
				items : [
					'->', '-',
					{ text: Const.UPDATE.text , iconCls: Const.UPDATE.icon , action : Const.UPDATE.action , cls: 'button-style' , itemId:'updateSale'},
					{ text: Const.DELETE.text , iconCls: Const.DELETE.icon , action : Const.DELETE.action , cls: 'button-style' , itemId:'deleteSale'}, '-'
				]
			}
		;
		return item;
	...
	createwest : function () {
		var me = this,
			item = {
				...
				flex	: 4,
				plain	: true,
				items	: [ me.createTab5(), me.createTab4(),me.createTab2()]
			}
		return item;
	},
	...
	createTab1 : function() {
		var me = this,
			item = {
				...
				flex		: 3,
				border		: 0	,
				bodyStyle	: { padding: '5px', margin : '0 0 5 0'},
				items		: [
					{	xtype	: 'fieldset', layout: 'vbox' ,padding:'0', border: 0, margin : '0 0 0 0',
						items		: [
							{	xtype	: 'fieldset', layout: 'hbox' ,padding:'0', border: 0, margin : '30 0 4 0',
								items	: [
...
//										readOnly	: true,
										...
										listeners	: {
											keydown : function(self, e) {
												if (e.keyCode == e.ENTER || e.keyCode == e.TAB) {
													var selectSaleButton = self.up('form').down('[itemId=selectSale]');
													// selectSale 버튼 클릭 이벤트 트리거
													selectSaleButton.fireEvent('click', selectSaleButton, e);
							...
							},{	xtype	: 'fieldset', layout: 'hbox' ,padding:'0', border: 0, margin : '15 0 4 0',
								items	: [
										...
										listeners	:{
											keydown : function(self, e) {
												...
												if(e.keyCode==e.ENTER){
													var trigger1 = Ext.dom.Query.select('.drtrTrigger0002')[0];
								...
								items	: [
									{	fieldLabel	: Language.get('cstm_name','거래처'),
										name		: 'cstm_idcd', xtype : 'textfield',
										width		: 150,
										readOnly	: true,
										...
										listeners	:{
											keydown : function(self, e) {
												...
												if(e.keyCode==e.ENTER){
													var trigger1 = Ext.dom.Query.select('.cstmTrigger0002')[0];
													Ext.get(trigger1).dom.click();
												}
												this.keyMap = Ext.create('Ext.util.KeyMap', this.el, {
													scope: this,
...
//												console.log(records[0].get('prod_widh'));
//												console.log(records[0].get('sets_qntt'));
//												me.down('[name=stok_qntt]').popup.params.prod_idcd = this.getValue();
															...
															wkctCopy.load({
																...
																callback: function () {
																	...
																	wkctCopy.each(function(r){
																		...
																		if(_global.hq_id.toUpperCase()=='N1000IYPKG' && (r.data.wkct_idcd == '150' ||
																				r.data.wkct_idcd == '510' || r.data.wkct_idcd == '370' )){
																			r.data.rep_chek = true;
																		}
																		wkct.add(r.copy());
																		fabclister.getSelectionModel().select(0);

																	});
//																	if(_global.hqof_idcd.toUpperCase() == "N1000IYPKG" || me.down('[name=scre_dvcd]').getValue() == '2'){
...
//																	}
										...
										listeners	:{
											keydown : function(self, e) {
												...
												if(e.keyCode==e.ENTER){
													var trigger1 = Ext.dom.Query.select('.itemTrigger0002')[0];
													Ext.get(trigger1).dom.click();
												}
												this.keyMap = Ext.create('Ext.util.KeyMap', this.el, {
													scope: this,
									...
									{	fieldLabel	: Language.get('bxty_name','상자형식'),
										...
										listeners	:{
											keydown : function(self, e) {
												...
												if(e.keyCode==e.ENTER){
													var trigger1 = Ext.dom.Query.select('.bxtyTrigger0002')[0];
													Ext.get(trigger1).dom.click();
												}
											}
										}
...
//										minValue	: 1,
										...
										clearable	: true,
										value		: '0',
										trigger1Cls : 'stok_qnttTrigger0002 ',
										enableKeyEvents : true,
										listeners	: {
											keydown : function(el,e){
												if(e.keyCode == e.ENTER){
													if(_global.hqof_idcd.toUpperCase() == "N1000DAE-A"){
														...
														var trigger1 = Ext.dom.Query.select('.stok_qnttTrigger0002')[0];
														Ext.get(trigger1).dom.click();
													}
												}
											},
													...
													if(val){
														if (/^\d{8}$/.test(val)) {
															...
															var year = val.substr(0, 4),
																month = val.substr(4, 2) - 1, // JavaScript의 Month는 0부터 시작
																day = val.substr(6, 2);
															...
															if (!isNaN(parsedDate.getTime())) {
																// 포맷에 맞게 값 설정
																var formattedDate = Ext.Date.format(parsedDate, Const.DATE_FORMAT_YMD_BAR);
																self.setValue(formattedDate); // 날짜 필드에 포맷된 날짜 설정
															} else {
																Ext.Msg.alert('Error', 'Invalid date entered.');
																}
															}
													}
									...
									{	fieldLabel	: Language.get('assi_cstm_idcd','납품처'),
										...
										listeners	:{
											keydown : function(self, e) {
												...
												if(e.keyCode==e.ENTER){
													var trigger1 = Ext.dom.Query.select('.assiTrigger0002')[0];
													Ext.get(trigger1).dom.click();
												}
												this.keyMap = Ext.create('Ext.util.KeyMap', this.el, {
													scope: this,
...
//											widget	: wmld_widget,
	...
	createTab5 : function() {
		...
		var item =
		{
			title	: Language.get( '' , '주문 현황'),
			xtype	: 'module-saleorder-lister-iypkg',
		};
		return item;
	},
	...
	selectAction: function(callbackFn) {
		var me = this,
			lister = Ext.ComponentQuery.query('module-saleorder-lister-iypkg')[0]
			;
		var today = Ext.Date.format(new Date(),'Ymd');

		var oneWeekAgo = Ext.Date.format(Ext.Date.add(new Date(), Ext.Date.DAY, -7), 'Ymd');

		var invc1_date = oneWeekAgo;
		var invc2_date = today;
		...
		Ext.merge({invc1_date: invc1_date, invc2_date: invc2_date}, { stor_grp: _global.stor_grp }));
	},
});
