Ext.define('module.custom.iypkg.sale.order.estimast2.view.EstiMast2WorkerSearch', { extend: 'Axt.form.Search',
	store	: 'module.custom.iypkg.sale.order.estimast2.store.EstiMast2Invoice',
			...
			line = {
				...
				items  : [
					{	xtype	: 'fieldcontainer',
						...
						width	: 150 ,
						items	: [
							{	text		: '원단', xtype : 'label', style : 'text-align:center;'
							},{	xtype		: 'textfield',
								name		: 'mtrl_name',
								width		: 250,
								margin		: '2 2 2 2',
...
//								}
	...
	attachItem : function (config) {
		...
		if (config.clear || !config.record) {
			...
			me.down('[name=cstm_name2]'	).setValue(record.get('cstm_name2' ));
			me.down('[name=cstm_idcd2]'	).setValue(record.get('cstm_idcd2' ));
			if (config.append) {
	...
	appendItem : function (config) {
		...
		store.each(function(findrecord) {
			if (   findrecord.get('item_idcd') == record.get('item_idcd')
				...
				&& findrecord.get('mxm2_qntt') == record.get('mxm2_qntt')
				...
				&& findrecord.get('bxty_name') == record.get('bxty_name')
				&& findrecord.get('bxty_idcd') == record.get('bxty_idcd')){
				is_equal = true;
				// 상품의 수량을 추가
