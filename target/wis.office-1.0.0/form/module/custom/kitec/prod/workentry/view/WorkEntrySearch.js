Ext.define('module.custom.kitec.prod.workentry.view.WorkEntrySearch', { extend: 'Axt.form.Search',

	alias: 'widget.module-kitec-workenty-search',
					...
					{	xtype: 'fieldset',
						...
						listeners : {
							render : function(feild) {
								var lister		= Ext.ComponentQuery.query('module-kitec-workenty-lister')[0],
									detail		= Ext.ComponentQuery.query('module-kitec-workenty-detail')[0],
									detail2		= Ext.ComponentQuery.query('module-kitec-workenty-detail2')[0],
							...
							change:function(feild,value,a){
								var lister		= Ext.ComponentQuery.query('module-kitec-workenty-lister')[0],
									...
									work_date	= Ext.util.Format.date(me.down('[name=work_date]').getValue(), "Ymd"),
									mask		= new Ext.LoadMask(Ext.getBody(), {msg: Const.SELECT.mask }),
									layout		= Ext.ComponentQuery.query('module-kitec-workenty-layout')[0],
									mask		= new Ext.LoadMask(Ext.getBody(), {msg: Const.SELECT.mask }),
...
//						value		: Ext.util.Cookies.get('cvic_idcd'),
						...
						text		: '<span class="btnTemp" style="font-size:2.5em;">'+Language.get('tag_label', '층별관리현품표')+'</span>',
						name		: 'printBtn',
						margin		: '0 0 0 10',
						cls			: 'btn btn-primary',
						margin		: '0 0 0 30',
						...
						hidden		: true,
						handler		: function(){
							var detail2 = Ext.ComponentQuery.query('module-kitec-workenty-detail2')[0],
								record  = detail2.getSelectionModel().getSelection()[0]
							;
							if(record == null){
								Ext.Msg.alert("알림","작업을 선택해주세요.");
							}else{
								me.printPopup(record);
							}
						},
					},{	buttonAlign	: 'right',
						xtype		: 'button',
