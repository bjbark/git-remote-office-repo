Ext.define('module.custom.aone.sale.esti.estimast.view.EstiMastWorkerSearch', { extend: 'Axt.form.Search',
	store	: 'module.custom.aone.sale.esti.estimast.store.EstiMastInvoice',
				...
				{	xtype	: 'fieldcontainer',
						...
						items	: [
							{	text		: '품목코드' , xtype : 'label', style : 'text-align:center;'
								...
								popup		: {
									select	: 'MULTI',
									widget	: 'lookup-item-popup-aone2',
									values	: { },
									...
									result	: function(records, nameField, pairField) {
										...
										var panel = nameField.up('form'),
											searchForm = Ext.ComponentQuery.query('module-estimast-worker-search')[0];
...
//										panel.down('[name=ttsm_amnt]').setValue(inv_tot);
								...
								listeners	: {
									keydown : function(self, e) {
										if (e.keyCode == e.ENTER) {
											var qty = self.up('form').down('[name=esti_pric]').getValue();
											if(qty == 0 ){
												self.up('form').down('[name=sale_qntt]').focus(true, 10);
											}
											if ( Ext.isEmpty( self.up('form').down('[name=deli_date2]').getValue()) ){
												self.up('form').down('[name=deli_date2]').focus(true, 10);
											}
											var panel = self.up('form');
											panel.down('[name=poor_cont]').focus(true, 10);
										}else if (e.keyCode == e.ESC) {
											me.attachItem({ clear : true });
										}
									}
		...
		line = {
				...
				items  : [
					{	xtype	: 'fieldcontainer',
						...
						width	: 455,
						items	: [
							{	text		: '의뢰 내용' , xtype : 'label', style : 'text-align : center;'
							},{	xtype		: 'textfield',
								name		: 'poor_cont',
								margin		: '2 2 2 2',
								enableKeyEvents : true,
						...
						width	: 455,
						items	: [
							{	text		: '전달 사항' , xtype : 'label', style : 'text-align : center;'
							},{	xtype		: 'textfield',
								name		: 'remk_text',
								margin		: '2 2 2 2',
								enableKeyEvents : true,
								listeners	: {
									keydown : function(self, e) {
										if (e.keyCode == e.ENTER) {
											var panel = self.up('form')
											self.up('form').down('[name=item_idcd]').getValue().trim() != ''
											me.appendItem({});
										} else if (e.keyCode == e.ESC) {
											me.attachItem({ clear : true });
										}
	...
	attachItem : function (config) {
		...
		if (config.clear || !config.record) {
			...
			var deli = Ext.Date.format(Ext.ComponentQuery.query('module-estimast-worker-editor')[0].down('[name=deli_date]').getValue(),'Y-m-d');
...
//			me.down('[name=ttsm_amnt]'	).setValue(record.get('ttsm_amnt' ));
			...
			me.down('[name=poor_cont]'	).setValue(record.get('poor_cont'));
...
//				me.down('#itemviewer').update( html );
	...
	appendItem : function (config) {
		...
		var amnd_degr = Ext.ComponentQuery.query('module-estimast-worker-editor')[0].down('[name=amnd_degr]').getValue();
...
//			ttsm_amnt		: me.down('[name=ttsm_amnt]').getValue(),
			...
			poor_cont		: me.down('[name=poor_cont]').getValue(),
