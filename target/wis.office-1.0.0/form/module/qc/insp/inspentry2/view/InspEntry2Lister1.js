Ext.define('module.qc.insp.inspentry2.view.InspEntry2Lister1', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-inspentry2-lister1',
	selModel	: { selType: 'checkboxmodel', mode : 'MULTI' },
	store		: 'module.qc.insp.inspentry2.store.InspEntry2Lister1',
	border		: 0,
	columnLines : true,
	...
	pagingItem : function () {
		var me = this,
			item = {
				...
				items	: [
					'->', '-' ,
					{	text : "<span>공정별검사</span>", action : 'typeItem' , iconCls: Const.INSERT.icon } ,
//					{	text : "<span>검사실적등록</span>", handler: me.insert ,iconCls: Const.INSERT.icon,itemId:'insert' } ,
	...
	columnItem : function () {
		var me = this,
			item = {
				...
				items : [
					{	dataIndex: 'insp_date'		, text : Language.get('insp_date'		,'검사일시'	)	, width : 80  , align : 'center'
					},{ dataIndex: 'insp_strt_time'	, text : Language.get('insp_strt_time'	,'검사시간'	)	, width : 60 , align : 'center'
					},{ dataIndex: 'wkct_code'		, text : Language.get('wkct_code'		,'공정코드'	)	, width : 120 , align : 'center', hidden : true
					},{ dataIndex: 'invc_numb'		, text : Language.get('invc_numb'		,'invc_numb'	)	, width : 120 , align : 'center', hidden : true
					...
					},{ dataIndex: 'wkct_insp_dvcd'	, text : Language.get('wkct_insp_dvcd'	,'검사구분'	)	, width : 120 , xtype : 'lookupcolumn', lookupValue : resource.lookup('wkct_insp_dvcd'), align : 'center'
					},{ dataIndex: 'insp_qntt'		, text : Language.get('insp_qntt'		,'검사수량'	)	, width : 80  , xtype : 'numericcolumn', summaryType: 'sum',hidden:true
					},{ dataIndex: 'good_qntt'		, text : Language.get('good_qntt'		,'양품수량'	)	, width : 80  , xtype : 'numericcolumn', summaryType: 'sum',hidden:true
					},{ dataIndex: 'msmt_valu_1fst'	, text : Language.get('msmt_valu_1fst'	,'x1'		)	, width : 80
					},{ dataIndex: 'msmt_valu_2snd'	, text : Language.get('msmt_valu_2snd'	,'x2'		)	, width : 80
					},{ dataIndex: 'msmt_valu_3trd'	, text : Language.get('msmt_valu_3trd'	,'x3'		)	, width : 80
					},{ dataIndex: 'msmt_valu_4frt'	, text : Language.get('msmt_valu_4frt'	,'x4'		)	, width : 80
					},{ dataIndex: 'msmt_valu_5fit'	, text : Language.get('msmt_valu_5fit'	,'x5'		)	, width : 80
	...
	insert : function () {
		...
		if(!selectMaster){
			...
			if(line_seqn){
				Ext.Ajax.request({
					url		: _global.location.http() + '/qc/insp/inspentry2/get/insp_cond.do',
					params	: {
						token : _global.token_id,
						param : JSON.stringify({
							stor_id		: _global.stor_id,
