Ext.define('module.mtrl.po.purctrstwork.view.PurcTrstWorkInsertPopup', { extend: 'Axt.popup.Search',
	alias: 'widget.module-purctrstwork-insert-popup',
	store	: 'module.mtrl.po.purctrstwork.store.PurcTrstWorkInsertPopup',
	...
	createGrid: function(){
			...
			grid = {
				...
				columns: [
					...
					},{	dataIndex:	'cont_pric'	, align : 'right'  , text: Language.get( 'cont_pric'      , '계약단가'	),
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					},{	dataIndex:	'pack_qntt'	, width:  80, align : 'left'   , text: Language.get( 'pack_qntt'      , '패킹단위'	),
						renderer: function(value, meta){
							if(me.popup.params.tema){
								this.columns[9].width = 120;
								meta.style = 'font-size:2em !important;height:30px;line-height:30px;width:120px;'; // applied style for DIV element
			...
			grid = {
				...
				columns: [
					{	dataIndex:	'line_stat'	, width:  50, align : 'center' , text: Language.get( 'line_stat'      , '상태'	) , xtype: 'lookupcolumn' , lookupValue: resource.lookup( 'line_stat' ),
						...
						hidden : _global.hq_id.toUpperCase() == 'N1000NBOLT'? false : true
					},{	dataIndex:	'pack_qntt'	, width: 110, align : 'right'   , text: Language.get( 'pack_qntt'      , '패킹단위'),
						renderer: function(value, meta){
							if(me.popup.params.tema){
								this.columns[5].width = 260;
								meta.style = 'font-size:2em !important;height:30px;line-height:30px'; // applied style for DIV element
