Ext.define('module.workshop.sale.order.ordermast.view.OrderMastWorkerLister', { extend: 'Axt.grid.Panel',
	alias: 'widget.module-ordermast-worker-lister',
	store: 'module.workshop.sale.order.ordermast.store.OrderMastInvoice',
...
//							listeners	: {
////								blur	: function(field){
////									var lister = Ext.ComponentQuery.query('module-ordermast-worker-lister')[0],
...
////											Ext.Ajax.request({
////												url			: _global.api_host_info + '/' + _global.app_site + '/custom/hjsys/sale/order/ordermast/get/chkOrdr.do',
////												method		: "POST",
////												params		: {
...
//	},
	...
	calculate : function(){
		...
		if(check==0){
			...
			Ext.Ajax.request({
				url			: _global.api_host_info + '/' + _global.app_site + '/custom/hjsys/sale/order/ordermast/get/cal.do',
				method		: "POST",
				params		: {
					token	: _global.token_id,
					param	: Ext.encode({
				...
				success : function(response, request) {
					...
					if	(!result.success ){
						...
						if(result.records.length >0){
							...
							var acpt_qntt = Ext.ComponentQuery.query('module-ordermast-worker-editor')[0].down('[name=acpt_qntt]').getValue();
...
//		}
	...
	listeners: {
		...
		render: function(){
			var me = this;
			var editor = Ext.ComponentQuery.query('module-ordermast-worker-editor')[0];
			new Ext.util.KeyMap({
				target: me.getEl().dom,
				binding: [
					/* Ctrl + Delete */
...
//		도면등록 체크
					...
					if (button==='yes') {
						if(records[0].data.pdsd_yorn == '1'){
							...
							if(records[0].data.drwg_chk == 'Y' || records[0].data.check == 'Y'){
								Ext.Ajax.request({
									url			: _global.api_host_info + '/' + _global.app_site + '/custom/hjsys/sale/order/ordermast/set/delete.do',
									method		: "POST",
									params		: {
									 	token	: _global.token_id,
										param	: Ext.encode({
	...
	plan : function (rec){
		var me		= this,
			...
			editor1 = Ext.ComponentQuery.query('module-ordermast-worker-editor')[0],
			lister = Ext.ComponentQuery.query('module-ordermast-worker-lister')[0],
			item1  = Ext.ComponentQuery.query('module-ordermast-lister-item1')[0]
			item2  = Ext.ComponentQuery.query('module-ordermast-lister-item2')[0]
...
//		}
	...
	suIt : function (rec){
		...
		editor1 = Ext.ComponentQuery.query('module-ordermast-worker-editor')[0],
		lister = Ext.ComponentQuery.query('module-ordermast-worker-lister')[0],
		item1  = Ext.ComponentQuery.query('module-ordermast-lister-subItem')[0],
		layout = Ext.ComponentQuery.query('module-ordermast-layout')[0],
		check = 1, chk = 0
		;
		item1.getStore().loadData([],true);
