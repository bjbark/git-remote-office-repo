Ext.define('module.workshop.sale.order.ordermast.view.OrderMastTree', { extend:  'Axt.grid.Panel',
	alias		: 'widget.module-ordermast-tree',
	store		: 'module.workshop.sale.order.ordermast.store.OrderMastTree',
...
//					}
					...
					},{ dataIndex: 'esti_pric'		, text : '견적단가'		, width :  90,xtype : 'numericcolumn', align : 'right'
								...
								if(rec.data.shet_count >= 1){
									Ext.defer(function() {
										Ext.widget('button', {
												...
												text	: '<span style="color : white !important">표지조회</span>',
												width	: 60,
												height	: 19,
							...
							if(rec.data.acct_bacd != '3000'){
								if(rec.data.shet_count >= 1){
									Ext.defer(function() {
										Ext.widget('button', {
											renderTo: Ext.query("#"+id)[0],
											text	: '<span style="color : white !important;">내지조회</span>',
											width	: 60,
											height	: 19,
											cls		: 'button-style',
											style	: 'background : #419641',
							...
							if(rec.data.acct_bacd != '3000'){
								if(rec.data.indx_count > 0){
									Ext.defer(function() {
										Ext.widget('button', {
											renderTo: Ext.query("#"+id)[0],
											text	: '<span style="color : white !important;">간지조회</span>',
											width	: 63,
											height	: 19,
											style	: 'background : #419641',
											cls		: 'button-style',
							...
							if(rec.data.acct_bacd != '3000'){
								if(rec.data.proc_count > 0){
									Ext.defer(function() {
										Ext.widget('button', {
											renderTo: Ext.query("#"+id)[0],
											text	: '<span style="color : white !important">후가공조회</span>',
											width	: 63,
											height	: 19,
											cls		: 'button-style',
											style	: 'background : #419641',
			...
			Ext.Msg.show({ title: '확인', msg: '삭제하시겠습니까?', icon: Ext.Msg.QUESTION, buttons: Ext.Msg.YESNO, defaultFocus: 'no', scope: me,
				fn: function (button) {
					if (button=='yes') {
						var a = [];
						Ext.Ajax.request({
							url		: _global.location.http() + '/workshop/sale/order/ordermast/set/delete.do',
							params	: {
...
//		}
		Ext.Ajax.request({
			url		: _global. location.http () + '/workshop/sale/order/ordermast/get/maxid.do',
			method		: "POST",
			params	: {
				token : _global. token_id ,
				param : JSON. stringify({
				...
				buttons: [
					{	text : Const.FINISH.text,
						...
						handler: function(rec) {
							...
							Ext.Ajax.request({
								url		: _global.location.http() + '/workshop/sale/order/ordermast/set/records2.do',
								params	: {
									token : _global.token_id,
									param : JSON.stringify({
...
//		}
	...
	draw : function(rec){
		...
		if(select){
			Ext.Ajax.request({
				url		: _global.location.http() + '/workshop/sale/order/ordermast/get/shet.do',
				params	: {
					token : _global.token_id,
					param : JSON.stringify({
	...
	shetItem:function(rec){
		var	me = this,
			...
			editor = Ext.ComponentQuery.query('module-ordermast-worker-editor2')[0],
			master = Ext.ComponentQuery.query('module-ordermast-lister-master')[0],
			select = master.getSelectionModel().getSelection()[0],
			lister = Ext.ComponentQuery.query('module-ordermast-lister-shetItem')[0]
	...
	mainItem:function(rec){
		var	me = this,
			layout = Ext.ComponentQuery.query('module-ordermast-layout')[0],
			editor = Ext.ComponentQuery.query('module-ordermast-worker-editor3')[0],
			master = Ext.ComponentQuery.query('module-ordermast-lister-master')[0],
			select = master.getSelectionModel().getSelection()[0],
			lister = Ext.ComponentQuery.query('module-ordermast-lister-mainItem')[0]
	...
	subItem:function(rec){
		var	me = this,
			layout = layout = Ext.ComponentQuery.query('module-ordermast-layout')[0],
			editor = Ext.ComponentQuery.query('module-ordermast-worker-editor4')[0],
			master = Ext.ComponentQuery.query('module-ordermast-lister-master')[0],
			select = master.getSelectionModel().getSelection()[0],
			lister = Ext.ComponentQuery.query('module-ordermast-lister-subItem')[0]
		;
		editor.selectRecord({ record : select }, me);
		lister.select({
			callback:function(records, operation, success) {
