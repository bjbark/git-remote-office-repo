Ext.define('module.workshop.sale.order.estimast.view.EstiMastMainItem', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-estimast-lister-mainItem',
	store		: 'module.workshop.sale.order.estimast.store.EstiMastMainItem',
	...
	columnItem : function () {
		var me = this,
			item = {
				...
				items		: [
							{	dataIndex:	'shet_idcd'			, width:  60, align : 'center'	, text: Language.get( ''	, ''	),hidden : true,
							},{	dataIndex:	'line_seqn'			, width: 200, align : 'left'	, text: Language.get( ''	, ''	),hidden : true,
							},{	dataIndex:	'indx_assi_seqn'	, width: 200, align : 'left'	, text: Language.get( ''	, ''	),hidden : true,
...
//													record.set('esti_pric',parent.data.esti_pric);
							...
							},{	dataIndex:	'shet_seqn'			, width: 80, align : 'left'		, text: Language.get( 'shet_seqn'		, '용지순번'	),hidden: true
								...
								editor	: {
									...
									listeners	: {
										...
										var grid = self.up('grid');
											val = this.getValue();
											myform	= Ext.ComponentQuery.query('module-estimast-lister-mainItem')[0];
	...
	rowInsert: function(){
		var me			= this,
			...
			master		= Ext.ComponentQuery.query('module-estimast-lister')[0],
			record		= undefined,
			max_seq		= 0,
//			select		= myform.getSelectionModel().getSelection()[0],
			lastidx		= store.count(),
		...
		// ROW 추가
		store.add(record);
		Ext.ComponentQuery.query('module-estimast-worker-editor3')[0].down('[name=modify]').setValue('Y');
