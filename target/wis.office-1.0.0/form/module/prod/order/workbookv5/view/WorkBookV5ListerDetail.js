Ext.define('module.prod.order.workbookv5.view.WorkBookV5ListerDetail', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-workbookv5-detail'			,
	selModel	: { selType: 'checkboxmodel', mode : 'MULTI' },
	store		: 'module.prod.order.workbookv5.store.WorkBookV5Detail',
	border		: 0,
			...
			item = {
				...
				items	: [
					{	text : '<span class="btnTemp" style="font-size:2.5em;">교대/마감</span>', cls : 'button-left btn btn-success' , width: 150, height : 50, margin: '0 0 0 5' , handler:me.shiftWork
						...
						margin : '0 0 0 5',
						handler:  function() {
							var detail    = Ext.ComponentQuery.query('module-workbookv5-detail')[0],
								master    = Ext.ComponentQuery.query('module-workbookv5-lister')[0],
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{ dataIndex: 'dayn_dvcd'		, text : Language.get('dayn_dvcd'		,'주/야'		) , width : 90	, xtype  : 'lookupcolumn'	, lookupValue : resource.lookup('dayn_dvcd'), align : 'center'
...
//									this.up('window').hide();
	...
	//TODO 종료
		...
		var search = Ext.ComponentQuery.query('module-workbookv5-search')[0],
			searchDate = search.down('[name=work_date]').getValue(),
			wkct_idcd = search.down('[name=wkct_name]').getValue(),
			store = Ext.ComponentQuery.query('module-workbookv5-detail')[0].getStore()
			store2 = Ext.ComponentQuery.query('module-workbookv5-detail2')[0].getStore()
		;
		var	form = Ext.widget('form', {
			border: false,
			bodyPadding: 10,
...
//										this.up('window').hide();
	...
	deleted : function () {
		var search = Ext.ComponentQuery.query('module-workbookv5-search')[0],
			searchDate = search.down('[name=work_date]').getValue(),
			wkct_idcd = search.down('[name=wkct_name]').getValue(),
			store = Ext.ComponentQuery.query('module-workbookv5-detail2')[0].getStore(),
			store2 = Ext.ComponentQuery.query('module-workbookv5-detail')[0].getStore(),
			store3 = Ext.ComponentQuery.query('module-workbookv5-lister')[0].getStore(),
...
//						Ext.ComponentQuery.query('#fail')[0].up('window').destroy();
	...
	poorupdate : function (param) {
		var me = this,
			...
			detail    = Ext.ComponentQuery.query('module-workbookv5-detail')[0],
			poor      = Ext.ComponentQuery.query('module-workbookv5-poor')[0],
			select    = detail.getSelectionModel().getSelection()[0],
			line_seqn = 0,
...
//TODO 제품정보
		...
		var form = Ext.widget('form', {
			...
			items:[
				...
				//							labelWidth	: 10,
								...
								},{	xtype	: 'fieldset', layout: 'hbox' ,padding:'0', border: 0,margin : '5 0 0 20', hidden:true,
									items	: [
									...
									items	: [
										{	fieldLabel	: Language.get('istt_wrhs_name','입고창고'),
											...
											readOnly	: true,
											width		: 290,
											value		: select.get('istt_wrhs_name'),
											height		: 40,
											labelStyle	: 'line-height: 35px;',
											labelCls	: 'textTemp '+_global.options.work_book_tema+'iteminfo',
											fieldCls	: 'textTemp '+_global.options.work_book_tema+'iteminfoPopupfield',
				...
				},{	xtype : 'module-workbookv5-poor', region:'west' , flex : 1, height:200 ,margin: '0 0 0 17'
				},{	xtype : 'module-workbookv5-fail',region:'center', flex : 1, height:200 ,margin: '0 17 0 0'
				}
			],
			buttons: [
	...
	shiftWork : function(){
		var me			= this,
			search		= Ext.ComponentQuery.query('module-workbookv5-search')[0],
			searchDate	= search.down('[name=work_date]').getValue(),
			wkct_idcd	= search.down('[name=wkct_name]').getValue(),
			lister		= Ext.ComponentQuery.query('module-workbookv5-lister')[0],
			detail		= Ext.ComponentQuery.query('module-workbookv5-detail')[0],
