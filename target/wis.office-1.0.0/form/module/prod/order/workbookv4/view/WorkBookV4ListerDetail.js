Ext.define('module.prod.order.workbookv4.view.WorkBookV4ListerDetail', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-workbookv4-detail'			,
	selModel	: { selType: 'checkboxmodel', mode : 'SINGLE' },
	store		: 'module.prod.order.workbookv4.store.WorkBookV4Detail',
	border		: 0,
		...
		var me = this,
			item = {
				...
				items	: [
					...
					'->','-',
					{	text : '<span class="btnTemp" style="font-size:2.5em;">생산정보입력</span>'  ,
						cls  : 'button-left btn btn-primary' ,
						width: 210,
						height : 50,
						margin : '0 0 0 5',
						handler:  function() {
							var detail    = Ext.ComponentQuery.query('module-workbookv4-detail')[0],
								master    = Ext.ComponentQuery.query('module-workbookv4-lister')[0],
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{ dataIndex: 'sysm_memo'		, text : Language.get(''				,'순번'		) , width : 50	, align  :'center' /*두인 요청사항으로 line_seqn대신 메모에 사출대기시 표시되는 순번 저장해서 불러오게 처리함*/
					...
					},{ dataIndex: 'wker_name'		, text : Language.get(''				,'작업관리자'	) , width : 100	, align  : 'center'
					...
					},{ dataIndex: 'work_cycl_time'	, text : Language.get(''				,'생산C/T'	) , flex  : 2	, hidden : true
...
//									});
	...
	stop : function (rec) {
		var search = Ext.ComponentQuery.query('module-workbookv4-search')[0],
			...
			prod_qntt = '';
		;
		Ext.Msg.confirm("확인", "생산 정보 입력을 실행하셨습니까?", function(button) {
		if (button == 'yes') {
						...
						{	text: '<span class="btnTemp" style="font-size:3em">확인</span>',
							...
							handler: function() {
								...
								if(param.prod_qntt < 0 ||param.prod_qntt == ''){
									...
									return;
								}else if(rec.get('work_cycl_time') < 1){
									Ext.Msg.alert("알림","생산C/T를 입력해주세요.");
									return;
								}else{
	...
	//TODO 종료
		...
		var search = Ext.ComponentQuery.query('module-workbookv4-search')[0],
			searchDate = search.down('[name=work_date]').getValue(),
			cvic_idcd = search.down('[name=cvic_name]').getValue(),
			store = Ext.ComponentQuery.query('module-workbookv4-detail')[0].getStore(),
			store2 = Ext.ComponentQuery.query('module-workbookv4-detail2')[0].getStore(),
			prod_qntt = '';
		;
							...
							handler: function() {
								...
								if(param.prod_qntt==null||param.prod_qntt==''){
									...
									if(param.prod_qntt < 1 ||param.prod_qntt == ''){
										...
										Ext.Msg.alert("알림","생산C/T를 입력해주세요.");
										return;
									}else if(rec.get('work_cavity')==''){
										Ext.Msg.alert("알림","생산CAVITY를 입력해주세요.");
										return;
									}else{
										record = Ext.create( store.model.modelName , {
											invc_numb		: rec.get('invc_numb'),
	...
	deleted : function () {
		var search = Ext.ComponentQuery.query('module-workbookv4-search')[0],
			searchDate = search.down('[name=work_date]').getValue(),
			cvic_idcd = search.down('[name=cvic_name]').getValue(),
			store = Ext.ComponentQuery.query('module-workbookv4-detail2')[0].getStore(),
			store2 = Ext.ComponentQuery.query('module-workbookv4-detail')[0].getStore(),
			store3 = Ext.ComponentQuery.query('module-workbookv4-lister')[0].getStore(),
...
//						Ext.ComponentQuery.query('#fail')[0].up('window').destroy();
	...
	poorupdate : function (param) {
		var me = this,
			...
			detail    = Ext.ComponentQuery.query('module-workbookv4-detail')[0],
			poor      = Ext.ComponentQuery.query('module-workbookv4-poor')[0],
			select    = detail.getSelectionModel().getSelection()[0],
			line_seqn = 0,
		...
		if(poor_qntt==0||poor_qntt==''||poor_qntt==null){
			...
			//line_seqn count
			Ext.Ajax.request({
				url		: _global.location.http() + '/prod/order/workbookv4/get/poorseqn.do',
				params	: {
					token : _global.token_id,
			...
			Ext.Ajax.request({
				url		: _global.location.http() + '/prod/order/workbookv4/set/poor.do',
				params	: {
					token : _global.token_id,
					param : JSON.stringify({
						_set			: 'insert',
	...
	failupdate : function (param) {
		var me = this,
			...
			edtm      = '',
			detail    = Ext.ComponentQuery.query('module-workbookv4-detail')[0],
			fail      = Ext.ComponentQuery.query('module-workbookv4-fail')[0],
			select    = detail.getSelectionModel().getSelection()[0],
			line_seqn = 0,
			loss_time = 0,
		...
		//line_seqn count
		Ext.Ajax.request({
			url		: _global.location.http() + '/prod/order/workbookv4/get/failseqn.do',
			params	: {
				token : _global.token_id,
				param : JSON.stringify({
					stor_id			: _global.stor_id,
		...
		Ext.Ajax.request({
			url		: _global.location.http() + '/prod/order/workbookv4/set/fail.do',
			params	: {
				token : _global.token_id,
				param : JSON.stringify({
					_set			: 'insert',
...
//TODO 품목정보
	iteminfo : function (select) {
		...
		;
		Ext.Ajax.request({
			url			: _global.location.http() + '/prod/order/workbookv4/get/realshot.do',
			method		: "POST",
			async		: false,
			params		: {
				token	: _global.token_id,
		...
		var form = Ext.widget('form', {
			...
			items:[
								...
								{	xtype	: 'fieldset', layout: 'hbox' ,padding:'0', border: 0,margin : '0 0 0 20',
									...
									listeners:{
										change:function(field,val){
											if(val!=''){
												Ext.Ajax.request({
													url			: _global.location.http() + '/prod/order/workbookv4/set/cvic_drtr.do',
													method		: "POST",
													params		: {
														token	: _global.token_id,
														param	: Ext.encode({
										...
										{	fieldLabel	: Language.get('invc_numb','지시번호'),
											...
											listeners	:{
												change	: function(){
													...
													if(val!=''){
														Ext.Ajax.request({
															url			: _global.api_host_info + '/' + _global.app_site + '/prod/order/workbookv4/set/mold_repa.do',
															method		: "POST",
															params		: {
																token	: _global.token_id,
																param	: Ext.encode({
				...
				//							labelWidth	: 10,
											...
											trigger1Cls : 'hideCls trigger23',
											listeners	:{
												render:function(field ){
													field.getEl().on('click', function( event, el ) {
														var trigger1 = Ext.dom.Query.select('.trigger23')[0];
														Ext.get(trigger1).dom.click();
													});
												}
											}
									...
									items	: [
										{	fieldLabel	: Language.get('item_code','품목코드'),
											...
											fieldCls	: 'textTemp '+_global.hq_id+'iteminfoPopupfield readonlyfield',
											cls			: 'textTemp'
										},{	fieldLabel	: Language.get('work_cycl_time','생산CAVITY'),
											name		: 'work_cavity',
											xtype		: 'popupfield',
											editable	: true,
											enableKeyEvents : true,
											...
											listeners	:{
												...
												change:function(field,val){
													if(val!=''){
														Ext.Ajax.request({
															url			: _global.location.http() + '/prod/order/workbookv4/set/work_cavity.do',
															method		: "POST",
															params		: {
																token	: _global.token_id,
																param	: Ext.encode({
											...
											listeners	:{
												...
												change	: function(){
													...
													if(select.get('work_cycl_time') != val){
														Ext.Ajax.request({
															url			: _global.location.http() + '/prod/order/workbookv4/set/cyclTime.do',
															method		: "POST",
															params		: {
																token	: _global.token_id,
																param	: Ext.encode({
				...
				},{	xtype : 'module-workbookv4-poor', region:'west' , flex : 1, height:200 ,margin: '0 0 0 17'
				},{	xtype : 'module-workbookv4-fail',region:'center', flex : 1, height:200 ,margin: '0 17 0 0'
				}
			],
			buttons: [
	...
	shiftWork : function(){
		var me			= this,
			search		= Ext.ComponentQuery.query('module-workbookv4-search')[0],
			searchDate	= search.down('[name=work_date]').getValue(),
			cvic_idcd	= search.down('[name=cvic_name]').getValue(),
			lister		= Ext.ComponentQuery.query('module-workbookv4-lister')[0],
			detail		= Ext.ComponentQuery.query('module-workbookv4-detail')[0],
								...
								buttons: [
									{	text: '<span class="btnTemp" style="font-size:3em">확인</span>',
										...
										handler: function() {
											...
											else if(select.get('work_cycl_time') < 1){
												Ext.Msg.alert("알림","생산C/T를 입력해주세요.");
												return;
											}else if(select.get('work_cavity')==''){
												Ext.Msg.alert("알림","생산CAVITY를 입력해주세요.");
												return;
											}else{
