...
//										}
						...
						pair		: 'brnd_bacd',
						clearable	: true ,
						hidden		: _global.hq_id !='N1000hntop',
						popup: {
							select : 'SINGLE',
							widget : 'lookup-base-popup',
					...
					},{	name : 'brnd_bacd', xtype : 'textfield' , hidden : true
						...
						pair		: 'lcls_idcd',
						clearable	: true ,
						hidden		: (_global.options.mes_system_type =='Frame'||_global.hq_id.toUpperCase()=='N1000NBOLT'),
						popup: {
							select : 'SINGLE',
					...
					},{	fieldLabel	: Language.get('colr_bacd_name','컬러'),
						xtype		: 'popupfield',
						editable	: true,
						enableKeyEvents : true,
						name		: 'colr_bacd_name',
						...
						xtype		: 'popupfield',
						hidden		: _global.options.mes_system_type =='Frame'||_global.hq_id=='N1000nbolt',
						editable	: true,
						enableKeyEvents : true,
						name		: 'wkfw_name',
						pair		: 'wkfw_idcd',
						...
						editable	: true,
						enableKeyEvents : true,
//						hidden		: (_global.hq_id != 'N1000kitec'),
						name		: 'rpst_item_name',
						width		: 340,
						pair		: 'rpst_item_idcd',
						clearable	: true,
						popup: {
							...
							params : { stor_grp : _global.stor_grp , line_stat : '0', acct_bacd:'제품' , repr_sent : '대표품목'},
							result : function(records, nameField, pairField) {
...
//															}
										...
										border: 0,
										items : [
											{	fieldLabel	: Language.get('wigt_info','중량'),
												xtype		: 'numericfield',
												name		: 'item_wigt',
												labelWidth	: 70,
												width		: 150
...
//																	Ext.get('prjt_work_imge').dom.src = event.target.result;
	...
	calc:function(){
		var me			= this,
			...
			drwg_numb	= form.down('[name=drwg_numb]'),
...
//						otsd_dimt		: param_otsd,
				...
				success	: function(response, request) {
					...
					if	(!result.success ){
						...
						form.down('[name=a_valu_scpe]').setValue(result.records[0].a_valu_scpe);
						form.down('[name=b_valu_scpe]').setValue(result.records[0].b_valu_scpe);
						form.down('[name=wigt_info]').setValue(result.records[0].wigt_info);
						form.down('[name=mtty_dvcd]').setValue(result.records[0].mtty_dvcd);
