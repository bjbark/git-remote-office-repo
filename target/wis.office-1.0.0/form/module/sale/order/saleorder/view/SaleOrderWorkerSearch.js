Ext.define('module.sale.order.saleorder.view.SaleOrderWorkerSearch', { extend: 'Axt.form.Search',
	store	: 'module.sale.order.saleorder.store.SaleOrderInvoice',
	alias	: 'widget.module-saleorder-worker-search',
...
//									},
									result	: function(records, nameField, pairField) {
										...
										var searchForm = Ext.ComponentQuery.query('module-saleorder-worker-search')[0];
						...
						width	: 120,
						hidden		: _global.options.mes_system_type.toUpperCase() == 'SJFLV',
						style	: Const.borderLine.top +  Const.borderLine.bottom  +  Const.borderLine.left  +  Const.borderLine.right  ,hidden		: _global.options.mes_system_type.toUpperCase() == 'SJFLV',
						margin	: '3 0 3 3',
						items	: [
								...
								listeners: {
									change : function(self, value) {
										...
										resId =  _global.options.mes_system_type?_global.options.mes_system_type.toUpperCase():_global.hq_id.toUpperCase()
												...
												if(resId == 'SJFLV'){
													inv_amt = Math.round((Number(value) * Number(panel.down('[name=invc_pric]').getValue())))
												}else{
								...
								listeners : {
									change : function(self, value) {
										var panel = self.up('form')
											...
											if(resId == 'SJFLV'){
												inv_amt = Math.round((Number(value) * Number(panel.down('[name=invc_qntt]').getValue())))
											}else{
