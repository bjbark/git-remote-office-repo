 ...
 * 거래 명세서 설정 팝업
 */
Ext.define('Axt.popup.view.InvoiceSaleConfig', { extend: 'Ext.window.Window',
    alias: 'widget.popup-invoice-sale-config',
...
//    popup: undefined,
    ...
    version : '3', // 1, 2, 3과같이 설정이 수정될때마다 version을 올려준다.
    ...
    initData : function () {
    	...
    	PrinterPlugin.getPrinterList({
	    	callback:function(success, printerList, msg){
	    		...
	    		var invoiceModel = ResourcePrint.getInvoiceModel(Const.InvoiceType.SALE, me.version);
...
//            id: 'center-region-container',
            ...
            items: [
                {
                	xtype:'image', src:'/resource/sdk/extjs/axtjs/resources/img/invoice_red.png', x:0, y:0, width:'100%', height:490 // 596*398
                },
                
                {  xtype:'label', html:'거래 명세서', x:310, y:10, style:'font-size:30px !important; color:red;'  },
                
                // header(title)부분
                me.createNF('top_x', 125, 30, 6, 'b', 'x좌표 시작'), // x좌표 시작 
                me.createNF('top_y', 125, 30, 30, 'r', 'y좌표 시작'), // y좌표 시작

                me.createNF('cust_id_x',    106, 596, 6,   'b', '거래처CODE'), me.createNF('cust_id_y', 32, 706, 6, 'r') , me.createNF('cust_id_fs', 25, 742, 6, 'g'), // 
                me.createNF('work_no_x',    106, 596, 30,   'b', '전표NO.'), me.createNF('work_no_y', 32, 706, 30, 'r') , me.createNF('work_no_fs', 25, 742, 30, 'g'), // 
                me.createNF('amount1_x',    106, 596, 54,   'b', '합계금액'), me.createNF('amount1_y', 32, 706, 54, 'r') , me.createNF('amount1_fs', 25, 742, 54, 'g'), // 
                
             // 직인 이미지 출력 여부
                {  xtype:'checkboxfield', name:'stamp_yn', x:200, y:31, inputValue:true, uncheckedValue:false, boxLabel  : '직인 출력' },
                me.createNF('inv_dt_x',    70, 185, 55,   'b', '일자', 30 ), me.createNF('inv_dt_y', 30, 260, 55, 'r') , me.createNF('inv_dt_fs', 25, 295, 55, 'g'), // 전화
                ...
                me.createNF('list_item_cd_x', 40, 95, 370, 'b'),
                {  xtype:'label', html:'코드', x:100, y:345, style:'color:#FF9695;'}, //코드
                {	
//            		fieldLabel   : 'Type',
            		xtype        : 'lookupfield',
            		x :95, y:342,
...
//            		selectedIndex : 0,
                ...
                me.createNF('list_item_nm_x', 40, 150, 370, 'b'),                                          // 품목 
                me.createNF('list_y', 40, 195, 370, 'r'),                                                  // 목록의 첫시작 y좌표
                me.createNF('list_fs', 40, 240, 370, 'g'),                                                 // 목록의 폰트 크기
                me.createNF('list_unit_nm_x', 40, 375, 370, 'b'),                                          // 단위
                me.createNF('list_qty_x', 40, 430, 370, 'b'),                                              // 수량 
                me.createNF('list_price_x', 40, 495, 370, 'b'),                                            // 단가 
                me.createNF('list_subtotal_x', 40, 575, 370, 'b'),                                         // 공급가액 
                me.createNF('list_tax_x', 40, 640, 370, 'b'),                                              // 세액
                me.createNF('list_amount_x', 40, 700, 370, 'b'),                            			    // 합계
                me.createNF('list_lh', 130, 5, 405, 'b', '품목 1라인당 폭', 95),                             // 품목 1라인당 폭 
                me.createNF('list_count', 130, 150, 405, 'b', '품목 출력 라인수', 95),                       // 품목 출력 라인수
                
                {  xtype:'label', html:'소비자가', x:325, y:347, style:'color:#FF9695;font-weight:bold;'},
                me.createNF('list_sobi_price_x', 40, 325, 370, 'b'),                                              // 소비자가
                
                
                
                // 합계 금액부분 (합계 현금 수표 어음)
                me.createNF('total_amount_x', 40, 5, 460, 'b'), me.createNF('total_amount_y', 40, 50, 460, 'r'), me.createNF('total_amount_fs', 25, 95, 460, 'g'),
                me.createNF('bottom_recv_y', 200, 10, 490, 'r', '하단의 공급 받는자 y좌표', 115),
                
                {  xtype:'label', html:'&nbsp;전&nbsp;일&nbsp;미&nbsp;수&nbsp;', x:405, y:440, style:'color:#FF9695;background-color:white;font-weight:bold;'}, // 전일미수
                me.createNF('pre_balance_x', 30, 385, 460, 'b'), me.createNF('pre_balance_y', 30, 420, 460, 'r'), me.createNF('pre_balance_fs', 25, 455, 460, 'g'),
                
                {  xtype:'label', html:'&nbsp;미&nbsp;수&nbsp;누&nbsp;계&nbsp;', x:510, y:440, style:'color:#FF9695;background-color:white;font-weight:bold;'}, // 미수누계 (현미수)
                me.createNF('balance_x', 30, 490, 460, 'b'), me.createNF('balance_y', 30, 525, 460, 'r'), me.createNF('balance_fs', 25, 560, 460, 'g'),
...
//				[Const.PaperType.A4_CORP,   '일반 양식지 A4 (업체양식지)']  /* 임시 숨김처리 */
    		]
    	});
    ...
    saveAction : function () {
    	var me = this;
    	var invoiceType = Const.InvoiceType.SALE;
    	var toolbarForm  = me.down('[name=toolbarForm]');
    	var centerForm  = me.down('[name=centerPanel]');
    	    	...
    	    	PrinterPlugin.getPrinterList({
    		    	callback:function(success, printerList, msg){
    		    		var printerNameArray = [];
    		   	    	...
    		   	    	// 새로 생성된 모델
    		    		var invoiceModel = ResourcePrint.createInvoiceModel(Const.InvoiceType.SALE);
