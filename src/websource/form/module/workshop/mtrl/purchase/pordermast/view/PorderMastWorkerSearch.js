Ext.define('module.workshop.mtrl.purchase.pordermast.view.PorderMastWorkerSearch', { extend: 'Axt.form.Search',
	store	: 'module.workshop.mtrl.purchase.pordermast.store.PorderMastInvoice',
	alias	: 'widget.module-pordermast-worker-search',
								...
								listeners: {
									change : function(self, value) {
										...
										resId =  _global.options.mes_system_type?_global.options.mes_system_type.toUpperCase():_global.hq_id.toUpperCase()
											...
											if(resId == 'SJFLV'){
												inv_amt = Math.round((Number(value) * Number(panel.down('[name=offr_pric]').getValue())))
											} else{
							...
							{	text	: '단가', xtype : 'label', style:'text-align:center;'
								...
								listeners : {
									change : function(self, value) {
										...
										if(resId == 'SJFLV'){
											inv_amt = Math.round((Number(value) * Number(panel.down('[name=offr_qntt]').getValue())))
										} else{
								...
								listeners: {
									change : function(self, value) {
										...
										if(editor.down('[name=supl_dvcd]').value == '6000' ){
											inv_vat = 0
											inv_vat2 = Math.round( Number(panel.down('[name=offr_amnt]').getValue()))

											panel.down('[name=ttsm_amnt]').setValue(inv_vat2);
										}else{
