Ext.define('module.prod.order.workentry.view.WorkEntryEndPopup', { extend: 'Axt.popup.Upload',
	alias		: 'widget.module-workentry-popup-end'			,
			...
			form = {
				...
				dockedItems	: [
					{	xtype : 'toolbar',
						...
						items : [
							'->' ,
							{	text : '<span class="write-button">생산취소</span>'		, scope: me , handler: me.cancelAction , cls: 'button1-style'},'-',
...
//								},
							...
							margin	: '0 0 5 0',
							items	: [
								{	fieldLabel	: Language.get('eddt_dttm','종료일시'),
									xtype		: 'datefield',
									name		: 'eddt_dttm',
									width		: 200,
									labelWidth	: 65,
...
//				features	: [{ ftype : 'grid-summary' }],
				...
				columns: [
					{	text : Language.get('mngt_sbsc_dvsn_name'	, '작업공정'	)	, dataIndex: 'mngt_sbsc_dvsn_name'	,  width : 260, align:'center'
					},{	text : Language.get('mngt_sbsc_name'		, '관리항목'	)	, dataIndex: 'mngt_sbsc_name'		,  width : 260, align:'center'
					},{	text : Language.get('mngt_sbsc_spec'		, '규격'		)	, dataIndex: 'mngt_sbsc_spec'		,  width : 190, align:'center'
	...
	closeAction: function(){
		...
		if(!me.popup.params.lott_numb || me.popup.params.lott_numb == ' '){
			//work_book
			Ext.Ajax.request({
				...
				success	: function(response, request) {
					...
					if (!max_lott_numb || max_lott_numb === undefined || max_lott_numb.isEmpty) {
						...
						var next_suffix = String(suffix + 1).padStart(4, '0'); // 1 증가 후 4자리로 패딩
						lott_numb = prefix + next_suffix;
					}
				}
			});
		}else{
			lott_numb = me.popup.params.lott_numb;
		}
