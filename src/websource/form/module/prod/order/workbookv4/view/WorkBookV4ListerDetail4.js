Ext.define('module.prod.order.workbookv4.view.WorkBookV4ListerDetail4', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-workbookv4-detail4'			,
	...
	initComponent: function () {
		var me = this;
		me.dockedItems = [{xtype: 'module-workbookv4-detailSearch2'}];
	...
	columnItem : function () {
		var me = this,
			item = {
				...
				items : [
					{	dataIndex: 'prog_stat_dvcd'	, text : Language.get(''			,'상태'		) , width	: 70	, xtype : 'lookupcolumn', lookupValue : resource.lookup('prog_stat_dvcd'), align : 'center',	sortable	: false
					},{ dataIndex: 'cvic_name'		, text : Language.get(''			,'설비'		) , width	: 125	, align : 'center',	sortable	: false
					},{ dataIndex: 'acpt_numb'		, text : Language.get(''			,'수주번호'	) , width	: 165	, align : 'center', hidden : true,	sortable	: false
					},{ dataIndex: 'wkod_numb'		, text : Language.get(''			,'지시번호'	) , flex	: 2		, align : 'center',hidden:true,	sortable	: false
					},{ dataIndex: 'wkod_seqn'		, text : Language.get(''			,'지시순번'	) , flex	: 2		, align : 'center',hidden:true,	sortable	: false
					},{ dataIndex: 'plan_strt_dttm'	, text : Language.get(''			,'시작예정'	) , width	: 100	, align : 'center',	sortable	: false
					},{ dataIndex: 'work_strt_dttm'	, text : Language.get(''			,'시작일시'	) , width	: 100	, align : 'center',	sortable	: false
					},{ dataIndex: 'work_endd_dttm'	, text : Language.get(''			,'종료(정지)일시'	) , width	: 115	, align : 'center',	sortable	: false
					},{ dataIndex: 'wkod_numb'		, text : Language.get(''			,'지시번호'	) , width	: 220	, hidden : true,	sortable	: false
					},{ dataIndex: 'lott_numb'		, text : Language.get(''			,'LOT번호'	) , width	: 100,	sortable	: false
					},{ dataIndex: 'mold_code'		, text : Language.get(''			,'금형코드'	) , width	: 100,	sortable	: false , align:'center'
					},{ dataIndex: 'item_code'		, text : Language.get(''			,'품목코드'	) , width	: 120,	sortable	: false
					},{ dataIndex: 'item_name'		, text : Language.get(''			,'품명'		) , flex	: 1		, minWidth:200,	sortable	: false
					},{ dataIndex: 'mtrl_name'		, text : Language.get(''			,'재질'		) , width	: 70	, sortable	: false, align : 'center'
					},{ dataIndex: 'pckg_cotr_name'	, text : Language.get(''			,'포장용기'	) , width	: 90	, sortable	: false, align : 'center'
					...
					},{ dataIndex: 'total_qntt'		, text : Language.get(''			,'누적수량'	) , width	: 95	, xtype:'numericcolumn',	sortable	: false
					},{ dataIndex: 'subs_qntt'		, text : Language.get(''			,'잔량'		) , width	: 95	, xtype:'numericcolumn',	sortable	: false,
						renderer:function(val,field,records){
							var value = records.get('indn_qntt')-records.get('total_qntt');
							return Ext.util.Format.number(value, '0,000');
						}
					},{ dataIndex: 'wker_name'		, text : Language.get(''			,'작업자'	) , width	: 115	, align  : 'center', hidden:true,	sortable	: false
						...
						renderer: function(val,meta,rec) {
							var id     = Ext.id(),
								search = Ext.ComponentQuery.query('module-workbookv4-detailSearch')[0]
	...
	insert : function (rec) {
		var search = Ext.ComponentQuery.query('module-workbookv4-detailSearch')[0],
			store = this.getStore(),
