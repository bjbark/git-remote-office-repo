Ext.define('module.prod.order.workbookv3.view.WorkBookV3ListerDetail', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-workbookv3-detail'			,
	selModel	: { selType: 'checkboxmodel', mode : 'MULTI' },
//	store		: 'module.prod.order.workbookv3.store.WorkBookV3Detail',
	border		: 0,
	...
	pagingItem : function () {
		var me = this,
			item = {
				xtype	: 'grid-paging',
				items	: [
					{	text : '<span class="btnTemp" style="font-size:2.0em;">근무조교대</span>', cls : 'button-left btn btn-success' , width: 150, height : 50, margin: '0 0 0 5', hidden : true
						...
						margin : '0 0 0 5',
						handler:  function() {
							var detail    = Ext.ComponentQuery.query('module-workbookv3-detail')[0],
	...
	columnItem : function () {
		var me = this,
			item = {
				...
				defaults: {style: 'text-align: center;font-size:10em !important;'},
				items : [
					...
					},{ dataIndex: 'lott_numb'		, text : Language.get('lott_numb'		,'LOT번호'	) , width : 220	, align  : 'center'
					},{ dataIndex: 'invc_numb'		, text : Language.get('wkod_numb'		,'작업번호'		) , width : 180	, hidden : true
					},{ dataIndex: 'item_name'		, text : Language.get('item_name'		,'품명'		) , flex  : 1	, minWidth : 120
...
//							this.up('window').hide();
		...
		var search = Ext.ComponentQuery.query('module-workbookv3-search')[0],
			searchDate = search.down('[name=work_date]').getValue(),
			wkct_idcd = search.down('[name=wkct_name]').getValue(),
			store = Ext.ComponentQuery.query('module-workbookv3-detail')[0].getStore()
	...
	insp : function(rec){
		...
		console.log(array);
		var store = Ext.ComponentQuery.query('module-workbookv3-detail')[0].getStore();
		var	form = Ext.widget('form', {
			border: false,
			itemId: 'insp',
			bodyPadding: 10,
				...
				{	text: '<span class="btnTemp" style="font-size:3em">확인</span>',
					...
					handler: function() {
						...
						store.each(function(record,idx){
							record.set('half_pich',values.half_pich);
