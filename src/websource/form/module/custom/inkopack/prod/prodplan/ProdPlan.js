Ext.define('module.custom.inkopack.prod.prodplan.ProdPlan', { extend : 'Axt.app.Controller',
	...
	models	: [
		'module.custom.inkopack.prod.prodplan.model.ProdPlan1',
		'module.custom.inkopack.prod.prodplan.model.ProdPlan2'
	],
	stores	: [
		'module.custom.inkopack.prod.prodplan.store.ProdPlan1',
		'module.custom.inkopack.prod.prodplan.store.ProdPlan2',
		'module.custom.inkopack.prod.prodplan.store.ProdPlanWrite'
	],
	views	: [
		'module.custom.inkopack.prod.prodplan.view.ProdPlanLayout',
		'module.custom.inkopack.prod.prodplan.view.ProdPlanLister1',
		'module.custom.inkopack.prod.prodplan.view.ProdPlanLister2',
		'module.custom.inkopack.prod.prodplan.view.ProdPlanSearch',
		'module.custom.inkopack.prod.prodplan.view.ProdPlanCancel',
		'module.custom.inkopack.prod.prodplan.view.ProdPlanWritePopup',
		'module.custom.inkopack.prod.prodplan.view.ProdPlanEditor'
	],
	...
	init : function() {
		...
		me.control({
			...
			'module-prodplan-lister1 button[action=onePlanAction]'		: { click : me.onePlanAction		},	/* 생산계획 */
	...
	modifyAction : function() {
		...
		;
		if(select.get('line_stat')=='1'){
			Ext.Msg.error('확정된 작업지시는 수정할 수 없습니다.');
		}else{
			editor.modifyRecord({
				caller	: me,
...
//						me.pocket.search().setDisabled(false);
	...
	onePlanAction:function() {
		var me  = this,
			...
			master	= Ext.ComponentQuery.query('module-prodplan-lister1')[0],
			a = new Array(),
			b = new Array(),
		...
		for(var i=0;i <records.length;i++){
			if(records[0].get('item_code')!=records[i].get('item_code')){
				Ext.Msg.alert("알림","품목코드가 같은 수주목록을 선택해주십시오.");
				checked = 1
				return;
			}
		}
...
//		var popup = window.open("http://localhost/module/testcall/mblePage1.do?param={}", "gantt_window", 'height:700,width:1500')
	...
	deleteAction:function() {
		...
		if (select == null || select == '' ) {
			Ext.Msg.alert("알림", "삭제 하시려는 건을 선택 후 진행하십시오!");
			return;
		}else{
			for(var i=0;i<select.length;i++){
				if (select[i].get('line_stat')=='1') {
		...
		Ext.Msg.confirm("확인", "삭제하시겠습니까?", function(button) {
			if (button == 'yes') {
				for(var i=0;i<select.length;i++){
					Ext.Ajax.request({
						url			: _global.api_host_info + '/' + _global.app_site + '/custom/inkopack/prod/prodplan/set/delete.do',
						method		: "POST",
						params		: {
						 	token	: _global.token_id,
							param	: Ext.encode({
								...
								acpt_seqn		: records[i].get('acpt_seqn'),
							})
						},
						success : function(response, request) {
							var object = response,
			...
			Ext.Msg.show({ title: '확인', msg: '선택하신 지시를 확정하시겠습니까?', icon: Ext.Msg.QUESTION, buttons: Ext.Msg.YESNO, defaultFocus: 'no', scope: me,
				...
				fn: function (button) {
					if (button=='yes') {
						...
						Ext.each(select, function(record) {
							Ext.Ajax.request({
								url		: _global.location.http() + '/custom/inkopack/prod/prodplan/set/ok.do',
								params	: {
									token : _global.token_id,
									param : JSON.stringify({
	...
	cancleAction:function() {
		...
		if (select && select.length != 0) {
			Ext.each(select, function(record) {
				if (record.get("line_stat") == "0" || record.get('prog_stat_dvcd')!=0) {
...
//			Ext.Msg.confirm("확인", "자재식별표를 발행하시겠습니까?", function(button) {
//				if (button == 'yes') {
//					Ext.Ajax.request({
