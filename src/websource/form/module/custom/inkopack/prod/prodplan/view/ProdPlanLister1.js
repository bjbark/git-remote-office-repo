Ext.define('module.custom.inkopack.prod.prodplan.view.ProdPlanLister1', { extend: 'Axt.grid.Panel',

	alias		: 'widget.module-prodplan-lister1',
	store		: 'module.custom.inkopack.prod.prodplan.store.ProdPlan1',
//	width		: 450,
//	minWidth	: 200,
//	split		: true,
	columnLines : true ,
	...
	pagingItem : function () {
		var me = this,
			item = {
				...
				items : [
					'->', '-' ,
					{text : '<span class="write-button">일정보기</span>'	, iconCls: 'icon-chart',     action : 'changeAction' , cls: 'button-style'} , '-' ,
					{text : '<span class="write-button">생산계획</span>'	, iconCls: Const.MODIFY.icon,action : 'onePlanAction', cls: 'button-style'} , '-' ,
			...
			item =
				{	defaults: {style: 'text-align:center' , align : 'center'},
					items : [
						...
						},{	dataIndex: 'cstm_name'		, width: 150, text: Language.get('cstm_name'		,	'거래처명'	) , align: 'left'
						},{	dataIndex: 'item_code'		, width:  80, text: Language.get('item_code'		,	'품목코드'	)
						},{	dataIndex: 'item_name'		, width: 250, text: Language.get('item'				,	'품명'		) , align: 'left'
						...
						},{	dataIndex: 'user_memo'		, width : 150, text: Language.get('user_memo'		,	'비고'		) , align: 'left'
						...
						},{	dataIndex: 'plan_qntt'		, width:  80, text: Language.get('plan_qntt'		,	'계획수량'	) , align: 'right' , xtype : 'numericcolumn' , summaryType : 'sum'
						},{	dataIndex: 'plan_date'		, width:  80, text: Language.get('plan_date'		,	'시작일시'	)
						},{	dataIndex: 'cmpl_schd_date'	, width:  80, text: Language.get('cmpl_schd_date'	,	'완료일시'	)
						}
					]
				};
		return item;
