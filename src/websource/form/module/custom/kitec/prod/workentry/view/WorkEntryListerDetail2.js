Ext.define('module.custom.kitec.prod.workentry.view.WorkEntryListerDetail2', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-kitec-workenty-detail2'			,
	selModel	: { selType: 'checkboxmodel', mode : 'MULTI' },
	store		: 'module.custom.kitec.prod.workentry.store.WorkEntryDetail2',
	border		: 0,
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{ dataIndex: 'prod_qntt'		, text : Language.get('prod_qntt_l'		,'생산수량(L)'	) , width	: 100	, xtype:'numericcolumn'
					},{ dataIndex: 'prod_qntt_1fst'	, text : Language.get('prod_qntt_r2'	,'생산수량(R)'	) , width	: 100	, xtype:'numericcolumn'
					},{ dataIndex: 'acum_qntt_l'	, text : Language.get('acum_qntt_l'		,'누적수량(L)'	) , width	: 100	, xtype:'numericcolumn'
					},{ dataIndex: 'acum_qntt_r'	, text : Language.get('acum_qntt_r'		,'누적수량(R)'	) , width	: 100	, xtype:'numericcolumn'
...
//			cvic_idcd = search.down('[name=cvic_name]').getValue(),
			...
			store2 = Ext.ComponentQuery.query('module-kitec-workenty-detail2')[0].getStore()
		;
	...
	cancel : function (rec) {
		var search = Ext.ComponentQuery.query('module-kitec-workenty-search')[0],
			searchDate = search.down('[name=work_date]').getValue(),
//			cvic_idcd = search.down('[name=cvic_name]').getValue(),
			store = Ext.ComponentQuery.query('module-kitec-workenty-detail2')[0].getStore()
