Ext.define('module.custom.symct.prod.workentry.view.WorkEntryListerDetail3', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-symct-workentry-detail3'			,
	...
	initComponent: function () {
		var me = this;
		me.dockedItems = [{xtype: 'module-symct-workentry-detailSearch'}];
	...
	columnItem : function () {
		var me = this,
			item = {
				...
				items : [
					{	dataIndex: 'prog_stat_dvcd'	, text : Language.get('prog_stat_dvcd'	,'상태'		)	, width	:100	, xtype : 'lookupcolumn', lookupValue : [["0","대기"],["1","진행중"],["2","지연"],["3","완료"]], align : 'center'
					...
					},{ dataIndex: 'invc_date'		, text : Language.get('indn_date'		,'지시일자'	)	, width	: 100		, align:'center',hidden:true
					},{ dataIndex: 'work_sttm'		, text : Language.get('work_sttm'		,'시작시간'	)	, width	: 100	, align : 'center',
					},{ dataIndex: 'work_edtm'		, text : Language.get('work_edtm'		,'종료시간'	)	, width	: 100	, align : 'center'
						...
						renderer: function(val,meta,rec) {
							var id     = Ext.id(),
								search = Ext.ComponentQuery.query('module-symct-workentry-detailSearch')[0]
	...
	insert : function (rec) {
		var search		= Ext.ComponentQuery.query('module-symct-workentry-search')[0],
			search2		= Ext.ComponentQuery.query('module-symct-workentry-detailSearch')[0],
			searchDate	= search.down('[name=work_date]').getValue(),
			wkct_idcd	= search2.down('[name=wkct_idcd]').getValue(),
			...
			grid		= Ext.ComponentQuery.query('module-symct-workentry-detail3')[0],
			store3		= grid.getStore(),
			index		= store2.indexOf(rec)
		;
...
//		console.log(select);
	...
	restart : function (rec) {
		var search = Ext.ComponentQuery.query('module-symct-workentry-detailSearch')[0],
