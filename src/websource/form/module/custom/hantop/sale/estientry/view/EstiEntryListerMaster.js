Ext.define('module.custom.hantop.sale.estientry.view.EstiEntryListerMaster', { extend: 'Axt.grid.Panel',

	alias		: 'widget.module-estientry-lister-master',
	store		: 'module.custom.hantop.sale.estientry.store.EstiEntryMaster',
		...
		var me = this,
			item = {
				...
				items	: [
					...
					'-', '->', '-',
					{	text : '<span class="write-button">견적금액업로드</span>', action : 'priceAction'		, cls: 'button1-style',width : 90 	},
					{	text : '<span class="write-button">시공 요청서 발행</span>', action : 'requestAction'	, cls: 'button1-style',width : 95 	},
					{	text : '<span class="write-button">생산 소요량 계산</span>'	, action : 'workAction'		, cls: 'button1-style',width : 95  	},
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{	dataIndex: 'esti_dvcd'		, width:  90, align : 'center'	, text: Language.get('esti_dvcd'		, '견적구분코드'	),xtype : 'lookupcolumn' , lookupValue : resource.lookup('esti_dvcd'),filter:true
