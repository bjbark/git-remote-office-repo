Ext.define('module.custom.sjflv.mtrl.po.purcordr.view.PurcOrdrWorkerLister', { extend: 'Axt.grid.Panel',
		...
		var me = this,
			item = {
				...
				items : [
					{	dataIndex:	'line_seqn'		, width: 50 , align : 'center'	, text: Language.get('line_seqn'	, '항번'		 )
					...
					},{	dataIndex:	'each_qntt'		, width:  60, align : 'right'	, text: Language.get(''				, 'Each'	 ), xtype: 'numericcolumn'
						, tdCls : 'editingcolumn'
						, editor	: {
							xtype		:'numericfield',
							selectOnFocus: true,
					...
					},{	dataIndex:	'pack_qntt'		, width:  70, align : 'right'	, text: Language.get(''				, 'Pack Size'), xtype: 'numericcolumn'
						, tdCls : 'editingcolumn'
						, editor	: {
							xtype		:'numericfield',
							selectOnFocus: true,
						...
						, hidden : _global.hq_id.toUpperCase() == 'N1000SJUNG' ? true : false
					},{	dataIndex:	'offr_qntt'		, width:  70, align : 'right'	, text: Language.get('offr_qntt'	, '수량'		 ), xtype: 'numericcolumn', summaryType: 'sum' ,format		: '#,##0.###'
						, tdCls : 'editingcolumn'
						, editor	: {
							xtype		:'numericfield',
							selectOnFocus: true,
...
//						}
					...
					},{	dataIndex:	'vatx_incl_yorn'	, width:  70, align : 'center'	, text: Language.get('vatx_incl_yorn', '부가세포함'	), xtype: 'lookupcolumn' , lookupValue: resource.lookup( 'yorn' ),
						tdCls	: 'editingcolumn',
						editor	: {
							xtype			: 'lookupfield',
...
//										row.set('offr_vatx'	, offr_vatx);
	...
	cellEditAfter : function (editor, context,rowIndexNum) {
		...
		var me = this,
			...
			field = context.field,
			value = context.value,
			each_qntt, pack_qntt, qntt,pric, vatx, rowIndex, vatx_incl_yorn
//			context.record.recalculation( me.editor.getRecord(),context)
		...
		if(field=='each_qntt' || field=='pack_qntt'){
			each_qntt		= me.getStore().getAt(rowIndex).get('each_qntt');		// Each수량
			pack_qntt		= me.getStore().getAt(rowIndex).get('pack_qntt');		// Pack수량
			qntt			= Math.round(each_qntt * pack_qntt);					// 발주량
	...
	listeners: {
		...
		render: function(){
			var me = this;
			var editor = Ext.ComponentQuery.query('module-purcordr-worker-editor')[0];
			new Ext.util.KeyMap({
				target: me.getEl().dom,
				binding: [
					/* Ctrl + Delete */
