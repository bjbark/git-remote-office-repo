Ext.define('module.custom.sjflv.mtrl.po.purcordr.view.PurcOrdrListerDetail', { extend: 'Axt.grid.Panel',

	alias: 'widget.module-purcordr-lister-detail',

	store: 'module.custom.sjflv.mtrl.po.purcordr.store.PurcOrdrDetail',
	...
	columnItem : function () {
		var me = this,
			sjflv_item = {
				...
				items : [
					...
					},{	dataIndex:	'each_qntt'		, width:  80, align : 'right'	, text: Language.get(''				, 'Each'		), xtype: 'numericcolumn',
					},{	dataIndex:	'pack_qntt'		, width:  80, align : 'right'	, text: Language.get(''				, 'Pack Size'	), xtype: 'numericcolumn',
					},{	dataIndex:	'offr_qntt'		, width:  80, align : 'right'	, text: Language.get('offr_qntt'	, '수량'			), xtype: 'numericcolumn', summaryType: 'sum', format: '#,##0.##',
					},{	dataIndex:	'offr_pric'		, width:  80, align : 'right'	, text: Language.get('offr_pric'	, '단가'			), xtype: 'numericcolumn', summaryType: 'sum', format: '#,##0.##',
			...
			sjung_item = {
					...
					items : [
						...
						},{	dataIndex:	'offr_qntt'		, width:  80, align : 'right'	, text: Language.get('offr_qntt'	, '수량'			), xtype: 'numericcolumn', summaryType: 'sum', format: '#,##0.##9',
						},{	dataIndex:	'offr_pric'		, width:  80, align : 'right'	, text: Language.get('offr_pric'	, '단가'			), xtype: 'numericcolumn', summaryType: 'sum', format: '#,##0.###',
						},{	dataIndex:	'offr_amnt'		, width:  80, align : 'right'	, text: Language.get('offr_amnt'	, '금액'			), xtype: 'numericcolumn', summaryType: 'sum', format: '#,##0.###',
						},{	dataIndex:	'offr_vatx'		, width:  80, align : 'right'	, text: Language.get('offr_vatx'	, '부가세'			), xtype: 'numericcolumn', summaryType: 'sum', format: '#,##0.###',
						},{	dataIndex:	'ttsm_amnt'		, width:  80, align : 'right'	, text: Language.get('ttsm_amnt'	, '합계금액'		), xtype: 'numericcolumn', summaryType: 'sum', format: '#,##0.###',
		...
		return  _global.hq_id.toUpperCase() =='N1000SJUNG' ? sjung_item : sjflv_item;
	}
});
