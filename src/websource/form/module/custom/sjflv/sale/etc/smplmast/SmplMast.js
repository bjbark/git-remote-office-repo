Ext.define('module.custom.sjflv.sale.etc.smplmast.SmplMast', { extend:'Axt.app.Controller',
	...
	models	: [
		'module.custom.sjflv.sale.etc.smplmast.model.SmplMast'
		],
	stores	: [
		'module.custom.sjflv.sale.etc.smplmast.store.SmplMast'
	 	],
	views	: [
		'module.custom.sjflv.sale.etc.smplmast.view.SmplMastLayout',
		'module.custom.sjflv.sale.etc.smplmast.view.SmplMastSearch',
		'module.custom.sjflv.sale.etc.smplmast.view.SmplMastLister',
		'module.custom.sjflv.sale.etc.smplmast.view.SmplMastEditor',
	],
	...
	init: function() {
		...
		me.control({
			...
			'module-smplmast-lister button[action=copyAction]'   : { click : me.copyAction   }, // 샘플복사
	...
	amendAction:function(callbackFn) {
		...
		if(select){
			Ext.Msg.confirm("확인", " Amend 하시겠습니까?", function(button) {
				if (button == 'yes') {
					Ext.Ajax.request({
						url			: _global.api_host_info + '/' + _global.app_site + '/custom/sjflv/sale/etc/smplmast/set/amend.do',
						method		: "POST",
						params		: {
	...
	copyAction:function() {
		...
		;
		if(select){
			Ext.Msg.confirm("확인", "선택하신 샘플을 복사 하시겠습니까?", function(button) {
				if (button == 'yes') {
					Ext.Ajax.request({
						url			: _global.api_host_info + '/' + _global.app_site + '/custom/sjflv/sale/etc/smplmast/set/copy.do',
						method		: "POST",
						params		: {
							token	: _global.token_id,
							param	: Ext.encode({
			...
			Ext.Msg.alert("알림","복사할 목록을 선택해주십시오.");
		}
	},
	...
	printAction:function() {
		...
		if(!records || records.length<1) {
			Ext.Msg.alert("알림", "목록중 1건이상을 선택하여주십시오.");
			return;
		}
