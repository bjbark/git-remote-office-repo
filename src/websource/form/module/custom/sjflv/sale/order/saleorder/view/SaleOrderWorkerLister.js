Ext.define('module.custom.sjflv.sale.order.saleorder.view.SaleOrderWorkerLister', { extend: 'Axt.grid.Panel',
	alias: 'widget.module-sjflv-saleorder-worker-lister',
	...
	initComponent: function () {
		var me = this;
		me.dockedItems = [{xtype: 'module-sjflv-saleorder-worker-search'}];
	...
	pagingItem : function () {
		var me = this,
			item = {
				...
				items : [
					'->', '-',
					{	text : '<span class="write-button">주문목록 보기</span>'		, action : 'goOrderList'		, cls : 'button1-style', width : 80  },
					{	text : '<span class="write-button">납기일자 일괄등록</span>'	, action : 'deliAction'			, cls : 'button1-style', width : 120 },
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{	dataIndex:	'prod_trst_dvcd', width: 120, align : 'center'	, text: Language.get('prod_trst_dvcd'	, '생산구분'	), xtype : 'lookupcolumn', lookupValue	: resource.lookup('prod_trst_dvcd')
						,tdCls	: 'editingcolumn',
						editor	: {
							xtype			:'lookupfield',
							selectOnFocus	: true,
					...
					},{	dataIndex:	'invc_qntt'		, width:  80, align : 'right'	, text: Language.get('invc_qntt'		, '수량'		), xtype: 'numericcolumn' , summaryType: 'sum', format: _global.hq_id.toUpperCase()!='N1000SJUNG'?'#,##0.##':'#,##0.###',
						tdCls	: 'editingcolumn',
						editor	: {
							xtype			:'numericfield',
							selectOnFocus	: true,
	...
	cellEditAfter : function (editor, context,rowIndexNum) {
		...
		var me		= this,
			...
			field	= context.field,
			value	= context.value,
			qntt,pric,vatx,rowIndex,vatx_incl_yorn
		;
		//
		...
		if(field=='invc_qntt' || field=='invc_pric'){
			...
			models[rowIndex].set('vatx_amnt',Math.round((qntt * pric)/Number(_global.tax_rt)));
			models[rowIndex].set('invc_amnt',Math.round((qntt * pric)/Number(_global.tax_rt))+Math.round(qntt * pric));
		}

		if(vatx_incl_yorn == 0 ){
			models[rowIndex].set('vatx_amnt',Math.round(0));
			models[rowIndex].set('invc_amnt',Math.round((qntt * pric)));
		}else{
			var sply_amnt = me.getStore().getAt(rowIndex).get('sply_amnt');
//			models[rowIndex].set('vatx_amnt',Math.round((qntt * pric)/Number(_global.tax_rt)));
//			models[rowIndex].set('invc_amnt',Math.round((qntt * pric)/Number(_global.tax_rt))+Math.round(qntt * pric));
			models[rowIndex].set('vatx_amnt',Math.round(sply_amnt / Number(_global.tax_rt)));
			models[rowIndex].set('invc_amnt',Math.round(sply_amnt / Number(_global.tax_rt))+ Math.round(sply_amnt));
		}

		if(field=='vatx_amnt'){
			models[rowIndex].set('invc_amnt',Math.round(vatx)+Math.round(qntt * pric));
		}
	...
	listeners: {
		...
		render: function(){
			var me = this;
			var editor = Ext.ComponentQuery.query('module-sjflv-saleorder-worker-editor')[0];
			new Ext.util.KeyMap({
				target: me.getEl().dom,
				binding: [
					/* Ctrl + Delete */
