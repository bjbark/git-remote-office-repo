Ext.define('module.custom.sjflv.sale.order.saleorder.model.SaleOrderDetail', { extend: 'Axt.data.Model',
	fields: [
		...
		},{	name: 'json_data'			, type: 'string'		//JSON데이터
		},{	name: 'pack_qntt'			, type: 'float'			//JSON데이터
...
//		;
//		if(resId == 'SJFLV' ){
//			row.set('sply_amnt'	, Math.round(row.get('invc_qntt') * row.get('invc_pric')));
//			row.set('vatx_amnt'	, Math.trunc(row.get('sply_amnt')/Number(_global.tax_rt))); //품목에 과세가 없는 경우가 있어 부가세 금액을 변경할 수 있도록 처리
...
//			row.set('vatx_amnt'	, Math.floor(Number(_global.tax_rt) * row.get('sply_amnt')/1000)*10 ); //품목에 과세가 없는 경우가 있어 부가세 금액을 변경할 수 있도록 처리
//			row.set('invc_amnt'	, row.get('sply_amnt') + row.get('vatx_amnt')) ;
//		}
//	}
});