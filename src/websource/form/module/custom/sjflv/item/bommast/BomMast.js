Ext.define( 'module.custom.sjflv.item.bommast.BomMast', { extend:'Axt.app.Controller',
	requires:[
	...
	models: [
		'module.custom.sjflv.item.bommast.model.BomMastLister1',
		'module.custom.sjflv.item.bommast.model.BomMastLister2',
		'module.custom.sjflv.item.bommast.model.BomMastLister3',
	],
	stores: [
		'module.custom.sjflv.item.bommast.store.BomMastLister1',
		'module.custom.sjflv.item.bommast.store.BomMastLister2',
		'module.custom.sjflv.item.bommast.store.BomMastLister3',
	],
	views : [
		'module.custom.sjflv.item.bommast.view.BomMastLayout',
		'module.custom.sjflv.item.bommast.view.BomMastLister1',
		'module.custom.sjflv.item.bommast.view.BomMastLister2',
		'module.custom.sjflv.item.bommast.view.BomMastLister3',
		'module.custom.sjflv.item.bommast.view.BomMastPopup',
		'module.custom.sjflv.item.bommast.view.BomMastCopyPopup',
		'module.custom.sjflv.item.bommast.view.BomMastExcelPopup',
		'module.custom.sjflv.item.bommast.view.BomMastSearch'
	],
	...
	init: function() {
		...
		me.control({
			// layout event
			'module-sjflv-bommast-search button[action=selectAction]'			: { click : me.selectAction }, // 조회
			// lister event
			'module-sjflv-bommast-lister1 button[action=excelAction]'		: { click : me.excelAction }, // 신규

			'module-sjflv-bommast-lister2 button[action=revInsertAction]'		: { click : me.revAction }, // 신규
			'module-sjflv-bommast-lister2 button[action=revDeleteAction]'		: { click : me.revAction }, // 삭제
			'module-sjflv-bommast-lister2 button[action=revUpdateAction]'		: { click : me.revUpdateAction }, // 리비전 저장

			'module-sjflv-bommast-lister3 button[action=copyAction]'		: { click : me.copyAction        }, // 배합표복사
			'module-sjflv-bommast-lister3 button[action=updateAction]'		: { click : me.updateAction      }, // 저장
			'module-sjflv-bommast-lister3 button[action=updownAction]'		: { click : me.updownAction      }, // 위아래
			'module-sjflv-bommast-lister3 button[action=exportAction]'		: { click : me.exportAction      }, // 엑셀
			// lister event
			'module-sjflv-bommast-lister1' : {
				itemclick      : me.selectAction1
			},
			'module-sjflv-bommast-lister2' : {
	...
	pocket : {
		layout  : function () { return Ext.ComponentQuery.query('module-sjflv-bommast-layout')[0]  } ,
		search  : function () { return Ext.ComponentQuery.query('module-sjflv-bommast-search')[0]  } ,
		lister1 : function () { return Ext.ComponentQuery.query('module-sjflv-bommast-lister1')[0] } ,
		lister2 : function () { return Ext.ComponentQuery.query('module-sjflv-bommast-lister2')[0] } ,
		lister3 : function () { return Ext.ComponentQuery.query('module-sjflv-bommast-lister3')[0] } ,
	},
	/**
	 * 조회
	 */
	...
	revAction:function(cont){
		...
		if(select){
			...
			if(set == 'insert'){
				...
				max++;
				resource.loadPopup({
					widget : 'module-sjflv-bommast-popup',
					param : {
						revs_numb : max,
				...
				if(records){
					Ext.Msg.show({ title: '확인', msg: '선택한 자료를 삭제하시겠습니까?', icon: Ext.Msg.ERROR, buttons: Ext.Msg.YESNO, defaultFocus: 'no', scope: me,
						fn : function (button) {
							if (button==='yes') {
								Ext.Ajax.request({
									url		: _global.location.http() + '/custom/sjflv/item/bommast/set/recordRevs.do',
									params	: {
										token : _global.token_id,
										param : JSON.stringify({
		...
		if(select){
			resource.loadPopup({
				...
				param : {
					item_idcd : select.get('item_idcd'),
					revs_dvcd : lister.down('[name=revs_dvcd]').getValue(),
					//acct_bacd : (lister.down('[name=revs_dvcd]').getValue() == 1) ? "생산품" : "품목보고서" // 배합비복사 품목 팝업창으로 이동 (23.11.15)
				},
			});
		}else{
			Ext.Msg.alert('알림','제품을 선택 후 진행해주세요.');
