Ext.define('module.custom.sjflv.item.itemmast.ItemMast', { extend:'Axt.app.Controller',
	...
	models:
	[
	 	'module.custom.sjflv.item.itemmast.model.ItemMast',
	 	'module.custom.sjflv.item.itemmast.model.ItemMastMemo',
	 	'module.custom.sjflv.item.itemmast.model.ItemMastMngt',
	 	'module.custom.sjflv.item.itemmast.model.ItemMastPkge',
	 	'module.custom.sjflv.item.itemmast.model.ItemMastFile'
	],
	stores:
	[
	 	'module.custom.sjflv.item.itemmast.store.ItemMast',
	 	'module.custom.sjflv.item.itemmast.store.ItemMastMemo',
	 	'module.custom.sjflv.item.itemmast.store.ItemMastMngt',
	 	'module.custom.sjflv.item.itemmast.store.ItemMastPkge',
	 	'module.custom.sjflv.item.itemmast.store.ItemMastFile'
	],
	views:
	[
		'module.custom.sjflv.item.itemmast.view.ItemMastLayout',
		'module.custom.sjflv.item.itemmast.view.ItemMastSearch',
		'module.custom.sjflv.item.itemmast.view.ItemMastLister',
		'module.custom.sjflv.item.itemmast.view.ItemMastEditor',
		'module.custom.sjflv.item.itemmast.view.ItemMastEditorLister',
		'module.custom.sjflv.item.itemmast.view.ItemMastMemoLister',
		'module.custom.sjflv.item.itemmast.view.ItemMastMngtLister',
		'module.custom.sjflv.item.itemmast.view.ItemMastPkgeLister',
		'module.custom.sjflv.item.itemmast.view.ItemMastSelectPopup',
		'module.custom.sjflv.item.itemmast.view.ItemMastModifyPopup',
	],
	...
	init: function() {
		...
		me.control({
			...
			// lister event
			'module-itemmast-lister button[action=printAction]'  : { click : me.printAction },
			'module-itemmast-lister button[action=popupAction]'  : { click : me.popupAction },	// 원료성분조회
			...
			'module-itemmast-lister button[action=popupAction2]' : { click : me.popupAction2 },	// 일괄정보변경
...
//					}, 50);
	...
	selectRecord:function( grid, record ){
		...
		if(record.length > 0 && record[0].get('item_idcd').length > 0){
			...
			if (_global.hq_id.toUpperCase() == 'N1000SJFLV') {
				...
				// 라벨
				if (record[0].get('acct_bacd') != '1001' && record[0].get('acct_bacd') != '1002' && record[0].get('acct_bacd') != '2003' ) {
					lister.down('[itemId=popup]').show();
				}
			}
	...
	popupAction:function() {
		var me = this,
			...
			select2 = master.getSelectionModel().getSelection(),
			mtrl_spec,mtrl_spec2
		;

		if(select){
			Ext.Ajax.request({
				url		: _global.location.http() + '/custom/sjflv/item/itemmast/get/select.do',
				params	: {
					token : _global.token_id,
					param : JSON.stringify({
...
//						console.log(result.records[0]);
						mtrl_spec = result.records[0].mtrl_spec,
						mtrl_spec_2 =result.records[0].mtrl_spec_2
					};
				},
				failure : function(result, request) {
				},
...
//			var numb = Ext.ComponentQuery.query('#invc_numb')[0].setValue(records[0].data.invc_numb);
			resource.loadPopup({
				widget : 'module-itemmast-select-popup',
				params :	{
					mtrl_spec	: mtrl_spec,
					mtrl_spec_2	: mtrl_spec_2
				}

			});
		}else{
			Ext.Msg.alert('알림','조회할 품목을 선택해주세요.')
		}
	},

	popupAction2:function() {
		var me = this,
			master = me.pocket.lister(),
			select = master.getSelectionModel().getSelection()[0],
			select2 = master.getSelectionModel().getSelection(),
		...
		resource.loadPopup({
			widget : 'module-itemmast-modify-popup',
//			params :	{
//				mtrl_spec	: mtrl_spec,
//				mtrl_spec_2	: mtrl_spec_2
//			}
...
//									purc.tab.show();
							...
							Ext.Ajax.request({
								url		: _global.location.http() + '/custom/sjflv/item/itemmast/get/itemCode.do',
								params	: {
									token : _global.token_id,
									param : JSON.stringify({
										stor_id			: _global.stor_id,
	...
	deleteAction:function() {
		...
		if (select) {
			Ext.Msg.confirm("확인", "삭제하시겠습니까?", function(button) {
				...
				mask.show();
					Ext.Ajax.request({
						url			: _global.api_host_info + '/' + _global.app_site + '/custom/sjflv/item/itemmast/set/del_yn.do',
						method		: "POST",
						params		: {
						 	token	: _global.token_id,
							param	: Ext.encode({
...
//			var arg = '_item_idcd~'+param.item_idcd;
//			var _param = '_param~{\'item_idcd\':\''+param.item_idcd.replace(/\-/g,'')+'\'}~';
...
//			return win;
		...
		if (select) {
			var item_idcd = select[0].get('item_idcd');
