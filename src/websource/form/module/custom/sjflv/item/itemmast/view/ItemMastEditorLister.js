Ext.define('module.custom.sjflv.item.itemmast.view.ItemMastEditorLister', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-itemmast-editorlister'			,
	store		: 'module.custom.sjflv.item.itemmast.store.ItemMastFile',
					...
					{	fieldLabel	: Language.get('file_name','파일첨부'),			// temp
						...
						popup: {
							...
							result : function(records, nameField, pairField) {
								var store = Ext.ComponentQuery.query('module-itemmast-editorlister')[0].getStore();
								store.reload();
							}
						},listeners:{
							render:function(){
						...
						text :'삭제',
						hidden : (_global.auth.auth_down_file_1001) ? false : true,	// 23.10.05 - 파일다운 권한이 있는 경우 삭제 버튼 활성화
						listeners:{
							click:function(){
				...
				items : [
					{	dataIndex: 'line_seqn'		, text : Language.get('line_seqn'		,'순번'	)		, width : 80 , align : 'center',hidden:true
						...
						renderer:function(val,meta,record){
							...
							// 23.10.05 - 파일다운 권한이 있는 경우  <a> 태그 적용
							if (_global.auth.auth_down_file_1001) {
								title = '<a>'+title+'</a>'
							}
							return title;
						},
						listeners:{
							click : function(view,el,pos){
								// 23.10.05 - 파일다운 권한이 있는 경우  click 이벤트  처리
								if (_global.auth.auth_down_file_1001) {
...
//												window.open(url,'down','width=1400,height=800');
												window.open(encodeURI(url),'test','width=1400,height=800');
											}
											console.log(result);
										},
										failure : function(result, request) {
