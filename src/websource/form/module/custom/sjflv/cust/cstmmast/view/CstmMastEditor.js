Ext.define('module.custom.sjflv.cust.cstmmast.view.CstmMastEditor', {
	extend : 'Axt.form.Editor',
	alias : 'widget.module-cstmmast-editor',
	height : _global.options.mes_system_type.toUpperCase() == 'SJFLV' ? 405 : 374,
	layout : {
		type : 'border'
	},
	title : '거래처 정보',
	collapsible : true,
	collapsed : _global.hq_id.toUpperCase() == 'N1000LIEBE' ? false : true,
	...
	createDock : function() {
		...
		var item = {
			...
			dock : 'bottom',
			items : [ '->', '-', {
				text : Const.UPDATE.text,
				iconCls : Const.UPDATE.icon,
				action : Const.UPDATE.action,
				cls : 'button-style'
			}, {
				text : Const.CANCEL.text,
				iconCls : Const.CANCEL.icon,
				action : Const.CANCEL.action,
				cls : 'button-style'
			}, '-' ]
		};
		return item;
	...
	createWest : function() {
		var me = this, item = {
			...
			items : [ {
				...
				clearable : false,
				onwerEditing : true,
				hidden : _global.hq_id == 'N1000HNTOP' ? false : true,
				popup : {
					select : 'SINGLE',
				...
				margin : '0 0 5 0',
				items : [ {
					fieldLabel : Language.get('', '종사업장번호'),
					name : 'spec_buss_numb',
					xtype : 'textfield',
					maxLength : 200,
					hidden : _global.options.mes_system_type.toUpperCase() == 'SJFLV' ? false : true,
	...
	createTabs : function() {
		var me = this, tabs = {
			...
			items : [ me.createTab1()/* , me.createTab2() */, me.createTab3(), me.createTab4(), {
				title : '은행',
				xtype : 'module-cstmmast-bank-lister',
				hidden : (_global.options.mes_system_type != "SJFLV")
			}, {
				title : '첨부파일',
				xtype : 'module-cstmmast-editorlister'
			}, me.createTab5(), {
				title : '관리항목',
				xtype : 'module-cstmmast-mngtlister'
			} ]
		};
		return tabs;
	},
...
//						emptyText : (_global.options.mes_system_type == 'Window' ? '' : Const.invalid.emptyValue)
				...
				items : [ {
					fieldLabel : Language.get('base_addr_engl', '영문주소'),
					xtype : 'textfield',
					name : 'base_addr_engl',
					width : 495,
					hidden : _global.options.mes_system_type.toUpperCase() == 'SJFLV' ? false : true,
				...
				items : [ {
					...
					popup : {
						select : 'SINGLE',
						widget : 'lookup-delylcal-popup',
						params : {
							stor_grp : _global.stor_grp,
							line_stat : '0'
						},
						result : function(records, nameField, pairField) {
							nameField.setValue(records[0].get('lcal_name'));
							pairField.setValue(records[0].get('lcal_idcd'));
						}
					}
				}, {
					name : 'lcal_idcd',
				...
				items : [ {
					fieldLabel : Language.get('rpst_cstm_name', '대표거래처'),
					xtype : 'popupfield',
					editable : true,
					enableKeyEvents : true,
					name  : 'rpst_cstm_name',
	...
	createTab3 : function() {
		...
		var item = {
			...
			xtype : 'module-cstmmast-drtr-lister',
		};
		return item;
	},
	/**
	...
	createTab4 : function() {
		...
		var item = {
			...
			xtype : 'module-cstmmast-deli-lister'
		};
		return item;
	},
