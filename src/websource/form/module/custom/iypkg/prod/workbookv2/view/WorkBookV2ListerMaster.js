Ext.define('module.custom.iypkg.prod.workbookv2.view.WorkBookV2ListerMaster', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-workbookv2-lister'			,
	selModel	: { selType: 'checkboxmodel', mode : 'SINGLE' },
	store		: 'module.custom.iypkg.prod.workbookv2.store.WorkBookV2',
	border		: 0,
...
//					},{ dataIndex: 'work_endd'		, text : Language.get('work_endd'		,'종료예정'		) , width : 100 , align : 'center'
	...
	insert : function (rec,upf) {
		var me = this;
		var search = Ext.ComponentQuery.query('module-workbookv2-search')[0],
			wkct_idcd = search.down('[name=wkct_name]').getValue(),
			store = Ext.ComponentQuery.query('module-workbookv2-lister')[0].getStore(),
			invc_date, dayn_dvcd, invc_numb, work_sttm
		;
//		if(rec.get('prog_stat_dvcd')==2){
//			Ext.Ajax.request({
//				url		: _global.location.http() + '/custom/iypkg/prod/workbookv2/get/pause.do',
//				params	: {
		...
		var	form = Ext.widget('form', {
			...
			items:[
				{	fieldLabel	: Language.get('work_date','작업일자'),
					...
					popup: {
						select : 'SINGLE',
						widget : 'lookup-workbookv2-user-popup',
						params : { stor_grp : _global.stor_grp, dept_idcd:'500' },
					...
					popup: {
						select : 'SINGLE',
						widget : 'lookup-workbookv2-cvic-popup',
...
//							Ext.Ajax.request({
//								url			: _global.api_host_info + '/' + _global.app_site + '/custom/iypkg/prod/workbookv2/get/book_cnt.do',
//								method		: "POST",
//								params		: {
//								 	token	: _global.token_id,
//									param	: Ext.encode({
								...
								if(select){
									...
									store.sync({
										callback: function(batch, options) {
											...
											Ext.Ajax.request({
												url			: _global.api_host_info + '/' + _global.app_site + '/custom/iypkg/prod/workbookv2/get/plc_cnt.do',
												method		: "POST",
												async		: false,
												params		: {
													token	: _global.token_id,
			...
			buttons: [
				{	text: '<span class="btnTemp" style="font-size:3em">확인</span>',
					...
					handler: function() {
						var rec2;
						Ext.Ajax.request({
							url			: _global.api_host_info + '/' + _global.app_site + '/custom/iypkg/prod/workbookv2/get/workbook.do',
							method		: "POST",
							async		: false,
							params		: {
								token	: _global.token_id,
						...
						{ layout : {type:'vbox', align: 'stretch' },
							...
							items:[
								{	layout : {type:'hbox', align: 'stretch' },
									...
									flex : 1,
									items:[
										{	fieldLabel	: Language.get('indn_date','작성일자'),
											name		: 'indn_date',
											xtype		: 'datefield',
											readOnly	: true,
											...
											items:[
												{	xtype	: 'button',
													...
													handler	: function(){
														...
														Ext.Ajax.request({
															url			: _global.api_host_info + '/' + _global.app_site + '/custom/iypkg/prod/workbookv2/get/cnt.do',
															method		: "POST",
															async		: false,
															params		: {
																token	: _global.token_id,
									...
									items:[
										{	fieldLabel	: Language.get('cstm_name','거래처명'),
											...
											items:[
												{	xtype	: 'button',
													text	: '<span class="btnTemp" style="font-size:2em;color:yellow;">일시정지</span>',
													cls		: 'btn btn-info',
													margin	: 5,
													flex	: 1,
													handler	: function(grid, rowIndex, colIndex, item, e, record){
														...
														if(stat != 0){
															//일시정지된 seqn 구하기
															Ext.Ajax.request({
																url			: _global.api_host_info + '/' + _global.app_site + '/custom/iypkg/prod/workbookv2/get/pauseSeqn.do',
																method		: "POST",
																async		: false,
																params		: {
																	token	: _global.token_id,
															...
															//일시정지
															Ext.Ajax.request({
																url			: _global.api_host_info + '/' + _global.app_site + '/custom/iypkg/prod/workbookv2/set/pause.do',
																method		: "POST",
																async		: false,
																params		: {
																	token	: _global.token_id,
												...
												{	xtype	: 'button',
													text	: '<span class="btnTemp" style="font-size:2em;color:red;">불량</span>',
													cls		: 'btn btn-info',
													margin	: 5,
													flex	: 1,
													handler	: function(grid, rowIndex, colIndex, item, e, record){
														...
														if(stat == 1){
															console.log('asd');
															resource.loadPopup({
//																	widget	: 'lookup-bookv2-poor-popup',
																widget	: 'lookup-bookv2-poor-popup',
												...
												{	xtype	: 'button',
													text	: '<span class="btnTemp" style="font-size:2em;color:yellow">유실</span>',
													cls		: 'btn btn-info',
													margin	: 5,
													flex	: 1,
													handler	: function(grid, rowIndex, colIndex, item, e, record){
														...
														if(stat == 1){
															resource.loadPopup({
																widget	: 'lookup-bookv2-fail-popup',
																params	: {invc_numb : rec.get('invc_numb'), line_seqn : rec.get('line_seqn'), record : rec},
															});
														}
													}
									...
									items:[
										{	fieldLabel	: Language.get('','상자규격'),
											...
											readOnly	: true,
											fieldCls	: 'readonlyfield',
											value		: rec.get('prod_leng')+'*'+rec.get('prod_widh')
										},{	fieldLabel	: Language.get('','상자타입'),
											name		: '',
											xtype		: 'textfield',
											readOnly	: true,
											fieldCls	: 'readonlyfield',
											value		: rec.get('bxty_name')
									...
									items:[
										{	fieldLabel	: Language.get('','원지구성'),
											...
											fieldStyle	: 'background-color:red;color:white;font-size:2em !important; alien;text-align: center;',
											value		: rec.get('stat_dvcd')
											...
											fieldCls	: 'readonlyfield',
											width		: 460,
											value		: rec.get('pcod_numb')
										}
									]
								}
							]
						},{	xtype	: 'module-workbookv2-fabc-lister',
							height	: 60,
							border	: 0
						},{	xtype	: 'carousel',
							name	: 'apnd_imge',
