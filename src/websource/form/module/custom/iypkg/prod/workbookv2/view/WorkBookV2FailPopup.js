Ext.define('module.custom.iypkg.prod.workbookv2.view.WorkBookV2FailPopup', { extend: 'Axt.popup.Search',
	id 		: 'lookup-bookv2-fail-popup',
	alias	: 'widget.lookup-bookv2-fail-popup',
	...
	initComponent: function(config){
		...
		//유실 조회
		var store = Ext.ComponentQuery.query('module-workbookv2-fail')[0].getStore(),
		...
		var me = this,
			form = {
				...
				items			: [
					...
					},{	xtype : 'module-workbookv2-fail', region:'center' , flex : 1, height:500 ,margin: '0 0 0 0', itemId : 'grid1'
					}
				],
				buttons: [
...
//							});
	...
	failupdate : function (val, param) {
		var me = this,
			...
			sttm      = sttm1.replace(':',''),
			edtm      = edtm1.replace(':',''),
			detail    = Ext.ComponentQuery.query('module-workbookv2-lister')[0],
			fail      = Ext.ComponentQuery.query('module-workbookv2-fail')[0],
		...
		if(sttm1==null||edtm1==null||sttm1==''||edtm1==''||sttm>edtm){
			...
			Ext.Ajax.request({
				url		: _global.location.http() + '/custom/iypkg/prod/workbookv2/set/fail.do',
				params	: {
					token : _global.token_id,
					param : JSON.stringify({
						_set			: 'insert',
