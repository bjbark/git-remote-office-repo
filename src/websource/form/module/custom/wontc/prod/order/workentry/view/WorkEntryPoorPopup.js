Ext.define('module.custom.wontc.prod.order.workentry.view.WorkEntryPoorPopup', { extend: 'Axt.popup.Search',
	id 		: 'lookup-wontc-poor-popup',
	alias	: 'widget.lookup-wontc-poor-popup',
	...
	initComponent: function(config){
		...
		//불량 조회
		var store = Ext.ComponentQuery.query('module-wontc-workentry-poor')[0].getStore(),
			param = Ext.merge({ invc_numb : me.popup.params.new_invc_numb
			})
		;
		...
		var me = this,
			form = {
				...
				items			: [
					...
					},{	xtype : 'module-wontc-workentry-poor', region:'center' , flex : 1, height:500 ,margin: '0 0 0 0', itemId : 'grid1'
					}
				],
				buttons: [
		...
		var form = Ext.widget('form', {
			...
			items:[
				{	xtype		: 'label',
					...
					listeners:{
						render:function(field ){
							field.getEl().on('click', function( event, el ) {
								var trigger1 = Ext.dom.Query.select('.trigger1qc')[0];
					...
					trigger1Cls : 'hideCls trigger1qc',
				},
			],
			buttons: [
	...
	poorupdate : function (rec, rec2, param) {
		var me = this,
			poor_bacd = rec,
			poor_qntt = param.poor_qntt,
			lister    = Ext.ComponentQuery.query('module-wontc-workentry-lister')[0],
			poor      = Ext.ComponentQuery.query('module-wontc-workentry-poor')[0],
			store     = poor.getStore(),
			record, line_seqn
		;
		...
		if(poor_qntt==0||poor_qntt==''||poor_qntt==null){
			...
			//line_seqn count
			Ext.Ajax.request({
				url		: _global.location.http() + '/custom/wontc/prod/order/workentry/get/poorseqn.do',
				params	: {
					token : _global.token_id,
					param : JSON.stringify({
						stor_id			: _global.stor_id,
						hqof_idcd		: _global.hqof_idcd,
						invc_numb		: me.popup.params.new_invc_numb
					})
				},
				async	: false,
				method	: 'POST',
			...
			Ext.Ajax.request({
				url		: _global.location.http() + '/custom/wontc/prod/order/workentry/set/poor.do',
				params	: {
					token : _global.token_id,
					param : JSON.stringify({
						_set			: 'insert',
