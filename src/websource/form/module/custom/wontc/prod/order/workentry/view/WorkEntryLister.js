Ext.define('module.custom.wontc.prod.order.workentry.view.WorkEntryLister', { extend: 'Axt.grid.Panel',
	id			: 'module-wontc-workentry-lister',
	alias		: 'widget.module-wontc-workentry-lister',
	store		: 'module.custom.wontc.prod.order.workentry.store.WorkEntryLister',
								...
								{	fieldLabel	: Language.get('item_name','품명'),
									...
									items	: [
										{	fieldLabel	: Language.get('indn_qntt','지시수량'),
											...
											fieldCls	: 'textTemp '+_global.options.work_book_tema+'field',
											labelWidth	: 80
										},{	text : '<span class="btnTemp" style="font-size:2em; font-weight:bold;">검사등록</span>'  ,
											xtype	: 'button',
											...
											handler	: function(){
												...
												var invc_numb = editor.down('[name=pror_numb]').getValue(),
												    chk       = 0
												;
												Ext.Ajax.request({
													url		: _global.location.http() + '/custom/wontc/prod/order/workentry/get/wkfw.do',
													params	: {
														token : _global.token_id,
														param : JSON.stringify({
															wkct_idcd : wkct_idcd,
													...
													success	: function(response, request) {
														...
														if	(!result.success ){
															...
															chk = result.records[0].wkct_insp_yorn!=""?result.records[0].wkct_insp_yorn:'0';
														}
													},
													failure : function(result, request) {
													},
...
//	},
	...
	workwkfw : function(){
		...
		var a = resource.loadPopup({
			...
			result	: function(records) {
				resource.loadPopup({
					widget : 'lookup-insptypeitem-popup',
					params	: { stor_grp : _global.stor_grp , row_sts : '0',tema : _global.options.work_book_tema,insp_type_idcd:records[0].get('insp_type_idcd')},
					result	: function(records) {
