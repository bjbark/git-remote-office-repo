Ext.define('module.custom.dehansol.sale.saleorder.view.SaleOrderLabelPopup', { extend: 'Axt.popup.Search',
	alias: 'widget.module-saleorder-label-popup',
	...
	finishAction: function(){
		...
		Ext.Ajax.request({
			url		: _global.location.http() + '/custom/dahansol/sale/saleorder/get/report_kind.do',
			params	: {
		...
		if(!jrf){
			Ext.Msg.alert("알림","TOOL번호 유형이같은 수주목록을 선택해주십시오.");
			return;
		}
		Ext.Ajax.request({
			url		: _global.location.http() + '/custom/dahansol/sale/saleorder/set/label.do',
			params	: {
			token	: _global.token_id,
			param	: JSON.stringify({
