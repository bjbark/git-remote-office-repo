Ext.define('module.custom.komec.sale.spts.sptsmast.model.SptsMastWorkerDetail', {extend:'Axt.data.Model',
	fields  : [
	   		...
	   		{	name: 'cstm_idcd',			type: 'string'},		//납품거래처ID
	   		{	name: 'cstm_code',			type: 'string'},		//납품거래처ID
			{	name: 'drtr_idcd',			type: 'string'},		//엽업당당자ID
			{	name: 'drtr_name',			type: 'string'},		//엽업당당자명
			...
			{	name: 'invc_date',			type: 'string' , convert : Ext.util.Format.strToDate, serialize: Ext.util.Format.dateToStr},
	   		...
	   		{	name: 'not_trst_qntt',		type: 'float' },		//미출잔량
	   		{	name: 'spts_qntt',			type: 'float' },		//출고의뢰수량
	   		{	name: 'invc_pric',			type: 'float' },		//단가
	   		{	name: 'sply_amnt',			type: 'float' },		//공급가액
	   		{	name: 'invc_amnt',			type: 'float' },		//거래금액
	   	]
});
