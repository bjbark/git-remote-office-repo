Ext.define('module.custom.kortc.qc.insp.inspentry.view.InspEntryFileLister', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-inspentry-filelister'			,
	store		: 'module.custom.kortc.qc.insp.inspentry.store.InspEntryFile',
				...
				fbar	: [
					{	fieldLabel	: Language.get('file_name','파일첨부'),					// temp
						...
						popup: {
							...
							params : { stor_grp : _global.stor_grp , row_sts : '0' ,invc_numb : '' , orgn_dvcd : '' , line_seqn : 1},
							result : function(records, nameField, pairField) {
								var store = Ext.ComponentQuery.query('module-inspentry-filelister')[0].getStore();
								store.reload();
							},
							create : function (self ) {
								filelister = Ext.ComponentQuery.query('module-inspentry-filelister')[0],
								store = Ext.ComponentQuery.query('module-inspentry-filelister')[0].getStore(),
								selection = filelister.getSelectionModel().getSelection()[0],
								records	= filelister.getSelectionModel().getSelection();
								console.log(filelister);
								console.log(store);
							}
						},listeners:{
							render:function(){
								this.popup.params.orgn_dvcd = 'insp_mast';											// table name이 필요하다
							}
						},
					},{	xtype:'button',
						text :'삭제',
