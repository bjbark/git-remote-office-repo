...
Ext.define('module.mtrl.po.purcordr.PurcOrdr', { extend : 'Axt.app.Controller',
	...
	models:[
		'module.mtrl.po.purcordr.model.PurcOrdrInvoice',
		'module.mtrl.po.purcordr.model.PurcOrdrMaster',
		'module.mtrl.po.purcordr.model.PurcOrdrDetail',
	],
	stores:[
		'module.mtrl.po.purcordr.store.PurcOrdrInvoice',
		'module.mtrl.po.purcordr.store.PurcOrdrMaster',
		'module.mtrl.po.purcordr.store.PurcOrdrDetail',
	],
	views : [
		'module.mtrl.po.purcordr.view.PurcOrdrLayout',
		/* 현황 */
		'module.mtrl.po.purcordr.view.PurcOrdrSearch',
		'module.mtrl.po.purcordr.view.PurcOrdrListerMaster',
		'module.mtrl.po.purcordr.view.PurcOrdrListerDetail',
		'module.mtrl.po.purcordr.view.PurcOrdrIsttPopup',
		/* 작업 */
		'module.mtrl.po.purcordr.view.PurcOrdrWorkerEditor',
		'module.mtrl.po.purcordr.view.PurcOrdrWorkerSearch',
		'module.mtrl.po.purcordr.view.PurcOrdrWorkerLister',
	...
	init: function() {
		...
		me.control({
			...
			'module-purcordr-lister-master button[action=isttAction2]'			: { click : me.isttAction2        }, /* 한탑입고접수 */
			...
			Ext.Msg.show({ title: '확인', msg: '선택하신 발주를 승인하시겠습니까?', icon: Ext.Msg.QUESTION, buttons: Ext.Msg.YESNO, defaultFocus: 'no', scope: me,
				fn: function (button) {
					if (button=='yes') {
						...
						Ext.each(select, function(record) {
							Ext.Ajax.request({
								url		: _global.location.http() + '/mtrl/po/purcordr/set/ok.do',
								params	: {
									token : _global.token_id,
									param : JSON.stringify({
										invc_numb		: record.get('invc_numb'),
...
//			for (var i = 0; i < records.length; i++) {
			Ext.each(select, function(record) {
				Ext.Ajax.request({
					url			: _global.api_host_info + '/' + _global.app_site + '/mtrl/po/purcordr/chk/cancel.do',
					method		: "POST",
					params		: {
						token	: _global.token_id,
						param	: Ext.encode({
	...
	deleteAction:function() {
		...
		Ext.Msg.confirm("확인", "삭제하시겠습니까?", function(button) {
			if (button == 'yes') {
				...
				Ext.Ajax.request({
					url			: _global.api_host_info + '/' + _global.app_site + '/mtrl/po/purcordr/set/del_yn.do',
					method		: "POST",
					params		: {
					 	token	: _global.token_id,
						param	: Ext.encode({
...
//					}
	...
	imptAction:function(){
		...
		if(select){
			Ext.Ajax.request({
				url			: _global.api_host_info + '/' + _global.app_site + '/mtrl/po/purcordr/get/impt.do',
				method		: "POST",
				async	: false,
				params		: {
					token	: _global.token_id,
			...
			Ext.Msg.confirm("확인", "수입 Order 등록하시겠습니까?", function(button) {
				if (button == 'yes') {
					if(select.get('supl_dvcd')=='6000' && cnt == 0){
						Ext.Ajax.request({
							url			: _global.api_host_info + '/' + _global.app_site + '/mtrl/po/purcordr/set/impt.do',
							method		: "POST",
							async	: false,
							params		: {
								token	: _global.token_id,
