Ext.define('module.mtrl.po.purcisttwork.view.PurcIsttWorkListerMaster', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-purcisttwork-lister-master',
	store		: 'module.mtrl.po.purcisttwork.store.PurcIsttWorkMaster',
	...
	pagingItem : function () {
		var me = this,
			item = {
				...
				items	: [
					'->','-' ,
					{	text : '<span class="write-button">라벨발행</span>'	, action : 'labelAction' , cls: 'button1-style',hidden :_global.options.mes_system_type.toUpperCase() != 'SJFLV' ? true : false,},
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{ dataIndex: 'acpt_numb'		, text : Language.get('acpt_numb'		,'수주번호'		) , width : 110 , align : 'center', hidden : (_global.hq_id.toUpperCase() == 'N1000DHTEC'|| _global.hq_id.toUpperCase() == 'N1000A-ONE'? true:false)
					...
					},{ dataIndex: 'zone_name'		, text : Language.get('zone_name'		,'입고구역'		) , width : 120 , align : 'left', hidden :  _global.hq_id.toUpperCase() == 'N1000A-ONE'? false:true,
					...
					},{ dataIndex: 'make_cmpy_name2', text : Language.get('make_cmpy_name'	,'제조회사명'	) , width : 150 , align : 'left',	hidden	: (_global.options.haccp_item_yorn == 0)/*||  _global.options.mes_system_type.toUpperCase() == 'SJFLV' ? true:false */,
					...
					},{ dataIndex: 'rtil_ddln_date'	, text : Language.get('rtil_ddln_date'	,'유통기한'		) , width : 80  , align : 'center',	hidden: _global.options.mes_system_type.toUpperCase() == 'SJFLV' || _global.hq_id.toUpperCase() != 'N1000A-ONE' ? false : true
					...
					},{ dataIndex: 'istt_qntt'		, text : Language.get('istt_qntt'		,'수량'		) , width : 60  , xtype : 'numericcolumn', align : 'right', summaryType: 'sum', format: _global.hq_id.toUpperCase()=='N1000SJUNG' ? '#,##0.##9' : (_global.hq_id.toUpperCase()!='N1000SJFLV'?'#,##0.##':'#,##0.###'),
