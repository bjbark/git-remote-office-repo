Ext.define('module.mtrl.po.purcordr2.view.PurcOrdr2ListerMaster', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-purcordr2-lister-master',

	store		: 'module.mtrl.po.purcordr2.store.PurcOrdr2Master',
		...
		var me = this,
			item = {
				...
				items	: [
					...
					},{	text : '<span class="write-button">입고접수</span>'	, action : 'isttAction' , cls: 'button1-style',hidden : _global.hq_id.toUpperCase()=='N1000KITEC' || _global.hq_id.toUpperCase()=='N1000HNTOP' || _global.options.mes_system_type.toUpperCase() == 'SJFLV' || _global.hq_id.toUpperCase()=='N1000A-ONE' ? true:false
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{	dataIndex: 'offr_qntt'	, width:  80, align : 'right'	, text: Language.get( 'offr_qntt'	, '합계수량'	), xtype: 'numericcolumn' , summaryType: 'sum' , format: _global.hq_id.toUpperCase()!='N1000SJUNG'?'#,##0.##':'#,##0.##0',
					},{	dataIndex: 'offr_amnt'	, width:  80, align : 'right'	, text: Language.get( 'offr_amnt'	, '발주금액'	), xtype: 'numericcolumn' , summaryType: 'sum' , format: _global.hq_id.toUpperCase()!='N1000SJUNG'?'#,##0.##':'#,##0.##0',
					},{	dataIndex: 'offr_vatx'	, width:  80, align : 'right'	, text: Language.get( 'vatx_amnt'	, '부가세'		), xtype: 'numericcolumn' , summaryType: 'sum' , format: _global.hq_id.toUpperCase()!='N1000SJUNG'?'#,##0.##':'#,##0.##0',
...
//					},{	dataIndex: 'trst_numb'	, width : 80, align : 'center'	, text: Language.get( 'trst_numb'	, '요청번호'	)
