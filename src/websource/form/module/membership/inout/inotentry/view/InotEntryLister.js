Ext.define('module.membership.inout.inotentry.view.InotEntryLister', { extend: 'Axt.grid.Panel',
	alias		: 'widget.module-inotentry-lister',
	store		: 'module.membership.inout.inotentry.store.InotEntry',
...
//						}
//					},
					{	text : '<span class="write-button">방문처리</span>'	, action : 'okAction'		, cls: 'button1-style',}, '-',
					{	text : '<span class="write-button">방문취소</span>'	, action : 'nokAction'		, cls: 'button1-style',},'-',
					'->', '-' ,
		...
		var me = this,
			item = {
				...
				items : [
					...
					},{ dataIndex: 'need_time'		, width:  80, align : 'right'	, text: Language.get('need_time'	, '소요시간'		), xtype : 'numericcolumn'
	...
	rowDelete: function(){
		...
		if(records.length == 0){
			...
			Ext.Msg.show({ title: '삭제 확인 요청', msg: '선택한 자료를 삭제하시겠습니까?', icon: Ext.Msg.ERROR, buttons: Ext.Msg.YESNO, defaultFocus: 'no', scope: myform,
				fn : function (button) {
					if (button==='yes') {
						Ext.Ajax.request({
							url		: _global.location.http() + '/membership/inotentry/set/inot.do',
							params	: {
		...
		} // Ext.ComponentQuery.query('module-inotentry-editor')[0].down('[name=modify]').setValue('Y');
	}
