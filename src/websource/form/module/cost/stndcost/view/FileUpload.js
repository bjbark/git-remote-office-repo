 ...
 * 파일 업로드 공통 팝업
 */
Ext.define('module.cost.stndcost.view.FileUpload', { extend: 'Axt.popup.Upload',

	alias: 'widget.module-stndcost-upload',
	...
	*/
	...
	finishAction : function (button) {
		var me = this
		form= me.down('[name=stnd_date]').getValue()
		;
		...
		var a = Ext.ComponentQuery.query('#stnd_date')[0].getValue(); //기준일자
		if	(a== null || a==''){
			Ext.Msg.show({ title: '알림', msg: '기준일자는 반드시 입력해야합니다.', icon: Ext.Msg.INFO, buttons: Ext.Msg.YES, defaultFocus: 'yes', scope: me });
			return;
		}
		// submit할 form가져오기
