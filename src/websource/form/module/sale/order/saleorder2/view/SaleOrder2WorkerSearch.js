Ext.define('module.sale.order.saleorder2.view.SaleOrder2WorkerSearch', { extend: 'Axt.form.Search',
	store	: 'module.sale.order.saleorder2.store.SaleOrder2Invoice',
	alias	: 'widget.module-saleorder2-worker-search',
			...
			line = {
				...
				items  : [
					{	xtype	: 'fieldcontainer',
							...
							{	text	: '수량', xtype : 'label',style:"text-align: right" , style : 'text-align:center;'
								...
								listeners: {
									change : function(self, value) {
										var panel = self.up('form')
											inv_amt = Math.floor((Number(value) * Number(panel.down('[name=invc_pric]').getValue()))/10)*10
										;
						...
						width	: 95,
						items	: [
							{	text		: '(협력사)납기일자' , xtype : 'label', style : 'text-align:center;'
							},{	xtype		: 'datefield'  ,
								margin		: '2 2 2 2',
								name		: 'deli_date',
								...
								listeners	: {
									keydown : function(self, e) {
										if (e.keyCode == e.ENTER) {
											var panel = self.up('form');
											panel.down('[name=cstm_offr_date]').focus(true, 10);
										} else if (e.keyCode == e.ESC) {
											me.attachItem({ clear : true });
										}
									}
						...
						width	: 95,
						items	: [
							{	text		: '발주일자' , xtype : 'label', style : 'text-align:center;'
							},{	xtype		: 'datefield'  ,
								margin		: '2 2 2 2',
								name		: 'cstm_offr_date',
								format		: Const.DATE_FORMAT_YMD_BAR,
								...
								listeners	: {
									keydown : function(self, e) {
										if (e.keyCode == e.ENTER) {
											var panel = self.up('form');
											panel.down('[name=cstm_deli_date]').focus(true, 10);
										} else if (e.keyCode == e.ESC) {
											me.attachItem({ clear : true });
										}
									}
								...
								listeners	: {
									keydown : function(self, e) {
										if (e.keyCode == e.ENTER) {
											...
											if ( Ext.isEmpty( self.up('form').down('[name=cstm_deli_date]').getValue()) ){
												self.up('form').down('[name=cstm_deli_date]').focus(true, 10);
											} else {
	...
	attachItem : function (config) {
		...
		if (config.clear || !config.record) {
			...
			me.down('[name=invc_pric]'		).setValue(record.get('invc_pric'));
			me.down('[name=vatx_amnt]'		).setValue(record.get('vatx_amnt'));
			me.down('[name=invc_amnt]'		).setValue(record.get('invc_amnt'));
			me.down('[name=cstm_deli_date]'	).setValue(record.get('cstm_deli_date' ));
			me.down('[name=cstm_offr_date]'	).setValue(record.get('cstm_offr_date' ));
	...
	appendItem : function (config) {
		...
		record = Ext.create( store.model.modelName , {
			...
			cstm_deli_date	: Ext.Date.format(me.down('[name=cstm_deli_date]').getValue(),'Y-m-d'),
			cstm_offr_date	: Ext.Date.format(me.down('[name=cstm_offr_date]').getValue(),'Y-m-d'),
		...
		store.each(function(findrecord) {
			if (   findrecord.get('item_idcd') == record.get('item_idcd')
				...
				&& findrecord.get('cstm_deli_date') == record.get('cstm_deli_date')
				&& findrecord.get('cstm_offr_date') == record.get('cstm_offr_date')
