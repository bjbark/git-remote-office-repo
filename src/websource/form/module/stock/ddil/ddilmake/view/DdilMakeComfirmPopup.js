Ext.define('module.stock.ddil.ddilmake.view.DdilMakeComfirmPopup', { extend: 'Axt.popup.Search',
	alias: 'widget.module-ddilmake-comfirm-popup',
	...
	finishAction: function(){
		var me = this,
			lister2	= Ext.ComponentQuery.query('module-ddilmake-lister2')[0],
			records	= lister2.getStore(),
			values	= Ext.ComponentQuery.query('module-ddilmake-search')[0].getValues(),
		...
		if(invc_date == '' || invc_date == null){
			...
			Ext.Msg.confirm("확인", "재고실사 결과를 수불부에 등록 하시겠습니까?", function(button) {
				if (button == 'yes') {
					Ext.Ajax.request({
						url		: _global.location.http() + '/stock/ddil/ddilmake/set/ddil.do',
						params	: {
