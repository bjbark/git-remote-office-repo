Ext.define('lookup.popup.view.WorkCvicPopup', { extend: 'Axt.popup.Search',
	id		: 'lookup-work-cvic-popup',
	alias	: 'widget.lookup-work-cvic-popup',
	store	: 'lookup.popup.store.WorkCvicPopup',
	closable: true,
	autoShow: true,
	width	: 400,
	height	: 500,
	...
	searchForm: function(){
		var me = this,
			text
		;
		if(me.popup.params.otod_yorn != ''){
			otod = me.popup.params.otod_yorn;
		}
		if(otod == '1') {
			text = '외주거래처의 코드 또는 명칭을 입력하세요....';
		} else if(otod == '0') {
			text = '설비코드 또는 설비명을 입력하세요....';
		}
		var	form = {
			xtype		: 'form-search',
	...
	createGrid: function(){
		...
		grid = {	xtype: 'grid-panel',
				...
				columns:[
					{	dataIndex:	'code'      	, width: 80 , align : 'left'   , text: Language.get(code	, '')
					},{	dataIndex:	'name'      	, width: 150, align : 'left'   , text: Language.get(name	, '')
					}
				],
				listeners: {
	...
	selectAction: function(){
		var me    = this,
			...
			test =  Ext.merge(me.down('form').getValues(), me.popup.params );
			param = Ext.merge({
				hq_id : _global.hq_id ,
				params : test
			});
