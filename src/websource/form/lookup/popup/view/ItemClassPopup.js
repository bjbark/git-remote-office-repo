/**
 */
Ext.define('lookup.popup.view.ItemClassPopup', { extend: 'Axt.popup.Search',
	alias	: 'widget.lookup-item-clss-popup',
	store	: 'lookup.popup.store.ItemClassPopup',

	title	: Language.get('item_clss_popup','품목 분류 찾기'),
	closable: true,
	autoShow: true,
	width	: 700,
	height	: 500,
					...
					{	xtype		: 'grid-panel',
						...
						listeners	: {
							itemclick : function (dv, record, item, index, e) {
								...
								me.down('[name=lcls_name]').setValue(record.get('clss_name'));
								me.down('[name=clss_idcd]').setValue(record.get('clss_idcd')); // A-ONE 품목코드
								me.selectRecord( record);
							},
							itemdblclick: function(dataview, index, item, e) {
								if (me.popup.choice && me.popup.choice == 'free') {
									me.down('[name=lcls_idcd]').setValue(index.get('clss_idcd'));
									me.down('[name=lcls_name]').setValue(index.get('clss_name'));
									me.down('[name=lcls_code]').setValue(index.get('lcls_code')); // A-ONE 품목코드
									me.finishAction();
								}
							},
							selectionchange : function(grid, records ) {
								...
								if(record) {
									me.down('[name=lcls_idcd]').setValue(record.get('clss_idcd'));
									me.down('[name=lcls_code]').setValue(record.get('clss_code')); // A-ONE 품목코드
									me.down('[name=lcls_name]').setValue(record.get('clss_name'));
								}
						...
						listeners	: {
							itemclick : function (dv, record, item, index, e) {
								...
								me.down('[name=mcls_code]').setValue(record.get('clss_code')); // A-ONE 품목코드
								me.down('[name=mcls_name]').setValue(record.get('clss_name'));
								me.selectRecord( record);
							},
							itemdblclick: function(dataview, index, item, e) {
								if (me.popup.choice && me.popup.choice == 'free') {
									me.down('[name=mcls_idcd]').setValue(index.get('clss_idcd'));
									me.down('[name=mcls_idcd]').setValue(index.get('clss_code')); // A-ONE 품목코드
									me.down('[name=mcls_name]').setValue(index.get('clss_name'));
									me.finishAction();
								}
							},
							selectionchange : function(grid, records ) {
...
//								me.down('[name=mcls_idcd]').setValue(record.get('clss_idcd'));
						...
						listeners	: {
							itemclick : function (dv, record, item, index, e) {
								...
								me.down('[name=scls_code]').setValue(record.get('clss_code')); // A-ONE 품목코드
								me.down('[name=scls_name]').setValue(record.get('clss_name'));
								me.selectRecord( record);
							},
							itemdblclick: function(dataview, index, item, e) {
								if ((me.popup.choice && me.popup.choice == 'free') || me.down('#itemclass4').hidden) {
									me.down('[name=scls_idcd]').setValue(index.get('clss_idcd')); // A-ONE 품목코드
									me.down('[name=scls_idcd]').setValue(index.get('clss_code')); // A-ONE 품목코드
									me.down('[name=scls_name]').setValue(index.get('clss_name'));
									me.finishAction();
								}
							},
							selectionchange : function(grid, records ) {
		...
		var me = this,
			tool = {
				...
				items	: [
					...
					{xtype: 'textfield' , name : 'lcls_code' , hidden : true , readOnly : true  }, // A-ONE 품목코드
					{xtype: 'textfield' , name : 'mcls_code' , hidden : true , readOnly : true  }, // A-ONE 품목코드
					{xtype: 'textfield' , name : 'scls_code' , hidden : true , readOnly : true  }, // A-ONE 품목코드
					{xtype: 'textfield' , name : 'lcls_name' , hidden : true , readOnly : true  }, // A-ONE 품목코드
					{xtype: 'textfield' , name : 'mcls_name' , hidden : true , readOnly : true  }, // A-ONE 품목코드
					{xtype: 'textfield' , name : 'scls_name' , hidden : true , readOnly : true  }, // A-ONE 품목코드
	...
	finishAction: function(){
		...
		if ((me.popup.choice && me.popup.choice == 'free') || panel.hidden) {
			if (me.choiceRecord) {
				...
				me.choiceRecord.set('lcls_code', base.down('[name=lcls_code]').getValue()); // A-ONE 품목코드
				me.choiceRecord.set('mcls_code', base.down('[name=mcls_code]').getValue()); // A-ONE 품목코드
				me.choiceRecord.set('scls_code', base.down('[name=scls_code]').getValue()); // A-ONE 품목코드
				me.choiceRecord.set('lcls_name', base.down('[name=lcls_name]').getValue()); // A-ONE 품목코드
				me.choiceRecord.set('mcls_name', base.down('[name=mcls_name]').getValue()); // A-ONE 품목코드
				me.choiceRecord.set('scls_name', base.down('[name=scls_name]').getValue()); // A-ONE 품목코드
			...
			if (selected.length === 0) {
				...
				Ext.each(selected , function(record) {
					...
					record.set('lcls_name', base.down('[name=lcls_name]').getValue());
					record.set('mcls_name', base.down('[name=mcls_name]').getValue());
					record.set('scls_name', base.down('[name=scls_name]').getValue());
				});
