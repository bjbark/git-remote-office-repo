/**
 */
Ext.define('lookup.popup.view.ItemPopup', { extend: 'Axt.popup.Search',
...
//
				...
				columns: [
					{	xtype: 'rownumberer'		, width:  50, text: '순번', align : 'center', hidden : _global.hq_id.toUpperCase() == 'N1000NBOLT'? false : true,
						...
						hidden : _global.hq_id.toUpperCase() == 'N1000NBOLT'? true : false
						...
						width : _global.hq_id.toUpperCase() == 'N1000NBOLT'? 75 : 95
						...
						width: _global.hq_id.toUpperCase() == 'N1000NBOLT'? 145 : 180
						...
						hidden : (_global.hq_id.toUpperCase() == 'N1000NBOLT' || _global.options.mes_system_type =='SJFLV') ? false : true
					},{	dataIndex:	'cont_pric'	, align : 'right'  , text: Language.get( 'cont_pric'      , '계약단가'	), xtype : 'numericcolumn', format	: '#,##0',
						renderer: function(value, meta){
							if(me.popup.params.tema){
...
//							return val;
						...
						width : _global.hq_id.toUpperCase() == 'N1000NBOLT'? 75 : 85,
						hidden: _global.options.mes_system_type =='Frame'?true:false
					},{	dataIndex:	'mtrl_bacd_name'	, width: 100, align : 'left'   , text: Language.get( 'mtrl_bacd_name'      , '재질'	), hidden : _global.hq_id.toUpperCase() == 'N1000HJSYS'? false : true,
							renderer: function(value, meta){
								if(me.popup.params.tema){
						...
						hidden: (_global.options.mes_system_type =='Frame' || _global.options.mes_system_type =='SJFLV'  || _global.hq_id.toUpperCase() == 'N1000NBOLT')?true:false
						...
						hidden : (_global.hq_id.toUpperCase() == 'N1000NBOLT' || _global.options.mes_system_type =='SJFLV') ? true : false
						...
						hidden : _global.hq_id.toUpperCase() == 'N1000NBOLT'? true : false
					},{	dataIndex:	'item_clss_bacd_name'	, width: 95, align : 'left'  , text: Language.get( 'item_clss_bacd_name'      , '품목군'	),
						renderer: function(value, meta){
//							debugger;
							if(me.popup.params.tema){
						...
						hidden : _global.hq_id.toUpperCase() == 'N1000NBOLT'? false : true
					...
					},{	dataIndex:	'stok_qntt'	, width:  65, align : 'right'  , text: Language.get( ''      , '현재고량'	),
						renderer: function(value, meta){
							if(me.popup.params.tema){
								this.columns[18].width = 120;
								meta.style = 'font-size:2em !important;height:30px;line-height:30px;width:120px;'; // applied style for DIV element
							}
							var val = Ext.util.Format.number(value, _global.options.mes_system_type == 'SJFLV' ? "0,000.000" : "0,000");
							return val;
						},
//						hidden : _global.hq_id.toUpperCase() != 'N1000SJFLV'? true : false
					},{	dataIndex:	'item_wigt'	, width:  65, align : 'right'  , text: Language.get( ''      , '중량'	),hidden : _global.hq_id.toUpperCase() != 'N1000NBOLT'? true : false,
						renderer: function(value, meta){
							if(me.popup.params.tema){
								this.columns[18].width = 120;
								meta.style = 'font-size:2em !important;height:30px;line-height:30px;width:120px;'; // applied style for DIV element
						...
						hidden : _global.hq_id.toUpperCase() != 'N1000NBOLT'? true : false
					},{	dataIndex:	'cstm_pric_name'	, width:  100, align : 'left'  , text: Language.get( ''      , '비고'	),
						hidden : _global.hq_id.toUpperCase() != 'N1000SJUNG'? true : false
					},{	dataIndex:	'stor_wrhs_idcd'	, width:  100, align : 'left'  , text: Language.get( ''      , '창고ID'),
						hidden : true
					},{	dataIndex:	'stor_wrhs_name'	, width:  100, align : 'left'  , text: Language.get( ''      , '창고'	),
						hidden : true
					},{	dataIndex:	'vatx_incl_yorn '	, width:  100, align : 'left'  , text: Language.get( ''      , '부가세포함여부'),
						hidden : true
					}
				],
				listeners: {
			...
			grid = {
				...
				columns: [
					{	dataIndex:	'line_stat'	, width:  50, align : 'center' , text: Language.get( 'line_stat'      , '상태'	) , xtype: 'lookupcolumn' , lookupValue: resource.lookup( 'line_stat' ),
						...
						hidden : _global.hq_id.toUpperCase() == 'N1000NBOLT'? false : true
					},{	dataIndex:	'unit_name'	, width: 110, align : 'left'   , text: Language.get( 'unit_name'      , '단위'),
						renderer: function(value, meta){
							if(me.popup.params.tema){
								this.columns[6].width = 260;
								meta.style = 'font-size:2em !important;height:30px;line-height:30px'; // applied style for DIV element
							}
							return value;
						},
						...
						hidden : _global.hq_id.toUpperCase() != 'N1000SJFLV'? true : false
					},{	dataIndex:	'item_wigt'	, width: 80, align : 'right'   , text: Language.get( 'item_wigt'      , '중량'),
						renderer: function(value, meta){
							if(me.popup.params.tema){
								this.columns[5].width = 260;
								meta.style = 'font-size:2em !important;height:30px;line-height:30px'; // applied style for DIV element
						...
						hidden : _global.hq_id.toUpperCase() != 'N1000NBOLT'? true : false
					},{	dataIndex:	'vatx_incl_yorn '	, width:  100, align : 'left'  , text: Language.get( 'vatx_incl_yorn' , '부가세포함여부'),
						hidden : true
					}
...
//					}
	...
	insert:function(){
		var	me = this,
			owner	= Ext.ComponentQuery.query('lookup-item-popup')[0],
			...
			if(_global.hq_id.toUpperCase() == "N1000NBOLT"){
				...
				for (var i = 0; i < select.length; i++) {
					record = Ext.create( store.model.modelName , {
						...
						stor_wrhs_idcd	: select[i].get('stor_wrhs_idcd'),
						stor_wrhs_name	: select[i].get('stor_wrhs_name'),
					});
					store.add(record);
				}

			}else if(_global.hq_id.toUpperCase() == "N1000SJUNG"){
				var maxseqn = 0;
				...
				// 선택된 항목들 각각을 새로운 레코드로 추가
				for (var i = 0; i < select.length; i++) {
					var record = Ext.create(store.model.modelName, {
						line_seqn		: maxseqn++,  // seqn을 증가시키며 설정
						item_idcd		: select[i].get('item_idcd'),
						item_code		: select[i].get('item_code'),
						item_name		: select[i].get('item_name'),
						item_spec		: select[i].get('item_spec'),
						...
						unit_name		: select[i].get('unit_name'),
						acct_bacd_name	: select[i].get('acct_bacd_name'),
						invc_qntt		: 1,  // 기본 수량 설정
						cont_pric		: select[i].get('cont_pric'),
						stok_qntt		: select[i].get('stok_qntt'),
						item_wigt		: select[i].get('item_wigt'),
						stok_qntt		: select[i].get('stok_qntt'),
						item_wigt		: select[i].get('item_wigt'),
						vatx_incl_yorn	: select[i].get('vatx_incl_yorn'),
					});
					store.add(record);  // Grid2에 추가
				}
