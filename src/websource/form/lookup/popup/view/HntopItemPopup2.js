/**
 */
Ext.define('lookup.popup.view.HntopItemPopup2', { extend: 'Axt.popup.Search',

	alias	: 'widget.lookup-hntop-item-popup2',
...
//		}
	...
	createGrid: function(){
			...
			grid = {
				...
				columns: [
					...
					},{	dataIndex:	'acct_bacd_name'	, width:  65, align : 'center' , text: Language.get( 'acct_bacd_name'      , '계정구분'),
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					},{	dataIndex:	'item_spec'	, width: 70, align : 'left'   , text: Language.get( 'item_spec'      , '품목규격'),
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					},{	dataIndex:	'offr_pric'	, width: 75, align : 'right'  , text: Language.get( 'offr_pric'      , '계약단가'	),
						renderer: function(value, meta){
							if(me.popup.params.tema){
	...
	insert:function(){
		var	me = this,
			owner	= Ext.ComponentQuery.query('lookup-hntop-item-popup2')[0],
		...
		for (var i = 0; i < select.length; i++) {
				...
				invc_qntt		: 1,
				offr_pric		: select[i].get('offr_pric')
				});
				store.add(record);
			}
		}
