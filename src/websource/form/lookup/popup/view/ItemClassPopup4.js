Ext.define('lookup.popup.view.ItemClassPopup4', { extend: 'Axt.popup.Search',
	alias	: 'widget.lookup-item-clss-popup4',
						...
						listeners	: {
							...
							itemdblclick: function(dataview, index, item, e) {
								if ((me.popup.choice && me.popup.choice == 'free') || me.down('#itemclass4').hidden) {
									me.down('[name=scls_idcd]').setValue(record.get('clss_idcd')); // A-ONE 품목코드
									me.down('[name=scls_code]').setValue(record.get('clss_code')); // A-ONE 품목코드
									me.down('[name=scls_name]').setValue(record.get('clss_name')); // A-ONE 품목코드
									me.finishAction();
								}
