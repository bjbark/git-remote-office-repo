/**
 */
Ext.define('lookup.popup.view.ItemPopupHjsys', { extend: 'Axt.popup.Search',

	alias	: 'widget.lookup-item-popup-Hjsys',
	store	: 'lookup.popup.store.ItemPopupHjsys',
	requires: [
	 ...
	 createGrid: function(){
			...
			grid = {
				...
				columns: [
					...
					},{	dataIndex:	'cont_pric'	, width: 70, align : 'right'  , text: Language.get( 'cont_pric'      , '계약단가'	),
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					},{	dataIndex:	'mtrl_bacd_name'	, width: 75, align : 'left'   , text: Language.get( 'mtrl_bacd_name'      , '재질'	), hidden : _global.hq_id.toUpperCase() == 'N1000HJSYS'? false : true,
							renderer: function(value, meta){
								if(me.popup.params.tema){
