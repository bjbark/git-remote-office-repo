/**
 */
Ext.define('lookup.popup.view.UserPopup', { extend: 'Axt.popup.Search',
	id   : 'lookup-user-popup',
	alias: 'widget.lookup-user-popup',
	store: 'lookup.popup.store.UserPopup',
	title: Language.get( 'usr_mst' , '담당자 찾기') ,
	closable: true,
	autoShow: true,
	width: 700,
	height: 500,
								...
								{	xtype	: 'fieldset',
									...
									items	: [
										{	xtype	: 'label',
											...
											flex	: 1,
											emptyText	: '담당자코드 또는 담당자명을 입력하세요....',
	...
	createGrid: function(tema){
		var	me		= this,
			grid	= {
				...
				selModel	: {	selType: 'checkboxmodel', mode : (me.popup.params.select == 'MULTI')?'MULTI':'SINGLE'  },
				...
				columns		:[
					{	text	: Language.get('drtr_name'    , '담당자명')	, dataIndex: 'user_name'	, width: 100,
					...
					{	text		: Language.get('user_code'    , '사원번호')	, dataIndex: 'user_code'	, width: 120,
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					{	text	: Language.get('dept_name'    , '부서'  )	, dataIndex: 'dept_name'	, width: 120 ,
						renderer: function(value, meta){
							if(me.popup.params.tema){
					...
					},{	text	: Language.get('dept_idcd'    , '부서ID'  )	, dataIndex: 'dept_name'	, hidden : true
					},
	//				{	text: Language.get('saup_area' , '사업장' )   , dataIndex: 'stor_nm' ,   flex  : 1  },
	//				{	text: Language.get('rank'      , '직급'   )   , dataIndex: 'jobcl_nm'   ,   width: 120  }
				],
				listeners	: {
