 ...
 * 공통상품수신 팝업
 */
Ext.define('system.popup.itembonsa.ItemBonsaPopup', { extend: 'Axt.popup.Search',

	alias   : 'widget.system-itembonsa-popup',
	id      : 'system-itembonsa-popup',
	requires: [
	...
	title   : Language.get('comm_itm_popup','공통 품목 정보'),
	closable: true,
	autoShow: true,
	width   : 700,
	height  : 508,
    ...
    initComponent: function(config){
        ...
        if ('BonsaChange' === me.popup.values.gubun){
        	...
        	me.title = Language.get('stor_itm_popup','지점 품목 정보') ;
        }

   		if (me.popup.values && me.popup.values.barcode && me.popup.values.barcode != '') {
   			me.selectAction();
		}
    },
    /**
     ...
     createGrid: function(){
        var  me = this,
        	grid = {
        		...
        		store   : Ext.create('system.popup.itembonsa.store.ItemBonsaPopup'),
        		paging  : {
        			 xtype: 'grid-paging',
        			 items: [
        		 		'->',
        		 ...
        		 columns: [
					 ...
					 },{ dataIndex: 'unt_qty'		, width:  50, text: Language.get('unt_qty',      'In Qty' ), align : 'right', xtype : 'numericcolumn'
    ...
    selectAction: function(){
        var  me    = this,
        	...
        	}, me.popup.params  ); // , { barcode : me.popup.values.barcode }
        ;
    	if ('BonsaChange' === me.popup.values.gubun){ /* 공통상품으로 전환 */
        	...
        	store.getProxy().api.read = _global.api_host_info + "/system/item/itemstore/get/dialogbonsa.do" ;
    	}
		store.load({
        ...
        if (selects.length <= 0) {
        	...
        	if ('BonsaChange' === me.popup.values.gubun){ /* 공통상품으로 전환 */
        		...
        		Ext.each( selects , function( record  ){
        			console.debug('bonsa record', record );
        			record.set('stor_grp', record.get('stor_grp') );
        			record.set('stor_grp', record.get('stor_grp') );
        			record.set('stor_id', record.get('owner_id') );
        			record.set('share_yn', '1' 					  );
        			record.set('upt_id', _global.login_pk );

        			record.dirty = true ;

        		});
        		bonsastore.getProxy().api.update = _global.api_host_info + "/system/item/itemrecv/set/bonsachange.do" ;
        	} else {
