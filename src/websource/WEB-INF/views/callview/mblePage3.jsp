...
<html>
	...
	var tag;
		...
		function search() {
			...
			var url;
			if(tag == '반품처리' || tag==null){
				url = api_url+"/system/mobile/prodretn/get/search.do";
			}else if(tag == '반품내역'){
				url = api_url+"/system/mobile/prodretn/get/search2.do";
			}
				 $.ajax({
					url: url,
					data:{param:jsonData},
						...
						if(data != null){
							if(tag == '반품처리' || tag==null){
								...
								for(var i= 0 ; i < data['records']['length'] ;i++){
									$('#ajaxTable1').append('<tr><td><input type="button" value="반품" class="insert" onclick="#"></td><td class="centers">'
											+chk(data['records'][i]['acpt_numb'])+'</td><td class="centers">'
											+chk(dateFmt(data['records'][i]['ostt_date']))+'</td><td class="centers">'
											+chk(data['records'][i]['acpt_numb'])+'</td><td>'
											+chk(data['records'][i]['cstm_name'])+'</td><td class="centers">'
											+chk(data['records'][i]['item_code'])+'</td><td>'
											+chk(data['records'][i]['item_name'])+'</td><td class="centers">'
											+chk(data['records'][i]['mold_spec'])+'</td><td class="numberFmt">'
											+numberFmt(chk(data['records'][i]['qntt']))+'</td><td class="numberFmt">'
											+numberFmt(chk(data['records'][i]['ostt_qntt']))+'</td><td>-</td></tr>');
								}
							}else if(tag == '반품내역'){
								...
								for(var i= 0 ; i < data['records']['length'] ;i++){
									$('#ajaxTable2').append('<tr><td class ="centers">'+dateFmt(chk(data['records'][i]['ostt_date']))+'</td><td class="centers">'
											+chk(data['records'][i]['acpt_numb'])+'</td><td class="centers">'
											+dateFmt(chk(data['records'][i]['acpt_date']))+'</td><td>'
											+chk(data['records'][i]['cstm_name'])+'</td><td class="centers">'
											+chk(data['records'][i]['item_code'])+'</td><td>'
											+chk(data['records'][i]['item_name'])+'</td><td class="centers">'
											+chk(data['records'][i]['mold_spec'])+'</td><td class="centers">'
											+chk(dateFmt(data['records'][i]['deli_date']))+'</td><td class="numberFmt">'
											+numberFmt(chk(data['records'][i]['qntt']))+'</td><td class="numberFmt">'
											+numberFmt(chk(data['records'][i]['ostt_qntt']))+'</td><td>-</td></tr>');
								}
							}
	...
	<form action="" method="get" >
		<div id="first" >
			<table style="margin-left: 2%">
					...
					<tr>
						<th>
							<span id="change">출고일자</span>
						</th>
						<td>
							<input type="text" id="fr_dt" class="Datepicker dt" name="fr_dt" ><span style="margin-left:1%;" >~</span>
							<input type="text" id="to_dt" class="Datepicker dt" name="to_dt" >
						</td>
					</tr>
					<tr>
					<tr>
						...
						<td>
							<input type="text" id="item_name" name="item_name" value="${item_name }" style="width: 60%; float:left;">
			...
			<ul class="nav nav-tabs">
				<li class="nav-item">
					<a class="nav-link active" data-toggle="tab" href="#release">반품처리</a>
				</li>
				<li class="nav-item">
					<a class="nav-link" data-toggle="tab" href="#releaselist">반품내역</a>
				</li>
			</ul>
		<div class="tab-content"  style="margin-top:0px;">
			<div id="release" class="tab tab-pane fade table-responsive active in">
				<table id = "ajaxTable1" class="table table-bordered table-responsive" >
					<tr>
						<th style="width:80px;">
							반품처리
						</th>
			...
			<div id="releaselist" class="tab tab-pane fade table-responsive " class="container">
				<table id = "ajaxTable2" class="table table-bordered table-responsive">
					<tr>
						<th style="width:80px;">
							출고일자
						</th>
