...
$( document ).ready(function(){
	...
	$.ajax({
		url: api_host+"/system/custom/sjflv/prod/prodplanlist/get/events.do",
		data:{param:jsonData},
		type:"post",
		dataType : 'json',
		async	 : false,
		...
		eventContent: function(info) {
			...
			element.innerHTML =
				'<div style="overflow-x: auto;">' +  // 가로 스크롤 추가
				  '<table style="border-collapse: collapse; width: 300px; table-layout: fixed; background-color: #f9f9f9; color: #333; border: 1px solid #ccc;">' + // 고정 너비 설정
				  '  <tr>' +
				  '    <td style="width: 90px; border: 1px solid #ccc; padding: 4px 8px; font-size: 12px; background-color: #e6e6e6; text-align: right;"><b>제물코드:</b></td>' +
				  '    <td style="border: 1px solid #ccc; padding: 4px 8px; font-size: 12px;">' + titleObj.item_code + '</td>' +
				  '  </tr>' +
				  '  <tr>' +
				  '    <td style="width: 100px; border: 1px solid #ccc; padding: 4px 8px; font-size: 12px; background-color: #e6e6e6; text-align: right;"><b>품명:</b></td>' +
				  '    <td style="border: 1px solid #ccc; padding: 4px 8px; font-size: 12px;">' + titleObj.item_name + '</td>' +
				  '  </tr>' +
				  '  <tr>' +
				  '    <td style="width: 100px; border: 1px solid #ccc; padding: 4px 8px; font-size: 12px; background-color: #e6e6e6; text-align: right;"><b>규격:</b></td>' +
				  '    <td style="border: 1px solid #ccc; padding: 4px 8px; font-size: 12px;">' + (titleObj.item_spec ? titleObj.item_spec : '') + '</td>' +
				  '  </tr>' +
				  '  <tr>' +
				  '    <td style="width: 100px; border: 1px solid #ccc; padding: 4px 8px; font-size: 12px; background-color: #e6e6e6; text-align: right;"><b>생산계획량:</b></td>' +
				  '    <td style="border: 1px solid #ccc; padding: 4px 8px; font-size: 12px;">' + titleObj.plan_qntt + '</td>' +
				  '  </tr>' +
				  '  <tr>' +
				  '    <td style="width: 100px; border: 1px solid #ccc; padding: 4px 8px; font-size: 12px; background-color: #e6e6e6; text-align: right;"><b>납기일자:</b></td>' +
				  '    <td style="border: 1px solid #ccc; padding: 4px 8px; font-size: 12px;">' + (titleObj.deli_date ? formatDate(titleObj.deli_date) : '') + '</td>' +
				  '  </tr>' +
				  '</table>' +
				'</div>';
	...
	function fireEvents(api_host,hq_id) {
		var	month = $('.fc-dayGridMonth-view').length,
			...
			stdt  = '',
			eddt  = '',
			url   = "/system/custom/sjflv/prod/prodplanlist/get/events.do",
			dvcd  = ''
		;
		if(month || list){
		...
		$.ajax({
			...
			success:function(data){
				if(data.success){
					for(var i= 0 ; i < data['records']['length'] ;i++){
						var eventData = data['records'][i];
						/* if (eventData.cofm_date.length === 8) {
							var year = eventData.cofm_date.substring(0, 4);
							var month = eventData.cofm_date.substring(4, 6);
							var day = eventData.cofm_date.substring(6, 8);
							eventData.cofm_date = year + '-' + month + '-' + day;
						} */
						var stdt = formatTimestamp(eventData.plan_sttm); 
						var eddt = formatTimestamp(eventData.plan_edtm);
