...
//			.query("	,	t2.tax_free , t2.taxation , t2.tax     , t2.amount				 						")
...
//			.where("       			,	sum(t1.tax_free) as tax_free , sum(t1.taxation) as taxation 		")
			...
			.where("       			,	sum(0) as payable  , sum(0)  as charge 			")
			.where("   			from	sale_mst t1 														")
            .where("					left outer join usr_mst t3 on t1.salesman_id = t3.emp_id 		")
			...
			.where("          	and     t1.salesman_id  = :inv_usr_id  " , arg.getParameter("inv_usr_id"  )) /* 영업담당 */
