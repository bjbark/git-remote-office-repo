...
//	private final Logger logg0er = LoggerFactory.getLogger(this.getClass());
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.query("                                                                                                                  ")
			.query("      ,t1.sum_qty                                                                                                 ")
			.query("      ,decode(sum(t1.sum_qty) over (partition by t1.stor_grp), 0, 0                                               ")
			.query("        ,round(abs(t1.sum_qty)/sum(abs(t1.sum_qty)) over (partition by t1.stor_grp), 3)*100)         as rate_qty  ")
			.query("                                                                                                                  ")
			.query("      ,t1.sum_amount                                                                                              ")
			.query("      ,decode(sum(t1.sum_amount) over (partition by t1.stor_grp), 0, 0                                            ")
			.query("        ,round(abs(t1.sum_amount)/sum(abs(t1.sum_amount)) over (partition by t1.stor_grp), 3)*100)   as rate_amount ")
		;
		data.param
			.where("  from (                                                                                                          ")
			...
			.where("              join order_item b on a.stor_id = b.stor_id                                                        ")
