...
//		String sts_gb = arg.getParamText("sts_gb" );
		...
		;
		data.param // 쿼리문  입력
			.query("select 	t1.item_cla1_id as cla1_id  																						")
			.query("   	,	(select class_nm from item_class					 																")
			.query("   		  where class_id = t1.item_cla1_id and row_lvl = 1) as cla1_nm 													")
			.query("   	,	t1.item_cla2_id as cla2_id 																							")
			.query("    ,	(select class_nm from item_class 																					")
			.query("          where class_id = t1.item_cla1_id||t1.item_cla2_id and row_lvl = 2) as cla2_nm 									")
			.query("    ,	t1.item_cla3_id as cla3_id 																							")
			.query("    ,	(select class_nm from item_class 																					")
			.query("          where class_id = t1.item_cla1_id||t1.item_cla2_id||t1.item_cla3_id and row_lvl = 3 ) as cla3_nm 				")
			.query("    ,	t1.item_cla4_id as cla4_id 																							")
			.query("    ,	(select class_nm from item_class 																					")
			.query("          where class_id = t1.item_cla1_id||t1.item_cla2_id||t1.item_cla3_id||t1.item_cla4_id and row_lvl = 4 ) as cla4_nm ")
...
//			.where("       	   item_class h3,  																									")
			...
			.where("               ,	t2.qty  																								")
			.where("             from 	sale_mst t1, sale_dtl t2, itm_mst t5   																")
...
//			.where("            		              and :to_dt     		 " 		, to_dt , "2".equals( inv_po_term 							))  // 주문종료일자 
            ...
            .where("     		  and   t5.class_id in ( select  class_id  from item_class a start with class_id = :class_id  connect by prior class_id = prnt_id )" , arg.getParameter("class_id"  ))			
...
//			.where("              and   t1.inv_work_id in ('1', '2', '3', '4')                                                                  ")
			.where("              and   t1.inv_no      = t2.inv_no  																			")
    		.where("			  and   t2.item_idcd    = t5.item_idcd											                                    ")
			.where("		) h1  																												")
...
//
		...
		;
		data.param // 쿼리문  입력
	    	.query("	select 	t1.item_cla1_id as clss_1 																				")
	    	.query("	    ,	(select class_nm from item_class 																		")
	    	.query("	          where class_id = t1.item_cla1_id and row_lvl = 1) as cla1_nm 										")
	    	.query("	    ,	t1.item_cla2_id as cla2_id 																				")
	    	.query("	    ,	(select class_nm from item_class 																		")
...
//			.where("  and h2.class_id = h3.class_id(+)  																						")
	...
	public SqlResultMap getSearchWeek(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("    	,  substr(h2.class_id,  5, 2) as item_cla3_id  																		")
			.where("    	,  substr(h2.class_id,  7, 2) as item_cla4_id  																		")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '1', h1.qty, 0)) as qty1				") /* 일요일 */
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '1', h1.amount, 0)) as week1			")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '2', h1.qty, 0)) as qty2				")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '2', h1.amount, 0)) as week2			")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '3', h1.qty, 0)) as qty3				")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '3', h1.amount, 0)) as week3			")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '4', h1.qty, 0)) as qty4				")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '4', h1.amount, 0)) as week4			")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '5', h1.qty, 0)) as qty5				")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '5', h1.amount, 0)) as week5			")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '6', h1.qty, 0)) as qty6				")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '6', h1.amount, 0)) as week6			")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '7', h1.qty, 0)) as qty7				")
			.where("			 ,	sum(decode(to_char(to_date(h1.inv_dt, 'yyyymmdd'), 'd'), '7', h1.amount, 0)) as week7			")
			.where("             ,	sum(h1.amount  ) as amount																		")
			.where("             ,	sum(h1.qty     ) as qty																			")
			.where("  	  from itm_mst h2,  																									")
//			.where("       	   item_class h3,  																									")
			.where("       	(  																													")
			.where("			select 	t1.stor_grp 	, t2.item_idcd  	, t1.inv_dt															")
			.where("               ,	t2.tax_free		, t2.taxation  																			")
			.where("               ,	t2.tax  	    , t2.amount  																			")
			.where("               ,	t2.qty  																								")
			.where("             from 	sale_mst t1, sale_dtl t2, itm_mst t5   																")
...
//			.where("  and h2.class_id = h3.class_id(+)  																						")
	...
	public SqlResultMap getSearchDay(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("    	,  substr(h2.class_id,  5, 2) as item_cla3_id  																		")
			.where("    	,  substr(h2.class_id,  7, 2) as item_cla4_id  																		")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '01', h1.qty, 0)) as qty1 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '01', h1.amount, 0)) as date1 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '02', h1.qty, 0)) as qty2 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '02', h1.amount, 0)) as date2 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '03', h1.qty, 0)) as qty3 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '03', h1.amount, 0)) as date3 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '04', h1.qty, 0)) as qty4 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '04', h1.amount, 0)) as date4 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '05', h1.qty, 0)) as qty5 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '05', h1.amount, 0)) as date5 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '06', h1.qty, 0)) as qty6 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '06', h1.amount, 0)) as date6 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '07', h1.qty, 0)) as qty7 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '07', h1.amount, 0)) as date7 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '08', h1.qty, 0)) as qty8 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '08', h1.amount, 0)) as date8 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '09', h1.qty, 0)) as qty9 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '09', h1.amount, 0)) as date9 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '10', h1.qty, 0)) as qty10 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '10', h1.amount, 0)) as date10 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '11', h1.qty, 0)) as qty11 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '11', h1.amount, 0)) as date11 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '12', h1.qty, 0)) as qty12 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '12', h1.amount, 0)) as date12 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '13', h1.qty, 0)) as qty13 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '13', h1.amount, 0)) as date13 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '14', h1.qty, 0)) as qty14 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '14', h1.amount, 0)) as date14 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '15', h1.qty, 0)) as qty15 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '15', h1.amount, 0)) as date15 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '16', h1.qty, 0)) as qty16 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '16', h1.amount, 0)) as date16 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '17', h1.qty, 0)) as qty17 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '17', h1.amount, 0)) as date17 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '18', h1.qty, 0)) as qty18 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '18', h1.amount, 0)) as date18 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '19', h1.qty, 0)) as qty19 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '19', h1.amount, 0)) as date19 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '20', h1.qty, 0)) as qty20 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '20', h1.amount, 0)) as date20 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '21', h1.qty, 0)) as qty21 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '21', h1.amount, 0)) as date21 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '22', h1.qty, 0)) as qty22 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '22', h1.amount, 0)) as date22 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '23', h1.qty, 0)) as qty23 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '23', h1.amount, 0)) as date23 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '24', h1.qty, 0)) as qty24 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '24', h1.amount, 0)) as date24 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '25', h1.qty, 0)) as qty25 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '25', h1.amount, 0)) as date25 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '26', h1.qty, 0)) as qty26 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '26', h1.amount, 0)) as date26 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '27', h1.qty, 0)) as qty27 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '27', h1.amount, 0)) as date27 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '28', h1.qty, 0)) as qty28 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '28', h1.amount, 0)) as date28 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '29', h1.qty, 0)) as qty29 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '29', h1.amount, 0)) as date29 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '30', h1.qty, 0)) as qty30 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '30', h1.amount, 0)) as date30 								")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '31', h1.qty, 0)) as qty31 									")
			.where(" 		 	,	sum(decode(substr(h1.inv_dt, 7, 2), '31', h1.amount, 0)) as date31 								")
			.where(" 		               ,sum(h1.amount) as amount 																")
			.where(" 		               ,sum(h1.qty      ) as qty 																")
			.where("  	  from itm_mst h2,  																									")
//			.where("       	   item_class h3,  																									")
...
//			.where("  and h2.class_id = h3.class_id(+)  																						")
	...
	public SqlResultMap getSearchMonth(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '01', h1.qty, 0)) as qty1 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '01', h1.amount, 0)) as month1 							")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '02', h1.qty, 0)) as qty2 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '02', h1.amount, 0)) as month2 							")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '03', h1.qty, 0)) as qty3 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '03', h1.amount, 0)) as month3 							")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '04', h1.qty, 0)) as qty4 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '04', h1.amount, 0)) as month4 							")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '05', h1.qty, 0)) as qty5 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '05', h1.amount, 0)) as month5 							")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '06', h1.qty, 0)) as qty6 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '06', h1.amount, 0)) as month6 							")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '07', h1.qty, 0)) as qty7 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '07', h1.amount, 0)) as month7 							")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '08', h1.qty, 0)) as qty8 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '08', h1.amount, 0)) as month8 							")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '09', h1.qty, 0)) as qty9 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '09', h1.amount, 0)) as month9 							")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '10', h1.qty, 0)) as qty10 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '10', h1.amount, 0)) as month10 						")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '11', h1.qty, 0)) as qty11 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '11', h1.amount, 0)) as month11 						")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '12', h1.qty, 0)) as qty12 								")
			.where(" 				,	sum(decode(substr(h1.inv_dt, 5, 2), '12', h1.amount, 0)) as month12 						")
			.where("  	  from itm_mst h2,  																									")
//			.where("       	   item_class h3,  																									")
			.where("       	(  																													")
			.where("			select 	t1.stor_grp 	, t2.item_idcd  	, t1.inv_dt															")
