...
//					.insert("ship_qty"   , row.getParameter("ship_qty"  ))
	        	...
	        	data.param
					...
					.update("eaqty"      , new SqlParamText(String.valueOf(Integer.parseInt(unt_qty) * Integer.parseInt(qty) * -1))) // 이동출고는 마이너스
					.update("qty"        , row.getParameter("qty"        ))
					.update("price"      , row.getParameter("price"      ))
					.update("amount"     , row.getParameter("amount"     ))
	...
	public SqlResultMap setDeleted(HttpRequestArgument arg) throws Exception {
		...
		temp.param
		 	.query("select sts_cd, ship_qty from move_ost_mst where inv_no = :inv_no", arg.fixParameter("inv_no"))
		;
		...
		double work_qty   = Double.parseDouble(arg.getParamText("ship_qty"));	// 로컬 수량
		double server_qty = Double.parseDouble(del.getParamText("ship_qty"));	// 서버 수량
	...
	public SqlResultMap getProduct(HttpRequestArgument arg) throws Exception {
		...
		.query("     ,  a.po_pri_type, a.po_pri_rt 														 	")
		.query("     ,  b.notax_yn                                      										  	")
	...
	public SqlResultMap getPlanDetail(HttpRequestArgument arg) throws Exception {
		...
		data.param
			...
			.query("       left outer join item_unit u  on u.unit_idcd = b.unit_idcd   ")
