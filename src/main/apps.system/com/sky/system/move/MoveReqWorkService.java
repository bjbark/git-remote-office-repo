...
public class MoveReqWorkService  extends DefaultServiceHandler {
		...
		for (SqlResultRow row:map) {
			...
			if (rowaction == Action.delete) {
	        	...
	        	data.param
					...
					.insert("inv_tm"      , new SimpleDateFormat("HHmmss").format(new Date()) )
	...
	public SqlResultMap setDeleted(HttpRequestArgument arg) throws Exception {
		...
		temp.param
		 	.query("select sts_cd, ship_qty from move_req_mst where inv_no = :inv_no", arg.fixParameter("inv_no"))
		;
...
//				.query("	,   substr((a.item_name||'/'||a.item_spec), 1, 63)      as item_name 			") /* 품목 + 규격 */
//				.query("	,   substr(a.item_name, 1, 63)      as item_name 								") /* 품목 + 규격 */
