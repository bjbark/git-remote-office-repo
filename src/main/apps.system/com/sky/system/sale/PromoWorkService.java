...
@Service
public class PromoWorkService  extends DefaultServiceHandler {
		...
		for(SqlResultRow row:map) {
			...
			if (rowaction == Action.delete) {
	        	...
	        	data.param
					...
					.update("recv_price" , row.getParameter("recv_price" ))
...
//			.where("         on c.emp_id = a.emp_id     ")
			.where(" where a.hq_id = :hq_id         ", arg.fixParameter("hq_id"))
			.where("   and :fr_dt  >= a.fr_dt        	  ", arg.fixParameter("fr_dt"))
			.where("   and :to_dt  >= a.to_dt        	  ", arg.fixParameter("to_dt"))
//			.where("   and a.to_dt <= :to_dt       		  ", arg.fixParameter("to_dt"))
//			.where("   and a.dept_id = :dept_id           ", arg.getParameter("dept_id"))
//			.where("   and a.emp_id = :emp_id           ", arg.getParameter("emp_id"))
			.where("   and a.prom_nm like %:prom_nm%    ", arg.getParameter("prom_nm"))
			.where("   and a.row_clos = :row_clos       ", arg.getParameter("row_clos"))
			.where("   and a.row_sts = 0 				  ")
			.where(" order by a.prom_id desc              ")
		;
