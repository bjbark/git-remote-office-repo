...
@Service
public class OnlineOrderListService  extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		String[] inv_work_id = arg.getParamCast("inv_work_id", String[].class);
			...
			data.param // 쿼리문  입력
				...
				.query("   ,   (select biz_tel_no from stor where stor_id = a.wrhs_id ) as wareh_tel_no 	")
				...
				.query("   ,   (select trim(substr(max(hdli_no), 0 ,12)) from sale_mst where ori_no = a.inv_no ) as hdli_no")
	...
	public SqlResultMap getDetail(HttpRequestArgument arg) throws Exception {
		...
		data.param // 쿼리문  입력
		    ...
		    .query("     , (select stor_nm from stor where stor_id = b.wrhs_id) as wareh_nm 	")
	...
	public SqlResultMap getCounsel(HttpRequestArgument arg) throws Exception {
		...
		data.param // 쿼리문  입력
		    ...
		    .query("     , (select emp_nm from usr_mst where emp_id = a.upt_nm) as update_usr_nm 	")
			.query("     , a.user_memo         									")
			.query("     , a.upt_nm , a.upt_dttm , a.crt_nm , a.crt_dttm ")
		...
		for(SqlResultRow inv:map){
			...
			if ( "delete".equals(inv.getParamText("del_gb")) ) {
				...
				if ( ctrno.length() == 0 || ctrno == "" ){ /* 상담번호가 없으면 신규 */
					...
					.query("     , a.cust_id   , a.mmb_id    , a.emp_id 				")
					.query("     , a.user_memo 											")
					.query("     , a.upt_dttm , a.crt_dttm  , a.upt_nm 			")
					.query("order by a.ctr_no		           							")
				;
				...
				if ( row != null   ) { /* 조회 결과값이 있으면 */
		        	...
		        	data.param
			        	...
			        	data.param
							...
							.update("ctr_dt"		, new SimpleDateFormat("yyyyMMdd").format(new Date()) )
