...
@Service
public class TotalArWorkService  extends DefaultServiceHandler {
...
//				.where(" 			, max(x.upt_dttm) as upt_dttm , max(x.crt_dttm) as crt_dttm 					")
				.where(" 		from ( 																					")
				...
				.where("	 	 		,  ( select stor_nm from store where stor_id = a.stor_id) as stor_nm 		")
...
//			.query("	 	 		,  ( select cust_nm from cust_mst where cust_id = a.cust_id) as cust_nm 		")
...
//			.query(" 	 		where   a.stor_grp 	  = :stor_grp      " , arg.fixParameter("stor_grp" 				))
	...
	public SqlResultMap getDialogMemb(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		;
		data.param // 쿼리문  입력
			.query("select b.cust_gb  , a.cust_id , b.cust_cd , b.cust_nm , b.cust_grp				")
			.query("    ,  c.mmb_id  , c.mmb_nm  , c.sales_id	, b.user_memo						")
...
//			.where("where  a.stor_id  = :stor_id    	 " 	  , arg.fixParameter("stor_id" 		))
			...
			.where("group by  b.cust_gb  , a.cust_id , b.cust_cd , b.cust_nm , b.cust_grp 			")
			.where(" 	, c.mmb_id  , c.mmb_nm  , c.sales_id	, b.user_memo  , b.biz_tel_no 		")
			.where("	, b.biz_addr_1 , b.biz_addr_2 												")
...
//				.query("     ,  (select account_no from stor_account where account_id = a.account_id) as account_no 	")
//				.query("     ,  (select account_nm from stor_account where account_id = a.account_id) as account_nm 	")
			...
			if ( "techAction".equals(gubun) ){ /* 문구/테크 구분 수금 */
				...
				.query("     ,  b.cust_grp 	, a.cust_id	,  max(b.cust_nm) as cust_nm 					")
//				.query("     ,  max(c.dept_id) as pay_dept_id 											")
//				.query("     ,  max( a.salesman_id) as pay_usr_id , max(c.dept_id) as pay_dept_id 		")
//				.query("     ,  max(c.emp_nm) as pay_usr_nm 											")
//				.query("     ,  max(e.dept_nm) as pay_dept_nm 											")
				.query("from    cust_stor     a 														")
				.query("        join cust_mst b on b.cust_id  = a.cust_id 								")
				.query("        join usr_mst c on c.emp_id  = a.salesman_id 							")
...
//				.query("        left outer join dept_mst e on e.dept_id  = c.dept_id 					")
				.query("where   a.stor_grp    = :stor_grp 			"  , arg.fixParameter("stor_grp"   	))
				...
				if (info.size() == 1) {
					...
					.query("  		join sale_mst  b on b.inv_no  = a.inv_no 										")
					//.query("  		join cust_memb  p on a.cust_id = p.cust_id and a.mmb_id = p.mmb_id 	")
//					.query(" where  a.stor_id      = :stor_id 		"  		, arg.fixParameter("stor_id"   	))
					.query(" where  a.stor_grp      = :stor_grp 		"  		, arg.fixParameter("stor_grp"   	))
					.query("   and  a.cust_id   	= :cust_id  		"  		, arg.fixParameter("cust_id"    	))
					.query("   and  a.inv_dt between  :fr_dt            "       , arg.getParameter("fr_dt"), "1".equals(arg.getParamText("period")))  // 留ㅼ텧�쒖옉�쇱옄 
					.query("                     and  :to_dt            "       , arg.getParameter("to_dt"), "1".equals(arg.getParamText("period")))  // 留ㅼ텧醫낅즺�쇱옄 
...
//					.query(" where  a.stor_id      = :stor_id 		"  		, arg.fixParameter("stor_id"   	))
	...
	public SqlResultMap getArList(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		data.param // 쿼리문  입력
			...
			.query("   ,   h1.cust_id    , h1.cust_cd     ,  h1.cust_nm     , h1.cust_gb     , h1.cust_sts  				")
			.query("   ,   h1.npay_amt      																					") // , b.price_no
		;
		data.param	
			.where(" from (    																								")
			.where("	select a.hq_id    , a.stor_grp     																") // , a.stor_id
			.where("    	,  b.owner_id    , (select stor_nm from store     where stor_id = b.owner_id    ) as owner_nm ")
			.where("   		,  b.cust_id     , b.cust_cd     ,  b.cust_nm     , b.cust_gb     , b.cust_sts  				")
			.where("   		,  sum(a.npay_amt) as npay_amt      																") // , b.price_no
			.where("	from   cust_stor                    a 																")
...
//			SqlResultMap picking = row.getParameter("picking", SqlResultMap.class) ;
			...
			if (rowaction == Action.delete) {
	        	...
	        	if (picking != null && picking.size() > 0 ){
		        	...
		        	for(SqlResultRow item:picking){
		        		
			        	data.param
			        		.table("sale_payment")
			        		.where("where  pay_no  		= :pay_no   						")
			        		//
			        		.unique("pay_no"   		, item.fixParameter("pay_no"			))
...
//							.insert("crt_dttm"     , new SqlParamText("to_char(sysdate, 'yyyymmddhh24miss')"))
			    		...
			    		data.param
							...
							.query("				     and row_sts = 0                                      ")
							.query("				)															")
							.query("where t.inv_no in (select inv_no from sale_payment where pay_no = :pay_no ) ", item.fixParameter("pay_no")) 
						;data.attach();	
		        		
		        	}
		        	
	        	} else {   /* picking이 null일 경우  */
	        		
					System.out.println("PICKING = NULL");
...
//	        	if(row.getParameter("picking", SqlResultMap.class) != null){
//	        		setPicking(arg, data , row.getParameter("picking", SqlResultMap.class), row );
//	        	}
//	        	for(SqlResultRow item:picking){
//	        		data.param
//	        		.table("sale_payment")
...
//	        		//
//	        		.unique("pay_no"   		, item.fixParameter("pay_no"))
//	        		.update("pay_usr_id"   , item.getParameter("pay_usr_id"))
//	        		.update("pay_dept_id"   , item.getParameter("pay_dept_id"))
//	        		.action = Action.update 
//	        		; 
//	        		data.attach();  
//	        	}
...
//	        	if(row.getParameter("picking", SqlResultMap.class) != null){
//	        		setProduct(arg, data , row.getParameter("picking", SqlResultMap.class), row );
//	        	}	
			}
		}
		data.execute();
...
//		String today =  x.format(day);
		...
		for(SqlResultRow row:map){
			...
			if (rowaction == Action.delete) {
				...
				// 매출정보 미수금 정보 변경
//				data.param
//					.query("update sale_mst a   					   ")
//					.query("   set (  a.payment  					   ")
//					.query("  	   )=( select isnull(sum(c.payment),0)     ")
//					.query("       	     from sale_payment c  		  ")
...
//				data.attach();	        	
	        	...
	        	data.param
	        		...
	        		//.update("sales_id"      , row.getParameter("sales_id"))
...
//				.action = Action.modify;
	 ...
	 * @throws Exception
	 */
	public SqlResultMap getExportDetail(HttpRequestArgument arg) throws Exception {
