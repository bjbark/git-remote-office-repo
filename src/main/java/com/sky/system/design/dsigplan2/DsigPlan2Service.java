...
@Service
public class DsigPlan2Service extends DefaultServiceHandler{
	@Autowired
	...
	public SqlResultMap getSearch1(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.query("        , a.level            , a.status           , a.depends           , a.startismilestone")
			.query("        , a.collapsed        , a.canwrite         , a.canadd            , a.candelete		")
			.query("        , a.canaddlssue      , a.haschild         , a.starttime         , a.endtime			")
			.query("        , a.wkct_idcd        , a.chge_coef        , a.duration          , a.endismilestone	")
			...
			.query("        , p.dsig_cofm_yorn																	")
			.query("        , DATE_FORMAT(FROM_UNIXTIME(a.start/1000)	,'%Y%m%d') as start						")
			.query("        , DATE_FORMAT(FROM_UNIXTIME(a.end/1000)	,'%Y%m%d')     as end						")
		;
		data.param //퀴리문
	...
	public SqlResultMap getSearch3(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.query("        , a.collapsed        , a.canwrite         , a.canadd            , a.candelete		")
			.query("        , a.canaddlssue      , a.haschild         , a.starttime         , a.endtime			")
			.query("        , a.wkct_idcd        , a.ivst_pcnt        , a.need_mnhr         , a.endismilestone	")
	...
	public SqlResultMap setWrite(HttpRequestArgument arg ) throws Exception {
		...
		stnd_idcd = arg.getParamText("stnd_idcd");
		st_dt     = arg.getParamText("st_dt");

		data.param
			.query("call project_dsig_order (											")
			.query("     :pjod_idcd              " ,  pjod_idcd							)
			.query("   , :stnd_idcd              " ,  stnd_idcd							)
			.query("   , :st_dt                  " ,  st_dt								)
			.query(" ) 																	")
		;
	...
	public SqlResultMap setOk(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");
			data.param
				...
				.update("dsig_cofm_yorn"	, arg.getParameter("dsig_cofm_yorn"))	//승인여부
			;
