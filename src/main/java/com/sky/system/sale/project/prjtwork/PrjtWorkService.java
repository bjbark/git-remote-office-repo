...
public class PrjtWorkService extends DefaultServiceHandler {
	...
	public SqlResultMap getMaster(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		;
		data.param
			.query("select     a.pjod_idcd       , m.amnd_degr         , a.pjod_code       , a.pjod_dvcd		")
		...
		data.param
			...
			.where("left outer join ( select pjod_idcd,max(amnd_degr) as amnd_degr								")
			.where("                  from pjod_amnd group by pjod_idcd 										")
			.where("                ) m on a.pjod_idcd = m.pjod_idcd											")
			.where("where   1=1																					")
...
//		String bom			= arg.getParamText("bom") ;
		...
		data.param
			...
			.query("   :_param "  , arg.fixParameter("params"))
			.query(" ) 								")
		;
		data.attach(Action.direct);
		data.execute();
...
//		;
	...
	public SqlResultMap deleteMaster(HttpRequestArgument arg) throws Exception {
		...
		data.param
			.query("call pjod_delete_1 (			")
			.query(" :pjod_idcd "  , arg.getParameter("pjod_idcd")	)  // 주문번호
			.query(" ) 								")
		;
		data.attach(Action.direct);
		data.execute();
