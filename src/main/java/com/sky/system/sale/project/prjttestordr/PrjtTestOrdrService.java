...
@Service
public class PrjtTestOrdrService  extends DefaultServiceHandler {
	@Autowired
	...
	public SqlResultMap getMaster(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("        , p.amnd_degr        , p.expt_dvcd													")
			.where("        , p.item_name        , p.item_spec													")
			.where("        , p.modl_name        , p.regi_date as regi_date2               , p.deli_date		")
			.where("        , p.cofm_yorn        , p.cofm_date													")
			...
			.where("and     p.cofm_yorn != '0'												 ")
			.where("and     p.regi_date >= :regi_date1     " , arg.getParamText("regi_date1" ))
			.where("and     p.regi_date <= :regi_date2     " , arg.getParamText("regi_date2" ))
			.where("and     p.prjt_idcd  = :prjt_idcd      " , arg.getParamText("prjt_idcd"  ))
			.where("and     p.cstm_idcd  = :cstm_idcd      " , arg.getParamText("cstm_idcd"  ))
			.where("and     p.line_clos  = :line_clos      " , arg.getParamText("line_clos"  ))
			.where("and     p.deli_date >= :deli_date1     " , arg.getParamText("deli_date1" ))
			.where("and     p.deli_date <= :deli_date2     " , arg.getParamText("deli_date2" ))
			.where("and     p.drtr_idcd  = :drtr_idcd      " , arg.getParamText("drtr_idcd"  ))
	...
	public SqlResultMap getDetail(HttpRequestArgument arg) throws Exception {
		...
		data.param
			...
			.query("        , a.cvic_idcd        , a.strt_date         , a.sttm            , a.endd_date		")
			.query("        , a.edtm             , a.indn_qntt         , a.poor_qntt       , a.pass_qntt		")
			.query("        , a.drtr_idcd																		")
	...
	public SqlResultMap setWorkAction(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");
		data.param
			.query("call project_test_schd( :pjod_idcd"     , arg.getParameter("pjod_idcd"		))
			.query("                          , :line_seqn",arg.getParamText("line_seqn"		))
			.query(")"																				)
		;
