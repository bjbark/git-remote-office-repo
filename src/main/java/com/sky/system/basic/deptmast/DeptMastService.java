...
@Service
public class DeptMastService extends DefaultServiceHandler{
	...
	public SqlResultMap getLookup(HttpRequestArgument arg , int page, int rows) throws Exception {
		...
		data.param //퀴리문
			...
			.where("and		a.find_name	like %:find_name%"	, arg.getParameter("find_name"))
			.where("and		a.dept_name	like %:dept_name%"	, arg.getParameter("dept_name"))
			.where("and		a.prnt_idcd	like %:prnt_idcd%"	, arg.getParameter("prnt_idcd"))
			.where("and		a.line_stat	= :line_stat1"		, arg.getParamText("line_stat" ) , !"".equals(arg.getParamText("line_stat" )) )
			.where("and		a.line_stat	< :line_stat"		, "2" , "".equals(arg.getParamText("line_stat" )) )
			.where("order by a.dept_idcd")
		;
		...
		for(SqlResultRow row:arg.getParameter("records", SqlResultMap.class)){
			...
			if (rowaction == Action.delete) {
				...
				data.param
					...
					.update("dept_code"			, row.getParameter("dept_code"))
