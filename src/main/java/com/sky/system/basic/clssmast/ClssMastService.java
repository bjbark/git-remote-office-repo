package com.sky.system.basic.clssmast;
...
@Service
public class ClssMastService extends DefaultServiceHandler{
	/**
	 */
	...
	public SqlResultMap getSearchExcel(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.where("    from   cte a																				")
			.where("           left outer join clss_mast m on a.prnt_idcd = m.clss_idcd								")
			.where("           left outer join clss_mast d on a.clss_idcd = d.clss_idcd								")
			.where(") x																								")
			.where("where 1=1																						")
	...
	public SqlResultMap getLookup(HttpRequestArgument arg , int page, int rows) throws Exception {
		...
		;
		data.param
		.query("select a.clss_idcd  , a.clss_code     , a.clss_name  , a.clss_code_numb  	")
	...
	data.param //퀴리문
		...
		.where("where  1=1																")
		.where("and    a.prnt_idcd  = :prnt_idcd"	, arg.getParamText("prnt_idcd"		))
		.where("and    a.line_levl  = 1   " 		, arg.getParamText("line_levl"		))
		.where("and    a.clss_dvcd  = :clss_dvcd"	, arg.getParamText("clss_dvcd"		))
		.where("and    a.clss_name  like %:find_name%"	, arg.getParamText("find_name"))
		.where("and    a.line_stat  = :line_stat"	, arg.getParamText("line_stat" ) , !"".equals(arg.getParamText("line_stat" )) )
		.where("and    a.line_stat  < :line_stat"	, "2" , "".equals(arg.getParamText("line_stat" )) )
...
//					.insert("clss_dvcd"			, row.getParameter("clss_dvcd"	))
		...
		for(SqlResultRow row:map){
			...
			if (rowaction.equals(Action.delete)){
				data.param
					...
					.query("    from   cte a																	")
					.query("           left outer join clss_mast m on a.prnt_idcd = m.clss_idcd					")
					.query("           left outer join clss_mast d on a.clss_idcd = d.clss_idcd					")
					.query( " )																					")
				...
				data.param
					...
					//
					.update("clss_name"			, row.getParameter("clss_name"	))
					.update("clss_dvcd"			, row.getParameter("clss_dvcd"	))
					.update("clss_desc"			, row.getParameter("clss_desc"	))
					.insert("prnt_idcd"			, row.fixParameter("prnt_idcd"	))
					.insert("clss_code_numb"	, "0")
					.insert("line_levl"			, row.getParameter("line_levl"	))
				...
				if (provider.equals("mysql")) {
					data.param
						...
						.query("       left outer join clss_mast d on a.clss_idcd = d.clss_idcd								")
						.query(") b																							")
						.query("on clss_mast.clss_idcd = b.clss_idcd														")
						.query("set  clss_desc = b.ref_name																	")
						.query("where clss_mast.clss_idcd = :clss_idcd " , row.fixParameter("clss_idcd"))
						.query("and   clss_mast.line_levl = :line_levl " , row.fixParameter("line_levl"))
					;
					data.attach(Action.direct);
				}
			}
