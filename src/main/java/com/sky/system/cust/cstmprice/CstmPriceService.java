...
@Service
public class CstmPriceService extends DefaultServiceHandler{
	...
	public SqlResultMap getSearch(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("      , e.line_seqn as addr_seqn																	")
	...
	public SqlResultMap getDetail(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			.where("from (																						")
			.where("select a.cstm_idcd        , a.line_seqn         , a.item_idcd         , a.cont_date			")
			...
			.where("     , a.last_yorn        , a.uper_seqn         , a.disp_seqn         , c.item_name			")
			.where("     , u.user_name as drtr_name                 , i.cstm_code         , i.cstm_name			")
			.where("     , a.pric_dvcd        , c.item_spec         , bs.base_name as item_clss_bacd_name		")
			...
			.where("left outer join   cstm_mast i on a.cstm_idcd = i.cstm_idcd									")
			...
			.where("and    c.item_name   like %:item_name%" , arg.getParameter("item_name"))
			.where("and    c.item_spec   like %:item_spec%" , arg.getParamText("item_spec") )
	...
	public SqlResultMap setDetail(HttpRequestArgument arg) throws Exception {
		...
		for(SqlResultRow row:arg.getParameter("records", SqlResultMap.class)){
			...
			if(rowaction == Action.delete){
				...
				//구매목록,판매목록을 동시에 사용하면 중복에러가 뜨므로 DB에서 가장 마지막 line_seqn를 가져온다.
				data.param
				.query("select MAX(line_seqn)										")

				.where("from item_cont												")
				.where("where cstm_idcd = :cstm_idcd",row.fixParameter("cstm_idcd"  ))
...
//				if(hq.toUpperCase().equals("N1000DEHAN")){
//					data.param
...
//			.where("and     a.find_name   like %:find_name%  " , arg.getParamText("find_name") )
	...
	public SqlResultMap getBank(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		data.param
			.where("from (																								")
			.where("select  a.cstm_idcd      , a.line_seqn      , a.bank_name      , a.acct_nmbr      ,a.rpst_acct_yorn	")
