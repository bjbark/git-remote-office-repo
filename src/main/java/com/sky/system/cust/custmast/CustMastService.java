...
public class CustMastService extends DefaultServiceHandler{
	...
	public SqlResultMap getLookup(HttpRequestArgument arg , int page, int rows) throws Exception {
		...
		if (arg.getParamText("hq_id").toUpperCase().equals("N1000SJFLV")) {
			if ("Y".equals(arg.getParamText("oem_company" ))) {
				data.param
					.where(" and a.cstm_idcd in (select b.cstm_idcd ")
					.where("                       from oemb_mast a ")
					.where("                            left outer join oemb_cstm b on b.oemb_idcd = a.oemb_idcd ")
					.where("                      where a.sysm_memo = 'OEM Company') ")
				;
			}
		}
		data.param
	...
	public SqlResultMap getLookup3(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		if(arg.getParamText("hq_id").toUpperCase().equals("N1000HJSYS")){
			data.param
				.where("and     a.cstm_idcd in ( select cstm_idcd from cstm_sbsc where mngt_sbsc_valu = :mngt_sbsc_valu " , arg.getParameter("mngt_sbsc_valu"))
				.where("                        and mngt_sbsc_idcd = :mngt_sbsc_idcd        " , arg.getParameter("mngt_sbsc_idcd"))
				.where("                        union all select cstm_idcd  from wkct_mast where wkct_idcd = :wkct_idcd )", arg.getParameter("wkct_idcd"))
			;
		}
		data.param
			.where("GROUP BY a.cstm_idcd																									")
	...
	public SqlResultMap getLookup4(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		;
		data.param
			.query("SELECT	cm.buss_name     , cm.buss_numb     , cm.boss_name     , cm.tele_numb		")
			.query("      , ca.post_code     , ca.addr_1fst     , cm.mail_addr     , cm.cstm_idcd		")
		;
		data.param //퀴리문
			.where("  FROM cstm_mast cm																	")
			.where("  LEFT OUTER JOIN cstm_addr ca ON cm.cstm_idcd = ca.cstm_idcd						")
			.where(" WHERE 1=1																			")
			.where("   AND cm.line_stat = '0'															")
			.where("   AND cm.find_name like %:find_name%		" , arg.getParamText("find_name"		))
		;
		if (page == 0 && rows == 0 ) {
			return data.selectForMap(sort);
		} else {
	...
	public SqlResultMap getIsos(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		// 24.05.17 a_one 일시 거래처리스트> 수불대장이 거래처별로 보이도록 수정
		if(arg.getParamText("stor_id").toUpperCase().equals("N1000A-ONE1000")){
			data.param
				.query("call book_cstm_list ( :param",arg.fixParameter("param"))
				.query(")")
			;
		} else {
			data.param
	...
	public SqlResultMap setRecord(HttpRequestArgument arg) throws Exception {
		...
		for(SqlResultRow row:arg.getParameter("records", SqlResultMap.class)){
			...
			if (rowaction == Action.delete) {
				...
				;
				if(row.getParamText("hqof_idcd").toUpperCase().equals("N1000DAE-A")){
					data.param
						.update("find_name"			, row.getParamText("cstm_code"           ).trim().substring(0, 3)
													+ " "
...
//				if(arg.getParamText("hq_id").toUpperCase().equals("N1000DEHAN")){
//					data.param
...
//			.where("and     a.find_name   like %:find_name%  " , arg.getParamText("find_name") )
	...
	public SqlResultMap setBank(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");
		for(SqlResultRow row:arg.getParameter("records", SqlResultMap.class)){
			Action rowaction = SqlParameter.Action.setValue(row.getParameter("_set"));
			...
			if (rowaction == Action.delete) {
				...
				data.param
					...
					.unique("line_seqn"			, row.fixParameter("line_seqn"			))
					//
					.update("bank_name"			, row.getParameter("bank_name"           ))  /*  주소구분코드  */
					.update("acct_nmbr"			, row.getParameter("acct_nmbr"           ))  /*  우편번호  */
					.update("addr_name"			, row.getParameter("addr_name"           ))  /*  FULL주소  */
					.update("rpst_acct_yorn"	, row.getParameter("rpst_acct_yorn"      ))  /*  FULL주소  */

					.update("user_memo"			, row.getParameter("user_memo_bank"      ))  /*  사용자메모  */
					.update("sysm_memo"			, row.getParameter("sysm_memo"           ))  /*  시스템메모  */
					.update("prnt_idcd"			, row.getParameter("prnt_idcd"           ))  /*  상위 ID  */
					.update("line_levl"			, row.getParameter("line_levl"           ))  /*  ROW레벨  */
					.update("line_ordr"			, row.getParameter("line_ordr"           ))  /*  ROW순서  */
