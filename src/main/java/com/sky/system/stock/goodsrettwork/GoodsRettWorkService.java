...
@Service
public class GoodsRettWorkService extends DefaultServiceHandler {
	@Autowired
	...
	public SqlResultMap getMaster(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			.where("from (																									")
			.where("select    a.invc_numb       , a.invc_date       , a.bzpl_idcd       , a.expt_dvcd       , a.cstm_idcd	")
			.where("        , a.ostt_dvcd       , a.drtr_idcd       , a.dept_idcd       , a.rett_hhmm       , a.trut_dvcd	")
			.where("        , a.dlvy_cond_dvcd  , a.deli_date       , a.sale_stor_yorn  , a.crny_dvcd       , a.excg_rate	")
			...
			.where("        left outer join sale_rett_item s on a.invc_numb = s.invc_numb									")
	...
	public SqlResultMap getDetail(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			.where("from (																										")
			.where("select    a.invc_numb       , a.line_seqn       , a.acpt_numb       , a.acpt_seqn       , a.dlvy_numb		")
			.where("        , a.dlvy_seqn       , a.item_idcd       , a.unit_idcd       , a.wrhs_idcd       , a.zone_idcd		")
			.where("        , a.rett_resn_dvcd  , a.norm_sale_pric  , a.sale_stnd_pric  , a.sale_pric       , a.rett_qntt		")
			.where("        , a.vatx_incl_yorn  , a.vatx_rate       															")
			...
			.where("        , a.updt_user_name  , a.updt_ipad       , a.updt_dttm       , a.updt_idcd       , a.updt_urif		")
			.where("        , a.crte_user_name  , a.crte_ipad       , a.crte_dttm       , a.crte_idcd       , a.crte_urif		")
			.where("        ,  (a.rett_qntt * a.sale_pric) as sply_amnt															")
			.where("        , ((a.rett_qntt * a.sale_pric) * 0.1) as vatx_amnt													")
			.where("        , ((a.rett_qntt * a.sale_pric)+((a.rett_qntt * a.sale_pric) * 0.1)) as ttsm_amnt					")
			.where("        , ac.acpt_dvcd      , json_value(ac.json_data, '$.prod_trst_dvcd') as prod_trst_dvcd				")
			...
			.where("        left outer join acpt_mast ac on a.acpt_numb = ac.invc_numb											")
			.where("where   1=1																									")
			.where("and     a.invc_numb   = :invc_numb    " , arg.getParamText("invc_numb"  ))
			.where("and     a.line_stat   < :line_stat    " , "2" , "".equals(arg.getParamText("line_stat" )))
	...
	public SqlResultMap getInvoice(HttpRequestArgument arg) throws Exception {
		...
		if (info.size() >=1) {
			data.clear();
			data.param
				.query("select    a.invc_numb       , a.line_seqn       , a.acpt_numb       , a.acpt_seqn       , a.item_idcd		")
				.query("        , a.sale_unit       , a.norm_sale_pric  , a.sale_stnd_pric  , a.sale_pric       , a.ostt_qntt		")
				.query("        , a.vatx_incl_yorn  , a.vatx_rate       , a.sale_amnt as sply_amnt , a.vatx_amnt, a.ttsm_amnt		")
				.query("        , a.deli_date       , a.dlvy_date       , a.dlvy_hhmm       , a.stnd_unit       , a.stnd_unit_qntt	")
				.query("        , a.zone_idcd       , a.dlvy_cstm_idcd  , a.dsct_yorn       , a.ostt_dvcd		")
				.query("        , a.insp_dvcd       , a.insp_date       , a.pcod_numb       , a.sale_date       , a.sale_invc_numb	")
				.query("        , a.sale_qntt       , lo.lott_numb																	")
				.query("        , a.invc_numb as orig_invc_numb         , a.line_seqn  as   orig_seqn								")
				.query("        , a.uper_seqn       , a.disp_seqn       , ac.invc_qntt as acpt_qntt									")
				.query("        , m.invc_date       , m.bzpl_idcd       , m.expt_dvcd       , m.cstm_idcd		, m.deli_date		")
				.query("        , m.ostt_dvcd       , m.drtr_idcd       , m.dept_idcd       , m.trut_dvcd       , m.dlvy_cond_dvcd	")
				.query("        , m.deli_date       , m.sale_stor_yorn  , m.crny_dvcd       , m.excg_rate       , m.remk_text		")
				.query("        , im.item_code      , im.item_name      , im.item_spec      , im.unit_idcd as item_unit				")
				.query("        , wh.wrhs_code      , wh.wrhs_name      , wh.func_wrhs_idcd											")
				.query("        , cm.cstm_code      , cm.cstm_name      , cm.boss_name      , cm.tele_numb							")
				.query("        , dm.dept_code      , dm.dept_name																	")
				.query("        , bz.bzpl_code      , bz.bzpl_name																	")
				...
				.query("        , c2.dely_cstm_name as dlvy_cstm_name   , (ifnull(lo.qntt,0) + ifnull(s.rett_qntt,0)) as unpaid	")
				.query("        , am.acpt_dvcd      , json_value(am.json_data, '$.prod_trst_dvcd') as prod_trst_dvcd				")
				.query("        , lo.qntt																							")
				.query("from    sale_ostt_item a																					")
				.query("        left outer join sale_ostt_mast m on a.invc_numb = m.invc_numb										")
				.query("        left outer join cstm_deli c2 on replace(json_extract(m.json_data, '$.dlvy_cstm_idcd'),'\"','') = c2.dlvy_cstm_idcd	")
				.query("        left outer join acpt_item ac     on a.acpt_numb = ac.invc_numb and a.acpt_seqn = ac.line_seqn		")
				.query("        left outer join acpt_mast am     on ac.invc_numb = am.invc_numb										")
				.query("        left outer join item_mast im     on a.item_idcd = im.item_idcd										")
				.query("        left outer join wrhs_mast wh     on a.wrhs_idcd = wh.wrhs_idcd										")
				...
				.query("        left outer join lot_isos_book lo on lo.invc_numb = a.invc_numb and lo.invc_seqn = a.line_seqn and lo.isos_dvcd = '2200'	")
				.query("        left outer join ( select sum(ifnull(s.rett_qntt,0))as rett_qntt, s.orig_invc_numb, s.orig_seqn		")
				.query("                               , s.lott_numb																")
				.query("                  from sale_rett_item s 																	")
				.query("                  left outer join sale_rett_mast m on s.invc_numb = m.invc_numb								")
				...
				.query("where   1=1																									")
				.query("and     (ifnull(a.ostt_qntt,0)-ifnull(s.rett_qntt,0)) > 0													")
				.query("and     im.acct_bacd in (3000, 4000)																		")
				.query("and     json_value(am.json_data, '$.prod_trst_dvcd') <> '2000'												")
				...
				.query("and     a.acpt_numb  =  :acpt_numb			" , arg.getParamText("acpt_numb"  ))
				.query("and     am.acpt_dvcd =  :acpt_dvcd			" , arg.getParamText("acpt_dvcd"  ))
				.query("and     a.wrhs_idcd  =  :wrhs_idcd			" , arg.getParamText("wrhs_idcd"  ))
				.query("and     a.item_idcd  =  :item_idcd			" , arg.getParamText("item_idcd"  ))
				.query("and     m.deli_date  >= :fr_date			" , arg.getParamText("ostt_date1" ))
				.query("and     m.deli_date  <= :to_date			" , arg.getParamText("ostt_date2" ))
				.query("and     a.dlvy_cstm_idcd =  :dlvy_cstm_idcd	" , arg.getParamText("dlvy_cstm_idcd" ))	// 납품처
...
//				.query("                      from sale_item si																		")
//				.query("                           left outer join txbl_item ti on ti.invc_numb = si.prnt_idcd						")
//				.query("                     where 1 = 1																			")
//				.query("                       and ti.orig_invc_numb = a.invc_numb													")
//				.query("                       and ti.orig_invc_seqn = a.line_seqn )												")
//				.query("order by  cm.cstm_name asc , m.invc_date asc ,  im.item_code asc											")
			;
	...
	public void setInvoiceDetail(HttpRequestArgument arg, DataMessage data, SqlResultRow mst, SqlResultMap map , String cstm_idcd) throws Exception {
		for(SqlResultRow row:map) {
			...
			data.param
				...
				.query(" , :table "  	, "sale_rett_mast"	)  // 테이블명
			...
			new_ttsm_amnt  = Double.parseDouble(row.getParamText("new_ttsm_amnt"));
				...
				data.param
					...
					.update("dlvy_numb"			, row.getParameter("orig_invc_numb"		))	// 납품번호
					.update("dlvy_seqn"			, row.getParameter("orig_seqn"			))	// 납품순번
				...
				sequence.setBook2(arg, invc_numb, 1 ,  row.getParamText("lott_numb") , "반품입고");

				data.param
					.query("call sale_insert(")
					.query("   :STOR", arg.store)
...
//			.update("orig_invc_numb"	, arg.getParameter("orig_invc_numb"	))
	...
	public SqlResultMap deleteMaster(HttpRequestArgument arg) throws Exception {
		...
		if (isExist) {
			throw new ServiceException("수금등록이 되어있는 품목이 선택되었습니다.<br><br>삭제 할 수 없습니다." );
		}
		...
		data.param
			.table("sale_mast												")
			.where("where prnt_idcd = :prnt_idcd								")		//invoice번호

			.unique("prnt_idcd"			, arg.fixParameter("invc_numb"			))
		...
		data.param
			.table("sale_item												")
			.where("where orig_invc_numb = :orig_invc_numb								")		//invoice번호

			.unique("orig_invc_numb"			, arg.fixParameter("invc_numb"			))
