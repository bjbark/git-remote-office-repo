package com.sky.system.report.orderuserrpt;
...
public class OrderUserRptService  extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.query("	, t2.colt_schd_amt  , t2.payment , t2.npay_amt 									")//,	t2.fee_amt
		...
		data.param
			.where("from	( 																		")
			.where(" 			select 																		")
			.where("        			t1.stor_grp, t1.sales_man_id 										")
			.where("       			,	sum(t1.txfree_amt) as txfree_amt , sum(t1.sply_amt) as sply_amt 		")
			.where("       			,	sum(t1.tax_amt) as tax_amt 			 , sum(t1.inv_amt )  as inv_amt 			")
			.where("       			,	sum(t1.payment ) as payment  , sum(t1.npay_amt ) as npay_amt 			")
			.where("       			,	sum(t1.colt_schd_amt ) as colt_schd_amt   			")//, sum(t1.fee_amt )  as fee_amt
			...
			.where("  			group by t1.stor_grp, t1.sales_man_id										")
			.where(" 		) t2 left outer join usr_mst t1	on t1.emp_id = t2.sales_man_id				")
	...
	public SqlResultMap getSearchTime(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
		    ...
		    .where("      		 	from    order_mst t1  															")
            .where("							join usr_mst t3 on t1.sales_man_id = t3.emp_id 					")
		    ...
		    .where(" 				group by t1.stor_grp, t1.sales_man_id											")
		    .where(" 				) t2  																			")
		    .where(" where    t1.emp_id(+) = t2.sales_man_id														")
		    .where(" order by t1.login_id  																			")
		;
		if (page == 0 && rows == 0){
			return data.selectForMap(sort);
