package com.sky.system.report.ordervendorrpt;
...
@Service
public class OrderVendorRptService  extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.total("      ,sum(t1.sum_qty)         as sum_qty      ")
			.total("      ,sum(t1.sum_tax_free)    as sum_tax_free ")
			.total("      ,sum(t1.sum_subtotal)    as sum_subtotal ")
			.total("      ,sum(t1.sum_tax)         as sum_tax      ")
			.total("      ,sum(t1.sum_amount)      as sum_amount   ")
		;
		data.param
			.query("select t1.vend_id                                                                                                 ")
			.query("      ,b.vend_nm                                                                                                  ")
			.query("      ,t1.sum_tax_free                                                                                            ")
			.query("      ,t1.sum_subtotal                                                                                            ")
			.query("      ,t1.sum_tax                                                                                                 ")
			.query("                                                                                                                  ")
			.query("      ,t1.sum_qty                                                                                                 ")
			.query("      ,case sum(t1.sum_qty) over (partition by t1.stor_grp) when 0 then 0                                               ")
			.query("            else round(abs(t1.sum_qty)/sum(abs(t1.sum_qty)) over (partition by t1.stor_grp), 3)*100 end    as rate_qty  ")
			.query("                                                                                                                  ")
			.query("      ,t1.sum_amount                                                                                              ")
			.query("      ,case sum(t1.sum_amount) over (partition by t1.stor_grp) when 0 then 0                                            ")
			.query("            else round(abs(t1.sum_amount)/sum(abs(t1.sum_amount)) over (partition by t1.stor_grp), 3)*100 end as rate_amount ")
		;
		data.param
			.where("  from (                                                                                                          ")
			...
			.where("              join order_dtl b on a.stor_id = b.stor_id                                                        ")
			.where("              				   and a.inv_no   = b.inv_no                                                          ")
			.where("              left outer join itm_stor c on b.stor_id = c.stor_id                                             ")
			.where("              				   and b.item_idcd  = c.item_idcd                                                         ")
			...
			.where("        group by a.stor_grp, c.vend_id                                                                            ")
			.where("       ) t1                                                                                                        ")
			.where("       left outer join vend_mst b                                                                                ")
			.where("         on b.vend_id = t1.vend_id                                                                                 ")
			.where(" order by t1.vend_id                                                                                               ")
		;//
