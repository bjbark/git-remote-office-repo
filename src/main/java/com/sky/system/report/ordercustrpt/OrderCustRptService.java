package com.sky.system.report.ordercustrpt;
...
public class OrderCustRptService  extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
        ...
        data.param
	        ...
	        .query("            else round(abs(a.sum_payment)/sum(abs(a.sum_payment)) over (partition by a.stor_grp), 3)*100 end as rate_payment ")
	        .query("      ,a.sum_balance                                                                                              ")
	        .query("      ,case sum(a.sum_balance) over (partition by a.stor_grp) when 0 then 0                                            ")
	        .query("            else round(abs(a.sum_balance)/sum(abs(a.sum_balance)) over (partition by a.stor_grp), 3)*100 end as rate_balance ")
	    ;
	    data.param
		    .where("  from (                                                                                                          ")
		    ...
		    .where("          and a.chnl = :chnl                    ", arg.getParameter("chnl"))
...
//			.where("             ,sum(decode(to_char(to_date(a.ori_dt, 'yyyymmdd'), 'D'), '7', a.inv_amt, 0)) as week_sat              ")
	...
	public SqlResultMap getMonth(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
	    ...
	    data.param
		    ...
		    .where("         from order_mst a                                                                                        ")
		    .where("              left outer join cust_stor s  on s.stor_id = a.stor_id                                                                        ")
		    .where("               and s.cust_id = a.cust_id                                                                          ")
		    .where("        where a.sts_cd > '0100'     	                                                                          ")
			.where("          and a.row_sts = 0 				                                                                      ")
			.where("          and a.stor_grp = :stor_grp           ", arg.fixParameter("stor_grp"))
