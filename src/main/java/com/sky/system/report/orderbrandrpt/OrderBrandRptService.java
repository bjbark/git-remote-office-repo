package com.sky.system.report.orderbrandrpt;
...
public class OrderBrandRptService  extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
        ...
        data.param
	        ...
	        .query("            else round(abs(a.sum_unit_amount)/sum(abs(a.sum_unit_amount)) over (partition by a.stor_grp), 3)*100) as rate_unit_amount ")
	        .query("      ,a.sum_tax_free                                                                                                         ")
	        .query("      ,a.sum_subtotal                                                                                                         ")
	        .query("      ,a.sum_tax                                                                                                              ")
	        .query("      ,a.sum_amount                                                                                                           ")
	        .query("      ,case sum(a.sum_amount) over (partition by a.stor_grp) when 0 then 0                                                         ")
	        .query("            else round(abs(a.sum_amount)/sum(abs(a.sum_amount)) over (partition by a.stor_grp), 3)*100)           as rate_amount      ")
	    ;
	    data.param
	        .where("  from (                                                                                                                      ")
	        ...
	        .where("              join order_dtl b            on b.inv_no = a.inv_no                                                                                        ")
	        .where("              left outer join itm_mst i   on i.item_idcd = b.item_idcd                                                                                      ")
	        .where("              left outer join base_mst m  on m.bas_id = i.brand_id                                                                                     ")
