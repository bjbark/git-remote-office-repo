...
public class CustItemRptService  extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
        ...
        data.param // 조회
	        ...
	        .query("            else round(abs(a.sum_qty)/sum(abs(a.sum_qty)) over (partition by a.stor_grp), 3)*100  as rate_qty                 ")
	        ...
	        .query("      ,(select clss_desct from item_class where clss_id = a.clss_id)  as clss_nm                 ")
	    ;
	    data.param // 조건
	        .where("  from (                                                                                                                              ")
	        ...
	        .where("          and a.inv_inpt_path in (:inv_inpt_path)                                                                                         ", inv_inpt_path, (inv_inpt_path.length>0))
	        ...
	        .where("          and a.retn_yn = :retn_yn                                                                                                      ", arg.getParameter("retn_yn"))
	        .where("		  and a.row_sts = 0					")
	        .where("        group by a.stor_grp, b.item_code, b.item_name, b.item_spec, b.unit_idcd, c.clss_1, c.clss_2, c.clss_3, i.clss_id                  ")
	        .where("       ) a                                                                                                                            ")
	        .where("       left outer join itm_unit b                                                                                                    ")
	        .where("         on b.unit_idcd = a.unit_idcd                                                                                                     ")
	        .where(" order by a.item_code, a.item_name, a.item_spec, a.unit_idcd, a.clss_1, a.clss_2, a.clss_3, a.clss_id                                     ")
	    ;
...
//			.where("             ,sum(decode(to_char(to_date(a.inv_dt, 'yyyymmdd'), 'D'), '7', b.inv_amt, 0)) as week_sat                                  ")
	...
	public SqlResultMap getDay(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
	    ...
	    data.param // 조건
	        ...
	        .where("              join cust_stor c           on c.stor_id = a.stor_id  and c.cust_id = a.cust_id                      ")
	        .where("              join itm_mst i             on i.item_idcd  = b.item_idcd                                                  ")
