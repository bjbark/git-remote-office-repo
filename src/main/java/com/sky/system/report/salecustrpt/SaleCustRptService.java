package com.sky.system.report.salecustrpt;
...
@Service
public class SaleCustRptService  extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.query("   , t2.payment, t2.npay_amt   							")//,    t2.fee_amt  , t2.colt_schd_amt
		;

		data.param
			.where("from    cust_mst t1,    																")
			.where("        ( select  											   						")
			.where("                  t1.stor_grp, t1.cust_id   										")
			...
			.where("                  join  cust_stor c2	on t1.stor_id = c2.stor_id  and t1.cust_id = c2.cust_id	")
			...
			.where("                                and :to_dt      " , to_dt , "1".equals( inv_po_term ))  // 배송예정종료일자
		   	...
		   	.where("          and     ( t1.cust_id , t1.mmb_id ) in ( select cust_id , mmb_id from cust_memb where cust_id = t1.cust_id and sales_id = :sales_id  ) "  , arg.getParameter("sales_id" ))
			.where("          and     t1.sts_cd > '0100'   										    	")
			.where("          and     t1.row_sts = 0 				                                    ")
			.where("	      and     t1.cust_id   =  :cust_id       	   "	, 		arg.getParameter("cust_id" )) // 고객코드
			.where("	      and     c2.clss_1   =  :clss_1       	   "	, 		arg.getParameter("clss_1" )) // 1차분류
			.where("	      and     c2.clss_2   =  :clss_2       	   "	, 		arg.getParameter("clss_2" )) // 2차분류
			.where("	      and     c2.clss_3   =  :clss_3       	   "	, 		arg.getParameter("clss_3" )) // 3차분류
			.where("          group by t1.stor_grp, t1.cust_id   										")
			.where("        ) t2  																		")
			.where("where   t1.cust_id = t2.cust_id   													")
			.where("order by t1.cust_id			   														")
		;
	...
	public SqlResultMap getSearchTime(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		;
		data.param
	        .query("select  t1.cust_cd, t1.cust_nm  , t2.*  														")
		;
		data.param
			.where("from    cust_mst t1, (  																		")
		    .where("                select  																		")
...
//	    	.where(" 					  ,  sum(decode(to_char(to_date(t1.inv_dt, 'yyyymmdd'), 'd'), '7', t1.inv_amt, 0)) as week7  ")
	...
	public SqlResultMap getSearchDay(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
    		...
    		.where("            			t1.stor_grp, t1.cust_id  												")
    		.where("           			,   sum(t1.inv_amt) as inv_amt  												")
			.where("             ,sum(case when substring(t1.inv_dt, 7, 2) = '01' then t1.inv_amt else 0 end) as date01                 ")
			.where("             ,sum(case when substring(t1.inv_dt, 7, 2) = '02' then t1.inv_amt else 0 end) as date02                 ")
			.where("             ,sum(case when substring(t1.inv_dt, 7, 2) = '03' then t1.inv_amt else 0 end) as date03                 ")
			.where("             ,sum(case when substring(t1.inv_dt, 7, 2) = '04' then t1.inv_amt else 0 end) as date04                 ")
			.where("             ,sum(case when substring(t1.inv_dt, 7, 2) = '05' then t1.inv_amt else 0 end) as date05                 ")
			.where("             ,sum(case when substring(t1.inv_dt, 7, 2) = '06' then t1.inv_amt else 0 end) as date06                 ")
			.where("             ,sum(case when substring(t1.inv_dt, 7, 2) = '07' then t1.inv_amt else 0 end) as date07                 ")
			.where("             ,sum(case when substring(t1.inv_dt, 7, 2) = '08' then t1.inv_amt else 0 end) as date08                 ")
			.where("             ,sum(case when substring(t1.inv_dt, 7, 2) = '09' then t1.inv_amt else 0 end) as date09                 ")
	...
	public SqlResultMap getSearchMonth(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
		    ...
		    .where("            			t1.stor_grp, t1.cust_id  												")
		    .where("           			,   sum(t1.inv_amt) as inv_amt  												")
			.where("             ,sum(case when substring(t1.inv_dt, 5, 2) = '01' then  t1.inv_amt else 0 end ) as month01              ")
			.where("             ,sum(case when substring(t1.inv_dt, 5, 2) = '02' then  t1.inv_amt else 0 end ) as month02              ")
			.where("             ,sum(case when substring(t1.inv_dt, 5, 2) = '03' then  t1.inv_amt else 0 end ) as month03              ")
			.where("             ,sum(case when substring(t1.inv_dt, 5, 2) = '04' then  t1.inv_amt else 0 end ) as month04              ")
			.where("             ,sum(case when substring(t1.inv_dt, 5, 2) = '05' then  t1.inv_amt else 0 end ) as month05              ")
			.where("             ,sum(case when substring(t1.inv_dt, 5, 2) = '06' then  t1.inv_amt else 0 end ) as month06              ")
			.where("             ,sum(case when substring(t1.inv_dt, 5, 2) = '07' then  t1.inv_amt else 0 end ) as month07              ")
			.where("             ,sum(case when substring(t1.inv_dt, 5, 2) = '08' then  t1.inv_amt else 0 end ) as month08              ")
			.where("             ,sum(case when substring(t1.inv_dt, 5, 2) = '09' then  t1.inv_amt else 0 end ) as month09              ")
