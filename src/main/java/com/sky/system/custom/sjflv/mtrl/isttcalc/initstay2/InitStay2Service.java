...
@Service("sjung.InitStay2Service")
public class InitStay2Service extends DefaultServiceHandler {
	@Autowired
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("             , a.invc_numb, a.invc_date																	")
			.where("             , b.line_seqn, b.item_idcd, d.item_code, d.item_name										")
			.where("             , b.istt_qntt, b.istt_pric, b.istt_amnt, b.istt_vatx, b.ttsm_amnt							")
			.where("             , b.lott_numb, CONCAT(COALESCE(c.cstm_name, ''), COALESCE(b.user_memo, '')) AS user_memo	")
			.where("             , json_value(b.json_data, '$.trns_date') as trns_date										")
			.where("             , m.invc_numb as purc_invc_numb , m.invc_date as purc_invc_date							")
			.where("          from purc_istt_mast a																			")
			.where("               left outer join purc_istt_item b on b.invc_numb = a.invc_numb							")
			.where("               left outer join purc_ordr_item p on b.orig_invc_numb = p.invc_numb and b.orig_seqn = p.line_seqn	")
		...
		if (arg.getParamText("row_type" ).contains("1")) {
			data.param
				...
				.where("         , null as invc_numb, null as invc_date																											")
				.where("         , null as line_seqn, null as item_idcd, null as item_code, null as item_name																	")
				.where("         , sum(a.istt_qntt) as istt_qntt, null istt_pric, sum(a.istt_amnt) as istt_amnt, sum(a.istt_vatx) as istt_vatx, sum(a.ttsm_amnt) as ttsm_amnt	")
				.where("         , null as lott_numb, null as user_memo																											")
				.where("         , null as trns_date, null as purc_invc_numb , null as purc_invc_date																			")
				.where("      from cte a																																		")
		...
		if (arg.getParamText("row_type" ).contains("2")){
			data.param
				...
				.where("         , null as invc_numb, null as invc_date																											")
				.where("         , null as line_seqn, null as item_idcd, null as item_code, null as item_name																	")
				.where("         , sum(a.istt_qntt) as istt_qntt, null istt_pric, sum(a.istt_amnt) as sale_amnt, sum(a.istt_vatx) as istt_vatx, sum(a.ttsm_amnt) as ttsm_amnt	")
				.where("         , null as lott_numb, null as user_memo																											")
				.where("         , null as trns_date, null as purc_invc_numb , null as purc_invc_date																			")
				.where("      from cte a																																		")
...
//			.where("         , json_value(m.json_data, '$.prod_trst_dvcd') as prod_trst_dvcd	")
			.where("    from purc_istt_mast a																")
			.where("    left outer join purc_istt_item b on b.invc_numb = a.invc_numb						")
			.where("    left outer join cstm_mast c on c.cstm_idcd = a.cstm_idcd							")
			.where("    left outer join item_mast d on d.item_idcd = b.item_idcd							")
...
//			boolean isExist2 = isExistTxblItem(arg, row);

			if (isExist) {
				throw new ServiceException("매입대금 지급이 등록되어있는 품목입니다.<br><br>이월 할 수 없습니다." );
			}

//			if (isExist2) {
//				throw new ServiceException("반품 등록되어있는 품목입니다.<br><br>이월 할 수 없습니다." );
//			}
			...
			data.param
				...
				.modify("json_data", "json_data", "JSON_MERGE_PRESERVE(JSON_REMOVE(ifnull(json_data,'{}'),'$.trns_date'), '" + json + "')")
			;
	...
	public SqlResultMap setDel_yn(HttpRequestArgument arg) throws Exception {
		...
		for (SqlResultRow row:map) {
			boolean isExist = isExistCrdtItem(arg, row);
			if (isExist) {
				throw new ServiceException("매입대금 지급이 등록되어있는 품목입니다.<br><br>삭제 할 수 없습니다." );
			}
	...
	public boolean isExistCrdtItem(HttpRequestArgument arg, SqlResultRow row) throws Exception {
		...
		data.param
			...
			.where("             and b.invc_numb = :invc_numb	" , row.getParamText("invc_numb"))
			.where("             and b.line_seqn = :line_seqn	" , row.getParamText("line_seqn"))
			.where("            )															")
		;
		int count = Integer.parseInt(data.selectForRow().getParamText("count"));
