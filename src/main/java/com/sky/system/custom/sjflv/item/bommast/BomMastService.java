...
public class BomMastService extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.where("and   substr(a.acct_bacd,1,1) in ('3','2','1')								")
	...
	public SqlResultMap getSearch3(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.query("       , i.item_name       , i.item_spec      , a.used_yorn								")
	...
	public SqlResultMap setCopyBom(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");
		String str = "prnt_item_idcd,item_idcd,befr_splr_name,splr_name,usag_qntt_memo,ecod_purp,drtr_idcd,test_date,adpt_date,item_revs_numb,item_revs_dvcd,prnt_idcd,remk_text,revs_dvcd";
		for(SqlResultRow row:arg.getParameter("records", SqlResultMap.class)){
...
//		String bacd = "";
	...
	public SqlResultMap setLog(HttpRequestArgument arg) throws Exception {
		DataMessage data	 = arg.newStorage("POS");
		String lgin_idcd	= arg.getParamText("lgin_idcd") ;
		String menu_idcd	= arg.getParamText("menu_idcd") ;
		String log_dvcd		= arg.getParamText("log_dvcd") ;
		String log_name		= arg.getParamText("log_name") ;

		data.param
			.query("call menu_log_insert (			")
			.query("   :lgin_idcd "  , lgin_idcd	)  // Invoice 번호
			.query(" , :menu_idcd "  , menu_idcd	)  // 메뉴명
			.query(" , :log_dvcd "   , log_dvcd	)  // 작업구분
			.query(" , :log_name "   , log_name	)  // 작업구분명
			.query(" ) 								")
		;
		data.attach(Action.direct);
		data.execute();
