package com.sky.system.custom.sjflv.sale.order.saleorder;
...
@Service("sjflv.SaleOrderService")
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		;
			...
			data.param
				...
				.where("		, c.tele_numb       , c.hdph_numb																	")
				.where("		, w.wrhs_code       , w.wrhs_name       , i.pdsd_yorn         										")
				.where("		, i.item_idcd       , i.invc_amnt       , i.sply_amnt         , i.vatx_amnt							")
				.where("		, i.invc_qntt       , i.invc_pric       , bs.base_name as mtrl_name									")
			...
			;
			data.param
				.where("        , case when a.acpt_dvcd = '2000' and json_value(a.json_data, '$.prod_trst_dvcd') = '2000' then i.ostt_qntt											") 	// 출고수량
				.where("               else z.ostt_qntt end ostt_qntt													 															")
				.where("        , case when a.acpt_dvcd = '2000' and json_value(a.json_data, '$.prod_trst_dvcd') = '2000' then														")	// 출고금액
				.where("                    case when a.acpt_stat_dvcd = '0400' or a.acpt_stat_dvcd = '0600' then i.invc_amnt else 0 end 											")
				.where("               else z.sale_amnt end sale_amnt																												")
				.where("        , case when a.acpt_dvcd = '2000' and json_value(a.json_data, '$.prod_trst_dvcd') = '2000' then ifnull(null, date_format(i.dlvy_date, '%Y-%m-%d')) 	")	// 출고일자
				.where("               else ifnull(null, date_format(z.invc_date, '%Y-%m-%d')) end ostt_date																		")
				.where("        , case when a.acpt_dvcd = '2000' and json_value(a.json_data, '$.prod_trst_dvcd') = '2000' then (i.invc_qntt - i.ostt_qntt) 							")	// 미납잔량
				.where("               else ( ifnull(i.invc_qntt,0)-ifnull(z.ostt_qntt,0)) end upid_baln_qntt						")
			;
			data.param
				.where("        , cast(replace(json_extract(i.json_data, '$.pack_qntt'),'\"','') as char) as pack_qntt				")
				.where("		, json_value(a.json_data, 	'$.prod_trst_dvcd') as prod_trst_dvcd									")
				.where("		, json_value(i.json_data, 	'$.cnfm_yorn') as cnfm_yorn												")
				.where("		, json_value(i.json_data, 	'$.prod_yorn') as prod_yorn												")
				.where("		, json_value(im.json_data,	'$.labl_chck_yorn') as labl_chck_yorn									")
				.where("		, y.pdod_date																						")
			;
			data.param
				...
				.where("                      and i.acct_bacd in ('3000', '4000') 													")
				...
				.where("left   outer join ( select MIN(substring(ppa.plan_sttm,1,8)) as pdod_date, ppa.invc_numb , ppa.acpt_numb	")
				.where("                    from prod_plan_acpt ppa  																")
				.where("                    where ppa.line_stat = '0'																")
				.where("                    group by ppa.acpt_numb																	")
				.where("                  ) y on y.acpt_numb = a.invc_numb															")
				.where("where  1=1																									")
				...
				.where("and    json_value(a.json_data, 	'$.prod_trst_dvcd')	 = :prod_trst_dvcd		" , arg.getParamText("prod_trst_dvcd"	))
...
//			.query("		, a.drtr_idcd       , a.dept_idcd       , a.crny_dvcd         , a.excg_rate				")
			...
			.query("		, json_value(a.json_data, '$.prod_trst_dvcd') as prod_trst_dvcd							")
		...
		if (info.size() >=1) {
			...
			data.param
				...
				.query("		, json_value(m.json_data, '$.prod_trst_dvcd') as prod_trst_dvcd									")
			;
...
//					.update("expt_dvcd"			, row.getParameter("expt_dvcd"			))  /*  수출구분코드  */
	...
	public void setInvoiceDetail(HttpRequestArgument arg, DataMessage data, SqlResultRow mst, SqlResultMap map) throws Exception {
		...
		for(SqlResultRow row:map) {
			...
			if (rowaction == Action.delete) {
				...
				String json_data2 = trans2.TranslateRow(arg, row, "acpt_mast_json_fields");
	...
	public void setInvoiceInsertDetail(HttpRequestArgument arg, DataMessage data, SqlResultRow mst, SqlResultMap map) throws Exception {
		...
		for(SqlResultRow row:map) {
			...
			// STEP 2) master 등록
			ParamToJson trans = new ParamToJson();
			String json_data = trans.TranslateRow(arg, mst, "acpt_mast_json_fields");
			...
			data.param
				...
				.update("amnd_degr"			, mst.getParameter("amnd_degr"			))  /*  amd차수  */
				.update("bzct_dvcd"			, mst.getParameter("bzct_dvcd"			))  /*  사업부문구분코드  */
				.update("invc_date"			, mst.getParameter("invc_date"			))  /*  invoice일자  */
				.update("ordr_dvcd"			, mst.getParameter("ordr_dvcd"			))  /*  오더구분코드  */
				.update("orig_invc_numb"	, mst.getParameter("orig_invc_numb"		))  /*  원invoice번호  */
				.update("pcod_numb"			, mst.getParameter("pcod_numb"			))  /*  pono */
				.update("deli_date"			, mst.getParameter("deli_date"			))  /*  납기일자 */
				.update("cstm_idcd"			, mst.getParameter("cstm_idcd"			))  /*  거래처ID */
				.update("dlvy_cstm_idcd"	, mst.getParameter("dlvy_cstm_idcd"		))  /*  납품처ID */
				.update("mdtn_prsn"			, mst.getParameter("mdtn_prsn"			))  /*  중개인  */
				.update("cont_date"			, mst.getParameter("cont_date"			))  /*  계약일자  */
				...
				.update("ostt_wrhs_idcd"	, mst.getParameter("ostt_wrhs_idcd"		))  /*  출고창고  */
				.update("acpt_dvcd"			, mst.getParameter("acpt_dvcd"			))  /*  수주구분코드  */
				.update("trut_dvcd"			, mst.getParameter("trut_dvcd"			))  /*  위탁구분코드  */
				.update("dlvy_cond_dvcd"	, mst.getParameter("dlvy_cond_dvcd"		))  /*  인도조건구분코드  */
				.update("crdt_exce_yorn"	, mst.getParameter("crdt_exce_yorn"		))  /*  여신초과여부  */
				.update("amnt_lack_yorn"	, mst.getParameter("amnt_lack_yorn"		))  /*  금액미달여부  */
				.update("sale_stor_yorn"	, mst.getParameter("sale_stor_yorn"		))  /*  판매보관여부  */
				.update("remk_text"			, mst.getParameter("remk_text"			))  /*  비고  */
				.update("memo"				, mst.getParameter("memo"				))  /*  메모  */
				.update("cofm_yorn"			, mst.getParameter("cofm_yorn"			))  /*  확정여부  */
				.update("cofm_dttm"			, mst.getParameter("cofm_dttm"			))  /*  확정일시  */
				.update("cofm_drtr_idcd"	, mst.getParameter("cofm_drtr_idcd"		))  /*  확정담당자ID  */
				.update("acpt_stat_dvcd"	, mst.getParameter("acpt_stat_dvcd"		))  /*  수주상태구분코드  */
				.update("expt_dvcd"			, 0)  /*  수출구분코드  */
			;

			data.param
				.update("json_data"			, json_data)
				.update("user_memo"			, mst.getParameter("user_memo"			))  /*  시스템메모  */
				.update("sysm_memo"			, mst.getParameter("sysm_memo"			))  /*  시스템메모  */
				.update("prnt_idcd"			, mst.getParameter("prnt_idcd"			))  /*  상위 ID  */
				.update("line_levl"			, mst.getParameter("line_levl"			))  /*  ROW레벨  */
				...
				.update("find_name"			, mst.getParamText("invc_date"			).trim()
											+ " "
											+ mst.getParamText("cstm_idcd"			).trim()
											+ " "
											+ mst.getParamText("cstm_name"			).trim())
				.update("updt_user_name"	, mst.getParameter("updt_user_name"		))  /*  수정사용자명  */
	...
	public SqlResultMap setDel_yn(HttpRequestArgument arg) throws Exception {
		...
		// 24.04.09 조달 구분이 직수출인 경우 삭제 추가
		String expt_dvcd  = (String) arg.fixParameter("expt_dvcd");

		if(expt_dvcd == "1" || expt_dvcd.equals("1")) {
			temp.param
				.query("select line_stat, line_clos				")
				.query("from  acpt_mast							")
			 	.query("where invc_numb = :invc_numb", arg.fixParameter("invc_numb"))
			;
			SqlResultRow del2 = temp.selectForRow();

			if ( Double.parseDouble( del2.getParamText( "line_clos" )) == 1) {
			...
			temp2.param
				...
				.query("from  acpt_item							")
			 	.query("where invc_numb = :invc_numb", arg.fixParameter("invc_numb"))
			;
			SqlResultRow cnts = temp2.selectForRow();

			if ( Double.parseDouble( cnts.getParamText( "cnt" )) == 0) {
				data.param
					.table("acpt_mast")
					.where("where invc_numb = :invc_numb ")
					.where("and   amnd_degr = :amnd_degr ")
...
//	}
	...
	public SqlResultMap setConfirm(HttpRequestArgument arg) throws Exception {
		...
		data.param
			...
			.unique("line_seqn"			, arg.getParameter("line_seqn"))
			//
			.modify("json_data", 		"json_data", "JSON_MERGE_PRESERVE(JSON_REMOVE(json_data, '$.cnfm_yorn'), '{\"cnfm_yorn\":\"" + arg.getParameter("cnfm_yorn") + "\"}')	")
			.update("updt_dttm"			, new SimpleDateFormat("yyyyMMddHHmmss").format(new Date())) /* 수정일자 */
	...
	public SqlResultMap getOrder_New(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.query("		 , e.dely_cstm_name   , b.invc_date	      , a.user_memo									")
			.query("		 , b.acpt_dvcd																			")
		;
		data.param
			.where("from acpt_item a 																				")
