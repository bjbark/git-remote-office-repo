...
public class OmsMastService {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.where("      , u.user_name as drtr_name             , i.item_code       , a.labr_drtr_idcd								")
			.where("      , c.cstm_code																								")
...
//			.where("      , u4.user_name as labr_drtr_name																			")
			...
			.where("        left outer join user_mast u on a.cstm_drtr_idcd = u.user_idcd and a.cstm_idcd = u.cstm_idcd				")
...
//			.where("        left outer join user_mast u4 on a.labr_drtr_idcd = u4.user_idcd											")
			...
			.where("   and a.cstm_drtr_idcd = :cstm_drtr_idcd		" , arg.getParamText("cstm_drtr_idcd"))	//요청담당자
	...
	public SqlResultMap getCstmInfo(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {

		DataMessage data = arg.newStorage("POS");

		data.param
			.query("select  c.cstm_name   , d.dlvy_addr_1fst   , c.cstm_code   , c.cstm_idcd										")
			.query("      , d.dlvy_zpcd   , d.dlvy_tele_numb																		")
			.query("from    user_mast a																								")
			.query("        left outer join cstm_mast c on a.cstm_idcd = c.cstm_idcd												")
			.query("        left outer join cstm_deli d on a.cstm_idcd = d.cstm_idcd												")
			.query("where   1=1																										")
			.query("   and a.user_idcd = :user_idcd			" , arg.getParamText("user_idcd"))	//요청담당자
	...
	public SqlResultMap getSmplItem(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.where("      ,(select base_name from base_mast m where json_value(a.json_data,'$.make_natn') = m.base_code and m.prnt_idcd  = '3300' ) as make_natn 				")
		...
		for(SqlResultRow row:arg.getParameter("records", SqlResultMap.class)){
			Action rowaction = SqlParameter.Action.setValue(row.getParameter("_set"));
				...
				data.param
					...
					.update("cstm_drtr_idcd"	, row.getParameter("cstm_drtr_idcd"		))  /*  거래처담당자명    */
