package com.sky.system.custom.sjflv.sale.order.estimast2;
...
@Service("sjflv.EstiMast2Service")
public class EstiMast2Service  extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.query("        , c.cstm_code             , c.boss_name			, a.acpt_numb							")
			.query("        , ifnull(c.cstm_name, a.cstm_name) as cstm_name											")
		...
		data.param
			...
			.query("order by a.invc_date desc , a.invc_numb desc) a														")
		;
		if (page == 0 && rows == 0){
	...
	public SqlResultMap getDetail(HttpRequestArgument arg) throws Exception {
		...
		data.param
			...
			.query("        , a.item_idcd         , a.cstm_item_name    , a.cstm_item_code   						")
			...
			.query("        , a.updt_user_name    , a.updt_ipad         , a.updt_dttm        , a.updt_idcd			")
			.query("        , a.updt_urif         , a.crte_user_name    , a.crte_ipad        , a.crte_dttm			")
			.query("        , a.crte_idcd         , a.crte_urif         , a.deli_date as deli_date2					")
			.query("        , i.item_code																			")
			.query("        , case when a.item_idcd is not null then a.item_name else i.item_name end as item_name	")
			.query("        , case when a.item_idcd is not null then a.item_spec else i.item_spec end as item_spec	")
			.query("        , case when a.item_idcd is not null then a.unit_idcd else i.unit_idcd end as unit_idcd	")
			.query("		, json_value(a.json_data, '$.pack_qntt') as pack_qntt									")
			.query("        , concat(ai.invc_numb, '-', ai.line_seqn) as acpt_numb									")
		;
		data.param
	...
	public SqlResultMap getInvoice(HttpRequestArgument arg) throws Exception {
		...
		data.param
			...
			.query("        , c.cstm_code             , a.cstm_name         , c.boss_name							")
			.query("        , u.user_name as drtr_name																")
			.query("from esti_mast a																				")
			.query("left outer join cstm_mast c on a.cstm_idcd = c.cstm_idcd										")
			.query("left outer join user_mast u on a.drtr_idcd = u.user_idcd										")
	...
	public SqlResultMap getInvoiceCheck(HttpRequestArgument arg) throws Exception {
	...
	public SqlResultMap setInvoice(HttpRequestArgument arg) throws Exception {
		...
		for (SqlResultRow row:map) {
			...
			if (rowaction == Action.delete) {
				...
				if(row.getParameter("invc_numb") == null || row.getParameter("invc_numb").equals("")) {
					SqlResultMap invoiceResult = getInvoiceNumber(arg);

					SqlResultRow firstRow = invoiceResult.get(0);
					// seqn 값 가져오기
					java.math.BigInteger seqnBigInt = (java.math.BigInteger) firstRow.get("seqn");

					int seqn = seqnBigInt.intValue();

					// NO-XXXX 형식으로 변환
					invc_numb = String.format("NO-%04d", seqn);

//					data.param
//						.query("call fn_seq_gen_v2 (			")
//						.query("   :STOR "   	, arg.store		)  // 본사코드
//						.query(" , :table "  	, "esti_mast"	)  // 테이블명
//						.query(" , :invc_numb " , "not defined"	)  // Invoice 번호
//						.query(" ) 								")
//					;
//					invc_numb = (String)data.selectForMap().get(0).get("seq");
//					data.clear();
				} else {
	...
	public SqlResultMap setDel_yn(HttpRequestArgument arg) throws Exception {
		...
		if(arg.getParameter("last_yorn").equals("1")){
			data.param
				.table("seqn_dflt")
				.where("where hqof_idcd = :hqof_idcd ")
				.where("and   tabl_name = :tabl_name ")

				.unique("hqof_idcd"		, arg.fixParameter("hqof_idcd"))
				.unique("tabl_name"		, arg.fixParameter("tabl_name"))

				.update("last_seqn"		, arg.getParameter("last_seqn"))
				.update("last_idcd"		, arg.getParameter("last_idcd"))
				.update("code_last_seqn", arg.getParameter("code_last_seqn"))
				.update("last_code"		, arg.getParameter("last_code"))
	...
	public SqlResultMap setAcpt(HttpRequestArgument arg) throws Exception {
		...
		Long newItemCount = this.getNewItemCount(arg);
		if (newItemCount > 0) {
			throw new ServiceException("등록되지 않은 신규 품목이 있습니다. 등록된 품목으로 변경 후 진행하시기 바랍니다.");
		}
		...
		data.param
			...
			.query(" , :invc_numb "  , invc_numb	 )  // Invoice 번호
			.query(" , :deli_date "  , deli_date	 )  // Invoice 번호
			.query(" ) 								")
		;
		data.attach(Action.direct);
		data.execute();
	...
	// 견적서에 신규 품목 건수를 가져온다.
	public long getNewItemCount(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");
		...
		data.param
			...
			.where("and   (a.item_idcd is null or a.item_idcd = '')				")
		;
		return Long.parseLong(data.selectForRow().get("count").toString());
	}

	// 견적서에 납기일자가 없는 품목 건수를 가져온다.
	public long getNotExistDeliDateCount(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");

		data.param
			.query("select if(length(ifnull(deli_date, '')) = 0, 1, 0) as deli_cont	")
		;
		data.param
			.where("from  esti_item a												")
	...
	// 견적서에 신규 품목 건수를 가져온다.
	public SqlResultMap getCheckLastSeqn(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");
		...
		;
		data.param
			.where("from  seqn_dflt a											")
			.where("where a.hqof_idcd = :hqof_idcd",arg.fixParameter("hqof_idcd"))
			.where("and   a.last_idcd = :last_idcd",arg.fixParameter("last_idcd"))
			.where("and   a.line_stat < 2										")
		;
		return data.selectForMap();
	}
	...
	// 견적서에 신규 품목 건수를 가져온다.
	public SqlResultMap getInvoiceNumber(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");

		data.param
			.query("WITH invc_list AS (												")
			.query("     SELECT CAST(SUBSTRING(invc_numb, 4) AS UNSIGNED) AS seqn	")
			.query("     FROM esti_mast												")
			.query("),																")
			.query("missing_numbers AS (											")
			.query("     SELECT a.seqn + 1 AS missing_seqn							")
			.query("     FROM invc_list a											")
			.query("     WHERE NOT EXISTS (											")
			.query("           SELECT 1 FROM invc_list b WHERE b.seqn = a.seqn + 1	")
			.query("     )															")
			.query("     AND a.seqn + 1 < (SELECT MAX(seqn) FROM invc_list)			")
			.query("),																")
			.query("last_number AS (												")
			.query("     SELECT MAX(seqn) AS max_seqn								")
			.query("     FROM invc_list												")
			.query(")																")

			.query("SELECT CASE 													")
			.query("       WHEN (SELECT COUNT(*) FROM missing_numbers) > 0 			")
			.query("       THEN (SELECT MIN(missing_seqn) FROM missing_numbers)		")
			.query("       ELSE (SELECT max_seqn + 1 FROM last_number)				")
			.query("       END AS seqn;												")
		;
		return data.selectForMap();
	}
