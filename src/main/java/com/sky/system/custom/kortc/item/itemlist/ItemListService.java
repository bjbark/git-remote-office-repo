...
@Service("custom.kortc.ItemListService")
public class ItemListService extends DefaultServiceHandler {
	@Autowired
	...
	public SqlResultMap getLookup(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		if(arg.getParamText("hq_id").toUpperCase().equals("N1000NBOLT") || arg.getParamText("mes_system_type").toUpperCase().equals("SJFLV")){
		...
		*/
		...
		if(arg.getParamText("hq_id").toUpperCase().equals("N1000SJFLV")) {
			data.param
				.where(", ifnull((select a.mngt_sbsc_valu from cstm_sbsc a ")
				.where("           where a.mngt_sbsc_idcd = '000008' ")
				.where("             and a.cstm_idcd = :cstm_idcd4 ", arg.getParameter("cstm_idcd"))
				.where("           limit 1), 10) as pack_qntt	")
			;
		}
		data.param
			.where("from   item_mast a																							")
...
//						.query("       left join item_pkge      g on g.item_idcd = a.item_idcd ")
				...
				;
				if(hq.toUpperCase().equals("N1000KORTC")){
