...
public class PurcOrdrService  extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.where("		, b.orig_invc_numb as acpt_numb         , c2.cstm_name as acpt_cstm_name						")
			.where("		, i.item_name																					")
			.where("from    purc_ordr_mast a																				")
			.where("        left outer join (select invc_numb, orig_invc_numb from purc_ordr_item group by invc_numb) b  on a.invc_numb = b.invc_numb		")
			.where("        left outer join cstm_mast      c  on a.cstm_idcd = c.cstm_idcd									")
			.where("        left outer join user_mast      d  on a.drtr_idcd = d.user_idcd									")
			.where("        left outer join acpt_mast      am  on a.prnt_idcd = am.invc_numb								")
			.where("        left outer join acpt_item      ac  on am.invc_numb = ac.invc_numb								")
			.where("        left outer join cstm_mast      c2  on c2.cstm_idcd = am.cstm_idcd								")
			.where("        left outer join item_mast      i   on i.item_idcd = ac.item_idcd								")
			.where("where   1=1																								")
			...
			.where("and     ac.item_idcd   = :item_idcd    " , arg.getParamText("item_idcd" ))
...
//			.where("and     (select min(deli_date) from purc_ordr_item r where r.invc_numb = a.invc_numb) <= :deli2_date" , arg.getParamText("deli2_date" ))
	...
	public SqlResultMap getDetail(HttpRequestArgument arg) throws Exception {
		...
		data.param
			...
			.query("		, a.orig_seqn       , json_value(a.json_data , '$**.stok_used_qntt') as stok_used_qntt	")
			...
			.query("		, u.unit_name       , m.need_qntt												")
			.query("		, round((ifnull(a.offr_qntt,0)*ifnull(i.unit_wigt,0)),2) as ordr_wigt			")
		;
		data.param
			.where("from   purc_ordr_item a																	")
			.where("       left outer join (select sum(need_qntt) as need_qntt, item_idcd, invc_numb		")
			.where("                        from mtrl_need													")
			.where("                        group by item_idcd, invc_numb									")
			.where("                       ) m on m.invc_numb = a.orig_invc_numb and a.item_idcd = m.item_idcd")
	...
	public SqlResultMap getDetail2(HttpRequestArgument arg) throws Exception {
		...
		data.param
			...
			.where("where 1=1 																				")
			.where("order by a.acct_bacd limit 999999999													")
			.where(") a																						")
		;
		return data.selectForMap();
	}
...
//			.query("		, (select max(deli_date) from purc_ordr_item r where r.invc_numb = a.invc_numb) as max_deli	")
	...
	public SqlResultMap getInvoice2(HttpRequestArgument arg) throws Exception {
		...
		data.param
			.query("with item as (																			")
			.query("select distinct item_idcd, invc_numb													")
			.query("from mtrl_need_ordr																		")
			.query(")																						")
			.query("select *																				")
		;
		...
		if(arg.getParamText("invc_numb").equals("")){
			data.param
				...
				.where("            , a.stok_used_qntt  , a.puch_qntt											")
				.where("from   mtrl_need a																		")
				.where("       left outer join item_mast i on a.item_idcd = i.item_idcd							")
				.where("where  1=1																				")

				.where("and    a.item_idcd not in ( select item_idcd from item )								")
				.where("and    a.line_stat   = :line_stat1 "	, arg.getParamText("line_stat" ) , !"".equals(arg.getParamText("line_stat" )))
				.where("and    a.line_stat   < :line_stat  "	, "2" , "".equals(arg.getParamText("line_stat" )))
				.where("and    a.acct_bacd   != :acct_bacd "	, "3000")
				.where("group by a.item_idcd																	")
				.where("order by a.acct_bacd, i.item_name limit 999999999										")
				.where(") a																						")
			;
		}else{
			data.param
				.where("from (select a.acct_bacd       , a.item_idcd       , a.item_spec       , a.item_name	")
				.where("           , p.offr_pric       , p.offr_amnt       , p.offr_vatx       , p.ttsm_amnt	")
				.where("           , p.offr_qntt as puch_qntt              , p.user_memo       , a.item_code	")
				.where("           , a.qntt  as need_qntt                  , if(p.offr_qntt or p.stok_used_qntt,'Y','' ) as modify	")
				.where("           , if(p.stok_used_qntt, p.stok_used_qntt , a.stok_used_qntt) as stok_used_qntt")
				.where("           , if(p.deli_date, p.deli_date, null   )  as deli_date2						")
				.where("from (select  a.acct_bacd      , a.item_idcd       , i.item_spec       , i.item_name	")
				.where("           , i.item_code       , sum(ifnull(a.need_qntt,0)) as qntt						")
				.where("           , a.stok_used_qntt  , a.puch_qntt											")
				...
				.where("      left outer join item_mast i on a.item_idcd = i.item_idcd							")
				.where("      where  1=1																		")
				.where("      and    a.item_idcd not in ( select item_idcd from item where item.invc_numb <> :invc_numb2 )", arg.getParamText("invc_numb"))
				.where("      and    a.line_stat   < 2															")
				.where("      group by a.item_idcd																")
				.where("      order by i.item_name																")
				.where(") a																						")
				.where("left outer join ( select a.item_idcd, a.offr_qntt, a.deli_date, b.stok_used_qntt		")
				.where("                       , a.offr_pric, a.offr_amnt, a.offr_vatx, a.ttsm_amnt				")
				.where("                       , b.puch_qntt, a.user_memo  										")
				.where("                  from purc_ordr_item a													")
				.where("                  left outer join ( select * from mtrl_need_ordr group by invc_numb, line_seqn ) b")
				.where("                                    on a.invc_numb = b.invc_numb and a.line_seqn = b.line_seqn")
				.where("                  where a.invc_numb = :invc_numb			", arg.getParamText("invc_numb"))
				.where("                  and a.line_stat < 2													")
				.where("                ) p on a.item_idcd = p.item_idcd										")
				.where("where 1=1 																				")
				.where("and   a.acct_bacd   != :acct_bacd "	, "3000"											)
				.where("order by a.acct_bacd, a.item_name limit 999999999										")
				.where(") a																						")
			;
		}
		info.get(0).put("product", data.selectForMap());
	...
	public SqlResultMap getInvoice3(HttpRequestArgument arg) throws Exception {
		...
		if(info.size()>0){
			data.param
				.query("select    invc_numb    , a.cstm_idcd    , cstm_name    , drtr_idcd		")
				.query("        , user_name    , invc_date      , deli_date    , stot_dvcd		")
				.query("        , a.prnt_idcd as acpt_numb										")
				.query("from purc_ordr_mast a													")
			...
			data.param
				.query("select  1 as a")
				.query("      , :cstm_idcd as cstm_idcd",arg.getParameter("cstm_idcd"))
				.query("      , :cstm_name as cstm_name",arg.getParameter("cstm_name"))
				.query("      , :drtr_idcd as drtr_idcd",arg.getParameter("drtr_idcd"))
				.query("      , :user_name as user_name",arg.getParameter("user_name"))
				.query("      , :invc_date as invc_date",arg.getParameter("invc_date"))
				.query("      , :deli_date as deli_date",arg.getParameter("deli_date"))
				.query("      , :stot_dvcd as stot_dvcd",arg.getParameter("stot_dvcd"))
			;
		}
		info = data.selectForMap();
		data.clear();
		...
		if(dvcd.equals("modify")){
			data.param
				.query("select   i.item_code     , i.acct_bacd     , i.item_name     , i.item_name     , i.item_spec				")
				.query("       , round(f.need_qntt,2) as need_qntt          , a.item_idcd     , ifnull(a.offr_qntt,0) as puch_qntt	")
				.query("       , round(i.unit_wigt,2) as unit_wigt          , ifnull(a.offr_vatx,0) as offr_vatx					")
				.query("       , ifnull(a.ttsm_amnt,0) as ttsm_amnt         , ifnull(a.offr_pric,0) as offr_pric					")
				.query("       , ifnull(a.offr_amnt,0) as offr_amnt																	")
				.query("       , a.deli_date as deli_date2                  , m.acpt_case_name as modl_name							")
				.query("       , round(ifnull(a.offr_qntt,0)*ifnull(i.unit_wigt,0),2) as ordr_wigt									")
				.query("       , (r.base_qntt- b2.sum) as base_qntt																	")
				.query("       , if(ifnull(round(f.need_qntt,2),0) - ifnull(a.offr_qntt,0) < 0										")
				.query("            , 0 																							")
				.query("            , ifnull(round(f.need_qntt,2),0) - ifnull(a.offr_qntt,0)										")
				.query("       ) as unoffr																							")
				.query("       , json_value(a.json_data , '$**.stok_used_qntt') as stok_used_qntt									")
				.query("")
			;
			data.param
				.where("from purc_ordr_item a																			")
				.where("left outer join purc_ordr_mast b  on a.invc_numb = b.invc_numb									")
				.where("left outer join acpt_mast      m  on b.prnt_idcd = m.invc_numb									")
				.where("left outer join ( select sum(need_qntt) as need_qntt, item_idcd, invc_numb						")
				.where("                  from mtrl_need																")
				.where("                  group by item_idcd, invc_numb													")
				.where("                ) f on f.invc_numb = m.invc_numb and a.item_idcd = f.item_idcd					")
				.where("left outer join item_mast      i  on a.item_idcd = i.item_idcd									")

				.where("left outer join ( select ifnull(sum(json_value(json_data , '$**.stok_used_qntt')),0) as sum		")
				.where("                       , orig_invc_numb, item_idcd												")
				.where("                  from purc_ordr_item															")
				.where("                  group by orig_invc_numb, item_idcd											")
				.where("                 ) b2 on m.invc_numb = b2.orig_invc_numb and a.item_idcd = b2.item_idcd			")

				.where("left outer join ( 																				")
				.where("      select b.item_idcd , ifnull(a.base_qntt,0)+ifnull(b.base_qntt,0) as base_qntt				")
				.where("      from ( select item_idcd, base_qntt + istt_qntt - ostt_qntt as base_qntt, a.clos_yymm		")
				.where("             from   isos_sum a																	")
				.where("             where  a.clos_yymm																	")
				.where("             in ( select ifnull(max(r.clos_yymm),'201901') as clos_yymm							")
				.where("                  from   isos_sum r																")
				.where("                  where  a.item_idcd = r.item_idcd												")
				.where("                  group by r.item_idcd															")
				.where("             )																					")
				.where("      ) a right outer join ( select sum(base_qntt) as base_qntt,invc_date,item_idcd				")
				.where("                             from(  select a.bzpl_idcd , a.wrhs_idcd , a.item_idcd				")
				.where("                                  , case invc_dvcd when '1100' then a.qntt        -- 구매입고	")
				.where("                                    when '1200' then a.qntt        -- 생산입고					")
				.where("                                    when '1300' then a.qntt        -- 기타입고					")
				.where("                                    when '1400' then a.qntt        -- 반품입고					")
				.where("                                    when '1500' then a.qntt        -- 이동입고					")
				.where("                                    when '2100' then a.qntt * -1   -- 생산출고					")
				.where("                                    when '2200' then a.qntt * -1   -- 판매출고					")
				.where("                                    when '2300' then a.qntt * -1   -- 기타출고					")
				.where("                                    when '2400' then a.qntt * -1   -- 이동출고					")
				.where("                                    when '2500' then a.qntt * -1   -- 재고조정					")
				.where("                                    when '2600' then a.qntt * -1   -- 자가소비					")
				.where("                                    when '2700' then a.qntt * -1   -- 출고취소					")
				...
				.where("and   substr(i.acct_bacd,1,1) in (1)															")
				.where("order by i.acct_bacd, a.line_seqn																")
			;
		}else{
			data.param
				.query("select * from (																				")
				.query("select     i.item_code     , a.acct_bacd      , i.item_name     , i.item_spec				")
				.query("         , ifnull(sum(round(a.need_qntt,2)),0) as need_qntt     , a.invc_numb				")
				.query("         , i.item_idcd     , 0 as puch_qntt   , m.acpt_case_name as modl_name				")
				.query("         , null as offr_amnt																")
				.query("         , null as offr_vatx                  , null as ttsm_amnt							")
				.query("         , a.line_seqn                        , i.unit_wigt									")
				.query("         , :deli_date as deli_date2				",arg.getParameter("deli_date"))
				.query("         , if(ifnull(sum(round(a.need_qntt,2)),0) - ifnull(b.offr_qntt,0) < 0 , 0 "
								+ ", ifnull(sum(round(a.need_qntt,2)),0) - ifnull(b.offr_qntt,0)) as unoffr			")
				.query("         , (r.base_qntt - b2.sum) as base_qntt												")
//				.query("         , pric.cont_pric as offr_pric														")
			;
			data.param
				.where("from mtrl_need a																			")
				.where("left outer join acpt_mast      m on a.invc_numb = m.invc_numb								")
				.where("left outer join item_mast      i on a.item_idcd = i.item_idcd								")
				.where("left outer join ( select invc_numb, line_seqn, sum(offr_qntt) as offr_qntt, orig_invc_numb, item_idcd			")
				.where("                  from purc_ordr_item														")
				.where("                  group by orig_invc_numb, item_idcd										")
				.where("                 ) b on a.invc_numb = b.orig_invc_numb and a.item_idcd = b.item_idcd		")

				.where("left outer join ( select ifnull(sum(json_value(json_data , '$**.stok_used_qntt')),0) as sum	")
				.where("                       , orig_invc_numb, item_idcd											")
				.where("                  from purc_ordr_item														")
				.where("                  group by orig_invc_numb, item_idcd										")
				.where("                 ) b2 on a.invc_numb = b2.orig_invc_numb and a.item_idcd = b2.item_idcd		")
				...
				.where("      ) b on b.item_idcd  = a.item_idcd and b.invc_date >= ifnull( a.clos_yymm,'20190101')		")
				.where(") r on a.item_idcd = r.item_idcd														")
//				.where("left outer join ( select a.cont_pric, a.item_idcd										")
//				.where("                  from item_cont a														")
//				.where("                  where 1=1																")
//				.where("                  and a.last_yorn = '1'													")
//				.where("                  and a.cstm_idcd = :cstm_idcd		",arg.fixParameter("cstm_idcd"))
//				.where("                ) pric on a.item_idcd = pric.item_idcd									")


				.where("where a.invc_numb = :acpt_numb		",arg.fixParameter("acpt_numb"))
				.where("and   substr(a.acct_bacd,1,1) in (1)														")
				.where("group by a.item_idcd																		")
				.where("having need_qntt > 0																		")
				.where("and need_qntt - puch_qntt > 0																")
				.where(") a																							")
				.where("where 1=1																					")
				.where("and   unoffr > 0																			")
				.where("order by a.acct_bacd, a.line_seqn															")
			;
		}
	...
	public SqlResultMap getSubItem(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("            , a.acct_bacd       , a.item_idcd       , i.item_widh       , i.item_leng			")
			.where("            , b.need_qntt       , a.exng_qntt       , a.stok_qntt       , a.puch_qntt			")
			.where("            , a.ostt_yorn       , a.ostt_date       , a.pqty_ndqt								")
			.where("            , a.ostt_numb       , a.uper_seqn       , a.disp_seqn       , a.user_memo			")
			.where("            , a.stok_used_qntt  , i.item_name       , i.item_code       , i.item_spec			")
			.where("            , a.revs_numb       , a.drwg_numb													")
			.where("from ( select prnt_idcd,invc_numb,need_qntt														")
			.where("       from mtrl_need																			")
			...
			.where("     ) b 																						")
			.where("left outer join mtrl_need a on a.item_idcd = b.prnt_idcd and a.invc_numb = b.invc_numb			")
			.where("left outer join item_mast i on a.item_idcd = i.item_idcd										")
...
//				}
				...
				data.param
					...
					.update("amnd_degr"			, row.getParameter("amnd_degr"		))	//amend
		...
		for (SqlResultRow row:map) {
			...
			if (rowaction == Action.delete) {
				...
				data.param
					...
					.update("remk_text"			, row.getParameter("remk_text"		))	//비고
					.update("user_memo"			, row.getParameter("user_memo"		))	//사용자메모
					.update("prnt_idcd"			, row.getParameter("acpt_numb"		))	//상위 ID
					.update("line_ordr"			, row.getParameter("line_ordr"		))	//ROW순서
					.update("line_stat"			, row.getParameter("line_stat"		))	//ROW상태
					.update("line_clos"			, row.getParameter("line_clos"		))	//마감여부
					.update("find_name"			, row.getParamText("invc_numb"		).trim()
	...
	public void setInvoiceDetail2(HttpRequestArgument arg, DataMessage data, SqlResultRow mst, SqlResultMap map,String amd, Action rowaction) throws Exception {
		...
		for (SqlResultRow row:map) {
			...
			if(!row.getParamText("_set").equals("") && !row.getParamText("puch_qntt").equals("0")){
				deli_date2 = row.getParamText("deli_date2");
				...
				if (rowaction == Action.delete) {
					...
					data.param
						...
						.update("item_spec"			, row.getParameter("item_spec"		))		//invoice일자
						.update("unit_idcd"			, row.getParameter("unit_idcd"		))		//단위ID
						.update("offr_qntt"			, row.getParamText("puch_qntt"		))		//발주수량
						.update("offr_pric"			, row.getParameter("offr_pric"		))		//발주단가
						.update("vatx_rate"			, row.getParameter("vatx_rate"		))		//부가세율
						.update("offr_amnt"			, row.getParameter("offr_amnt"		))		//발주금액
						.update("offr_vatx"			, row.getParameter("offr_vatx"		))		//발주부가세
						.update("ttsm_amnt"			, row.getParameter("ttsm_amnt"		))		//발주금액계
						.update("orig_invc_numb"	, mst.getParameter("acpt_numb"		))		//주문번호
						.update("deli_date"			, deli_date2						)		//납기일자
					...
					data.param
						.where("from (select  a.invc_numb       , a.amnd_degr       , a.acpt_seqn       , a.line_seqn			")
						.where("            , a.need_qntt       , a.item_idcd       , a.acct_bacd								")
						.where("from mtrl_need a																				")
						.where("where		1=1																					")
						.where("and			a.item_idcd	= :item_idcd		" ,row.getParameter("item_idcd"		))
						.where("and			a.line_stat	< :line_stat		" , "2" , "".equals(arg.getParamText("line_stat")))
						.where("order by	a.line_seqn	limit 99999999) a														")
					;
					SqlResultMap info = data.selectForMap();
					...
					for (SqlResultRow row2:info) {
						if(!(row2.getParamText("invc_numb").equals("")) && !(row2.getParamText("acpt_seqn").equals(""))){
							data.param
								.table ("mtrl_need_ordr")
								...
								.update("acpt_numb"        , row2.getParameter("invc_numb"))
								.update("acpt_amnd_degr"   , row2.getParameter("amnd_degr"))
								.update("acpt_seqn"        , row2.getParameter("acpt_seqn"))
								.update("need_seqn"        , row2.getParameter("line_seqn"))
								.update("acct_bacd"        , row2.getParameter("acct_bacd"))
								.update("item_idcd"        , row2.getParameter("item_idcd"))
								.update("need_qntt"        , row2.getParameter("need_qntt"))
								.update("stok_used_qntt"   , row.getParameter("stok_used_qntt"))
								.update("puch_qntt"        , row.getParameter("puch_qntt"))
							...
							data.execute();
							data.clear();
//							if(!row.getParameter("stok_used_qntt").equals("")){
