...
@Service("iypkg.IsttWork1Service")
public class IsttWork1Service extends DefaultServiceHandler {
	@Autowired
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.where("          , c.cstm_name       , a.item_idcd as fabc_idcd              , ifnull(p.item_name,f.fabc_name) as fabc_name	")
			.where("          , f.fabc_code       , f.ppln_dvcd       , bm.fdat_spec      , a.istt_qntt								")
			.where("          , a.istt_amnt       , a.istt_vatx       , a.ttsm_amnt       , ba.scre_spec_frml						")
			...
			.where("          , bm.item_leng      , bm.item_widh      , a.istt_qntt as istt_qntt2									")
			.where("          , concat(ifnull(bm.item_leng,'0'),'*',ifnull(bm.item_widh,'0')) as item_spec							")
			...
			.where("     left outer join purc_ordr_mast pm on p.invc_numb = pm.invc_numb and json_value(pm.json_data, '$**.offr_path_dvcd')=1 		")
	...
	public SqlResultMap getSearch3(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("		, f.fabc_name       , f.fabc_code              , f.ppln_dvcd						")
			.where("		, ifnull(ba.item_ttwd,bm.item_ttwd)as item_ttwd             , ifnull(ba.item_fxqt,bm.item_fxqt) as item_fxqt ")
			.where("		, ifnull(ba.item_ttln,bm.item_ttln) as item_ttln , ba.fdat_spec as scre_spec      , a.offr_qntt	")
			.where("		, ifnull(pi.istt_qntt,0) as istt_qntt   , (a.offr_qntt - ifnull(pi.istt_qntt,0)) as unistt		")
			.where("		, json_value(a.json_data , '$**.mxm2_qntt') as mxm2_qntt    , ifnull(ba.item_widh,bm.item_widh) as item_widh	")
			.where("		, json_value(a.json_data , '$**.mxm2_pric') as mxm2_pric    , a.orig_seqn as acpt_seqn			")
			.where("		, json_value(a.json_data , '$**.pqty_pric') as pqty_pric    , a.orig_seqn as boxx_bom_seqn		")
			.where("		, a.orig_invc_numb as acpt_numb         , c.cstm_name as acpt_cstm_name							")
			.where("		, null as vatx_incl_yorn                , null as istt_amnt , null as ttsm_amnt					")
			.where("		, null as istt_vatx , null as istt_qntt2, null as subt_qntt										")
			.where("		, bt.prod_idcd      , bt.prod_name      , b.cstm_idcd       , c2.cstm_name						")
			...
			.where("   left outer join product_bom    bm on bt.prod_idcd = bm.prod_idcd and a.line_seqn = bm.line_seqn		")
			...
			.where("             , sum(json_value(json_data,'$**.subt_qntt')) as subt_qntt									")
			.where("        from purc_istt_item group by orig_invc_numb, orig_seqn											")
			.where("      ) pi on a.invc_numb = pi.orig_invc_numb and a.line_seqn = pi.orig_seqn							")
			.where("where  1=1																								")
			.where("and    json_value(b.json_data , '$**.offr_path_dvcd') = 1												")
			.where("and    a.offr_qntt - (ifnull(pi.istt_qntt,0)+ifnull(subt_qntt,0)) > 0									")
	...
	public SqlResultMap setRecord(HttpRequestArgument arg) throws Exception {
		...
		for (SqlResultRow row:map) {
			...
			temp.param
				...
				.query("	, a.item_ttln    , a. item_widh    , a.item_fxqt			")
			...
			if(row.fixParamText("new_line_seqn").equals("1")){
				...
				data.param
					...
					.update("acct_bacd"        , 1001						)			//계정구분코드 = 원재료
				...
				data.param
					...
					.unique("line_seqn"        , row.fixParameter("boxx_bom_seqn"))

					.update("istt_yorn"        , 1)								//입고여부
					.update("istt_date"        , row.getParameter("istt_date"))	//입고일자
					.update("istt_qntt"        , istt_qntt)						//입고수량
					.update("item_ttln"        , row.getParameter("item_ttln"))	//장
					.update("item_widh"        , row.getParameter("item_widh"))	//폭
					.update("item_fxqt"        , row.getParameter("item_fxqt"))	//절수
					.update("cstm_idcd"        , row.getParameter("cstm_idcd"))	//입고거래처ID
				;
...
//				sequence.setBook(arg, row.getParamText("new_invc_numb"), 0 , "구매입고");
	...
	public SqlResultMap setRecord2(HttpRequestArgument arg) throws Exception {
		...
		for (SqlResultRow row:map) {
			...
			.unique("invc_numb"        , row.fixParameter("acpt_invc_numb"))
			.unique("line_seqn"        , row.fixParameter("acpt_line_seqn"))
...
//		}
	...
	public SqlResultMap getCalc(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");
		data.param
			.query("call  ostt_product_calc(					")
	...
	public SqlResultMap getSearchPopup(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");

		data.param
			.query("call  stock_istt_list_popup2(					")
			.query("   :invc_numb	",	arg.fixParameter("invc_numb"))
			.query(")												")
		;
		return data.selectForMap();
