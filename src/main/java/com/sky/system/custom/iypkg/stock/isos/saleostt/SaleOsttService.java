...
public class SaleOsttService extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("left outer join cstm_mast      c on s.dlvy_cstm_idcd = c.cstm_idcd							")
...
//			.where("and     s.item_idcd  <= :item_idcd2       " , arg.getParamText("item_idcd2" ))
	...
	public SqlResultMap getSearch3(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			.where("from (																						")
			.where("select    ai.invc_date     , ai.invc_numb as acpt_numb           , p.prod_name				")
			...
			.where("        , p.prod_leng      , c.cstm_name      , pm.prog_stat_dvcd							")
			...
			.where("left outer join pror_mast   pm on pm.acpt_numb = ai.invc_numb								")
			.where("where   1=1																					")
			.where("and     a.line_stat < 2																		")
			.where("and     (ifnull(a.trst_qntt,0) - ifnull(s.ostt_qntt,0)) > 0									")
			.where("and     ai.acpt_qntt > 0																	")
		...
		for (SqlResultRow row:map) {
			...
			if(i == 0){
				...
				data.param
					...
					.update("dlvy_cstm_idcd"   , row.getParameter("cstm_idcd"))			//거래처ID
...
////		String updt_dttm	= arg.getParamText("updt_dttm");
//
//		for (SqlResultRow row:map) {
	...
	public SqlResultMap setModify(HttpRequestArgument arg) throws Exception {
		...
		for (SqlResultRow row:map) {
			...
			if (rowaction == Action.delete) {
				...
				if(row.getParamText("acpt_numb") != null && !row.getParamText("acpt_numb").isEmpty()){
					data.param
						.table ("boxx_acpt")
						.where ("where invc_numb = :invc_numb")
...
//				data.clear();
				...
				data.param
					.query ("update boxx_acpt set line_clos = '1'																					")
					.query ("where  acpt_qntt <= (	select sum(r.ostt_qntt) + sum(ifnull(json_value(r.json_data , '$*.porm_qntt'),0)) as ostt_qntt	")
					.query ("						from   sale_ostt_item r																			")
					.query ("						where  r.acpt_numb = :acpt_numb1	",row.fixParameter("acpt_numb"))
					.query ("						group by r.acpt_numb																			")
					.query ("					)																									")
					.query ("and    invc_numb = :acpt_numb2",row.fixParameter("acpt_numb"))
				;
				data.attach(Action.direct);
				data.execute();
				data.clear();
