...
public class SptsMastService extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.query("		, a.invc_date as invc_date2             , a.item_idcd         , b.prod_name    , b.acpt_qntt	")
...
//			.query("		, (select lcal_name from dely_lcal l where (json_value(c.json_data,'$.lcal_idcd')) = l.lcal_idcd) as lcal_name	")
			...
			.where("    left outer join car_mast  c3 on a.cars_idcd = c3.cars_idcd											")
			.where("where  1=1																								")
...
//			.where("and    b.deli_date  <= :deli_date2				" , arg.getParamText("deli_date2"))
			...
			.where("and    a.acpt_numb   = :acpt_numb				" , arg.getParamText("invc_numb" ))
			.where("and    a.line_stat   < :line_stat		" , "2" , "".equals(arg.getParamText("line_stat" )))
			.where("order by a.disp_seqn asc																				")
		;
		if (page == 0 && rows == 0){
	...
	public SqlResultMap getSearch2(HttpRequestArgument arg, int page, int rows, String sort ) throws Exception {
		...
		data.param
			...
			.where("    left outer join (select sum(ifnull(a.trst_qntt,0)) as trst_qntt, a.acpt_numb , a.line_stat		")
			.where("                     from ostt_plan a 																")
...
//			.query("		, (select lcal_name from dely_lcal l where (json_value(c.json_data,'$.lcal_idcd')) = l.lcal_idcd) as lcal_name	")
			...
			.query("		, json_value(a.json_data , '$**.dlvy_cstm_name')   as dlvy_cstm_name							")
			.where("from   ostt_plan a																						")
			.where("    join (select invc_numb from boxx_acpt																")
			.where("                           where  1=1																	")
	...
	public SqlResultMap setRecord(HttpRequestArgument arg) throws Exception {
		...
		for(SqlResultRow row:arg.getParameter("records", SqlResultMap.class)){
			...
			data.param
				...
				.update("invc_date"			, row.getParameter("invc_date2"			))
				.update("acpt_numb"			, row.getParameter("acpt_numb"			))
				.update("item_idcd"			, row.getParameter("prod_idcd"			))
				.update("trst_qntt"			, row.getParameter("trst_qntt"			))
