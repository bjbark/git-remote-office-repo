...
@Service("iypkg.WorkBookV1service")
...
public class WorkBookV1Service extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("        , i.prod_leng        , i.prod_widh       , i.prod_hght        , i.prod_idcd			")
			.where("        , bx.bxty_name       , json_value(a.json_data , '$**.pkg_qntt') as pkg_qntt			")
			.where("        , json_value(ac.json_data , '$**.stat_dvcd') as stat_dvcd							")
			.where("        , a.pref_rank        , cv.cvic_name													")
			.where("        , ( select concat(bm.item_ttln,'*',bm.item_ttwd)									")
			.where("            from   boxx_acpt_bom bm															")
			.where("            where bm.invc_numb = b.acpt_numb limit 1										")
			.where("        ) as bom_spec																		")
			.where("        , ( select f.fabc_idcd 																")
			.where("            from boxx_acpt_bom bm															")
...
//			.where("and     a.line_stat = 1																		")
			.where("and     ac.invc_numb is not null															")
	...
	public SqlResultMap getSearchDetail(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("     , u.user_name as wker_name             , i.prod_name       , a.prog_stat_dvcd		")
			.where("     , c.cvic_code      , c.cvic_name       , m.mold_code       , m.mold_name			")
			.where("     , p2.acpt_numb     																")
			.where("     , i.prod_code      , m.cavity          , m.totl_shot								")
			...
			.where("     , i.prod_leng      , i.prod_widh       , i.prod_hght        						")
			.where("     , bx.bxty_name     , c2.cstm_name													")
			...
			.where("       left outer join cstm_mast c2 on p1.cstm_idcd   = c2.cstm_idcd						")
			...
			.where("and     (( substr(a.work_strt_dttm,1,8)														")
			.where("          between STR_TO_DATE(:invc_date1,'%Y%m%d') " , arg.fixParameter("invc_date1"))
			.where("          and STR_TO_DATE( :invc_date2,'%Y%m%d')    " , arg.fixParameter("invc_date2"))
			.where("        ) or																				")
			.where("        ( substr(a.work_endd_dttm,1,8)														")
			.where("          between STR_TO_DATE(:invc_date3,'%Y%m%d') " , arg.fixParameter("invc_date1"))
			.where("          and STR_TO_DATE( :invc_date4,'%Y%m%d')    " , arg.fixParameter("invc_date2"))
			.where("        ))																					")
...
//			.where("and     a.line_stat = 1																		")
	...
	public SqlResultMap getSearchDetail3(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("        , @curRank:=@curRank+1 as seqn														")
			.where("        , i.prod_leng        , i.prod_widh       , i.prod_hght        , i.prod_idcd			")
			.where("        , bx.bxty_name       , w.wkct_name													")
			.where("from    pror_item a																			")
			.where("left outer join pror_mast b on a.invc_numb = b.invc_numb									")
			.where("left outer join cstm_mast c on a.cstm_idcd = c.cstm_idcd									")
			.where("left outer join product_mast i on a.item_idcd = i.prod_idcd									")
			...
			.where("where   1=1																					")
//			.where("and     a.line_stat = 1																		")
			.where("and     a.prog_stat_dvcd in('0')															")
			.where("and     (( substr(a.plan_strt_dttm,1,8)														")
			.where("          between STR_TO_DATE(:invc_date1,'%Y%m%d') " , arg.fixParameter("invc_date1"))
			.where("          and STR_TO_DATE( :invc_date2,'%Y%m%d')    " , arg.fixParameter("invc_date2"))
			.where("        ) or																				")
		...
		for (SqlResultRow row : map) {
			if(set.equals("insert")){
				...
				data.param
					...
					.update("prog_stat_dvcd"	, dvcd)
					.update("cvic_idcd"			, row.getParameter("crte_idcd"))
