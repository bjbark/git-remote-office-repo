...
@Service("nbolt.GoodsOsttWorkService")
public class GoodsosttWorkService extends DefaultServiceHandler {
	...
	public SqlResultMap getMaster1(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("order by a.invc_date desc , invc_numb desc																	")
			.where(") a																							")
		;
		if (page == 0 && rows == 0){
	...
	public SqlResultMap getWorkerlister1(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		DataMessage data = arg.newStorage("POS");
		...
		data.param
			...
			.query("        , a.updt_user_name   , a.updt_ipad       , a.updt_dttm        , a.updt_idcd									")
			.query("        , a.updt_urif        , a.crte_user_name  , a.crte_ipad        , a.crte_dttm									")
			.query("        , i.item_code        , i.item_name       , i.item_spec        , ifnull(a.wrhs_idcd,r.wrhs_idcd) as wrhs_idcd")
	...
	public SqlResultMap setRecords(HttpRequestArgument arg) throws Exception {
		...
		ArrayList<String> invc_numb = new ArrayList<String>();

		for(SqlResultRow row:map) {
			if(!row.getParamText("_set").equals("delete")){
				if(row.getParamText("new_line_seqn").equals("1")){
					...
					data.execute();
					data.clear();
					invc_numb.add(row.fixParamText("new_invc_numb"));
				}
				data.param
		...
		for(String invc : invc_numb){
			sequence.setBook(arg, invc , 0 , "판매출고");
	...
	public SqlResultMap deleteMaster(HttpRequestArgument arg) throws Exception {
		...
		DataMessage data = arg.newStorage("POS");
		SqlResultMap rec = arg.getParameter("records", SqlResultMap.class);
		//2022.04.25 - 이강훈 - 수주진행상태를 처리하기 위하여 삭제 전 출고에  등록된 수주번호를 가져온다.
		// 2022.12.20 - 장우영 - 다중삭제가능하게 변경
		for(SqlResultRow item : rec){
			...
			data.param
				...
				.where("where invc_numb = :invc_numb2				" ,item.fixParameter("invc_numb"))
			...
			for (SqlResultRow row:map) {
				String ostt_qntt = "0";
				if(!row.getParamText("ostt_qntt").equals("")){
					ostt_qntt = row.getParamText("ostt_qntt");
				}
				data.param
					.query("update acpt_item set ostt_qntt = (ostt_qntt - :ostt_qntt)",ostt_qntt)
					.query("where invc_numb = :invc_numb", row.fixParameter("acpt_numb"))
					.query("and   line_seqn = :line_seqn", row.fixParameter("acpt_seqn"))
				;
