package com.sky.system.custom.symct.prod;
...
public class WorkEntryService extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			.where("from (																					")
			.where("select																					")
			.where("      a.pjod_idcd        , a.line_seqn        , a.item_idcd        , a.item_name		")
			.where("    , a.item_spec        , a.modl_name        , a.drwg_numb        , a.mtrl_dvcd		")
			.where("    , a.acpt_qntt        , a.esti_pric        , a.esti_amnt        , a.cofm_yorn		")
			.where("    , a.cofm_date        , a.cofm_pric        , a.cofm_amnt        , a.sply_amnt		")
			.where("    , a.vatx             , a.ttsm_amnt        , a.deli_date        , a.ppsl_deli_date	")
			.where("    , a.supl_dvcd        , a.prod_cofm_yorn   , a.imge_1fst        , a.imge_2snd		")
			.where("    , a.prod_yorn        , a.prod_date        , a.user_memo        , a.sysm_memo		")
			.where("    , m.pjod_code        , ifnull(w.prod_qntt,0) as prod_qntt      , a.line_stat		")
			.where("    , w.wkct_idcd																		")
			.where("from    pjod_item a																		")
			.where("        left outer join pjod_mast m on a.pjod_idcd = m.pjod_idcd						")
			.where("        left outer join pjod_work_book w on a.pjod_idcd = w.pjod_idcd					")
			.where("                                        and a.line_seqn = w.line_seqn					")
			.where("        left outer join ( select pjod_idcd,line_seqn,prog_stat_dvcd from pjod_work_book	")
			.where("                          where prog_stat_dvcd = '3'									")
			.where("        ) r on r.pjod_idcd = a.pjod_idcd and r.line_seqn = a.line_seqn					")
			.where("where   1=1																				")
			.where("and     m.line_clos  <> 1																")
			.where("and     r.pjod_idcd is null																")
			.where("and		m.line_stat  < :line_stat"		, "2" , "".equals(arg.getParamText("line_stat" )))
	...
	public SqlResultMap getSearchDetail2(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("    , m.pjod_code        , ifnull(w.prod_qntt,0) as prod_qntt      , a.line_stat		")
			.where("    , w.wkct_idcd		")
			.where("from    pjod_work_book w																")
			.where("        left outer join pjod_item a	 on a.pjod_idcd = w.pjod_idcd						")
			.where("                                        and a.line_seqn = w.line_seqn					")
			.where("        left outer join pjod_mast m on a.pjod_idcd = m.pjod_idcd						")
			.where("where   1=1																				")
			.where("and     m.line_clos <> 1 																")
	...
	public SqlResultMap setMaster(HttpRequestArgument arg) throws Exception {
		...
		for (SqlResultRow row : map) {
			...
			if(!row.getParamText("work_edtm").equals("")){
				edtm = row.getParamText("work_edtm")+"00";
			}
	...
	public Object setComplete(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		DataMessage data = arg.newStorage("POS");
		...
		if(rowaction == Action.delete){
			...
			data.param
				...
				.update("work_ordr_dvcd"	, arg.getParameter("work_ordr_dvcd"))
				.update("idcd"				, arg.getParameter("idcd"))
				.update("name"				, arg.getParameter("name"))
				.update("progress"			, arg.getParameter("progress"))
				...
				.update("work_mnhr"			, arg.getParameter("work_mnhr"))
				.update("work_pcnt"			, arg.getParameter("work_pcnt"))
				.update("work_cond_1fst"	, arg.getParameter("work_cond_1fst"))
				.update("work_cond_2snd"	, arg.getParameter("work_cond_2snd"))
				.update("work_cond_3trd"	, arg.getParameter("work_cond_3trd"))
				.update("work_cond_5fit"	, arg.getParameter("work_cond_5fit"))
				.update("work_cond_6six"	, arg.getParameter("work_cond_6six"))
				.update("work_cond_7svn"	, arg.getParameter("work_cond_7svn"))
				.update("work_dvcd"			, arg.getParameter("work_dvcd"))
				.update("wkct_insp_yorn"	, arg.getParameter("wkct_insp_yorn"))
				.update("last_wkct_yorn"	, arg.getParameter("last_wkct_yorn"))
				.update("work_para"			, arg.getParameter("work_para"))
