...
//				.where("		, ifnull(a.invc_qntt,0) - ifnull(a.trst_qntt1,0) as spts_qntt									")
			...
			.where("         , a.orig_invc_numb , a.orig_seqn      , a.orig_invc_qntt , a.optn_dvcd      , a.optn_psbl_yorn	")
			.where("         , a.optn_adtn      , a.pric_adpt      , a.norm_sale_pric , a.sale_stnd_pric , a.invc_qntt		")
			.where("         , a.invc_pric      , a.vatx_incl_yorn , a.vatx_rate      , a.cstm_lott_numb					")
			.where("         , a.krwn_amnt      , a.krwn_vatx      , a.krwn_ttsm_amnt , a.stnd_unit							")
			.where("         , a.stnd_unit_qntt , a.wrhs_idcd      , m.dlvy_cstm_idcd , a.deli_date      , a.dlvy_date		")
			.where("         , a.dlvy_hhmm      , a.remk_text      , a.ostt_dvcd      , a.dsct_qntt      , a.dlvy_memo		")
			.where("         , a.uper_seqn      , a.disp_seqn      , a.user_memo      , m.pcod_numb      , m.drtr_idcd		")
			.where("         , b.item_code      , b.item_name      , b.item_spec      , u.unit_name      , m.acpt_stat_dvcd	")
			.where("         , m.invc_date      , m.cstm_idcd      , m.line_clos      , c.cstm_name      , c.cstm_code		")
			.where("         , s.trst_qntt as trst_qntt1           , ifnull(p.prog_stat_dvcd,0) as prog_stat_dvcd			")
			.where("         , pi.lott_numb																					")
			.where("   from   acpt_item a																					")
			...
			.where("          left outer join pror_item pi on p.invc_numb = pi.invc_numb									")
			.where("          left outer join cstm_mast c on c.cstm_idcd = m.cstm_idcd										")
			.where("   where  1=1																							")
			.where("   and     m.invc_date >= :invc_date1       " , arg.getParamText("invc1_date" ))
			.where("   and     m.invc_date <= :invc_date2       " , arg.getParamText("invc2_date" ))
			...
			.where("   and     ifnull(p.prog_stat_dvcd,0) <> '2' 															")
			.where("   and     a.line_stat   < :line_stat	" , "2" , "".equals(arg.getParamText("line_stat" )))
	...
	public SqlResultMap setInvoice2(HttpRequestArgument arg) throws Exception {
		...
		for (SqlResultRow row:map) {
			...
			System.out.println(row.getParameter("_drtr_idcd"	));
			...
			data.param
				...
				.update("ttsm_amnt"			, row.getParameter("invc_amnt"))
