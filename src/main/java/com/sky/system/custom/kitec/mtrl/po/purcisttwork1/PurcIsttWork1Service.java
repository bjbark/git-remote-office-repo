...
@Service
public class PurcIsttWork1Service extends DefaultServiceHandler {
	@Autowired
	...
	public SqlResultMap getMaster(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.where("where   1=1																				")
			.where("and     (b.lott_numb <> null or b.lott_numb <> '')										")
			.where("and     c.sysm_memo in ('1','3')										")
		...
		for(SqlResultRow row:arg.getParameter("records", SqlResultMap.class)){
			...
			if(diff_qntt == 0){
				data.param
					...
					.query("set  dlvy_qntt  = dlvy_qntt + :istt_qntt ", row.getParameter("istt_qntt"			))
					.query("where invc_numb = :invc_numb  ", row.getParameter("invc_numb"			))
					.query("and line_seqn   = :line_seqn  ", row.getParameter("line_seqn"			))
					.query("and amnd_degr   = :amnd_degr  ", row.getParameter("amnd_degr"			))
				;
	...
	public SqlResultMap getBundle(HttpRequestArgument arg, int page, int rows, String sort) throws Exception {

		DataMessage data = arg.newStorage("POS");

		data.param // 집계문  입력
		.query("call calc_bundle (	")
		.query(" :param	" , arg.getParamText("param"))
		.query(" )	")
		;
