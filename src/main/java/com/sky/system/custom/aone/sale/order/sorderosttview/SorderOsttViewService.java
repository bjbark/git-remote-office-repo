...
@Service("aone.SorderOsttViewSerivce")
...
//조회2
	public SqlResultMap getMaster2(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		data.param
			...
			.query("( select substr(a.dlvy_date,1,6) as dlvy_date, sum(a.invc_qntt*a.invc_pric) as invc_amnt , count(a.invc_numb) as ostt_qntt	")
...
//조회3
		...
		//조회4
				public SqlResultMap getMaster4(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
					...
					;
					data.param
						.query("select   u.user_name      , count(a.invc_numb) as ostt_qntt										")
						.query("       , (a.invc_qntt * a.invc_amnt) as invc_amnt 											")
					...
					data.param
						...
						.where("and   u.find_name like %:find_name% ", arg.getParameter("find_name"))
						.where("and   b.line_clos = :line_clos ", arg.getParameter("line_clos"))
						.where("group by json_value(a.json_data,'$.prod_drtr_idcd')												")
					;
