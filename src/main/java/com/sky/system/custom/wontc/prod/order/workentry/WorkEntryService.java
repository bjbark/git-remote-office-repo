...
@Service("wontc.WorkEntryService")
public class WorkEntryService extends DefaultServiceHandler {
	...
	public SqlResultMap getSearch3(HttpRequestArgument arg , int page, int rows, String sort) throws Exception {
		...
		data.param
			.where("from(																								")
			.where("select	  p.invc_numb      , p.item_idcd      , p.indn_qntt											")
			.where("        , m.regi_date      , c.cstm_name      , i.item_name											")
			.where("        , p.sysm_memo      , p.prnt_idcd      , p.line_levl											")
			.where("        , p.line_ordr      , p.line_stat      , p.line_clos      , p.find_name						")
			.where("        , p.updt_user_name , p.updt_ipad      , p.updt_dttm      , p.updt_idcd						")
			.where("        , p.updt_urif      , p.crte_user_name , p.crte_ipad      , p.crte_dttm						")
			.where("        , p.crte_idcd      , p.crte_urif      , m.modl_name	     , m.deli_date						")
			.where("from	pror_mast p																					")
			.where("left outer join pjod_mast m on p.acpt_numb = m.pjod_idcd											")
			...
			.where("and		p.line_stat	< :line_stat"				, "2" , "".equals(arg.getParamText("line_stat" )))
			.where(") a																									")
		;
		if (page == 0 && rows == 0 ) {
...
//			.where("		, w.good_qntt         , w.poor_qntt        , u.user_name        , w.invc_date				")
			...
			.where("				   and a.line_seqn = r.wkod_seqn and prog_stat_dvcd = 3								")
			.where("				),0) as chk_prog																	")
			.where("from	pror_item a																					")
			.where("        left outer join work_book w on a.invc_numb = w.wkod_numb and a.line_seqn = w.wkod_seqn		")
			.where("        left outer join user_mast u on w.wker_idcd = u.user_idcd									")
	...
	public SqlResultMap getMtrlStock(HttpRequestArgument arg) throws Exception {
		...
		data.param
			.where("from (																						")
			.where("select  a.invc_numb        , b.need_qntt       , i.item_name       , i.item_code			")
			.where("      , i.item_spec        , r.base_qntt													")
			.where("from pror_mast a ")
			.where("left outer join pjod_bom  b on a.acpt_numb = b.pjod_idcd and a.item_idcd = b.prnt_idcd		")
			.where("left outer join item_mast i on b.item_idcd = i.item_idcd									")
			...
			.where("where 1=1																					")
			.where("and   substr(i.acct_bacd,1,1) = 1															")
	...
	//검사확인
	public SqlResultMap getWkfw(HttpRequestArgument arg) throws Exception {
		DataMessage data = arg.newStorage("POS");
		data.param
			.query("select b.wkct_insp_yorn 																")
			.where("from pror_item a  																		")
			.where("left outer join wkfw_rout b on a.wkfw_idcd = b.wkfw_idcd and a.wkct_idcd = b.wkct_idcd	")
			.where("where  a.invc_numb = :invc_numb"		,arg.fixParameter("invc_numb"))
			.where("and    a.wkct_idcd = :wkct_idcd"		,arg.fixParameter("wkct_idcd"))
		;
		return data.selectForMap();
	}
