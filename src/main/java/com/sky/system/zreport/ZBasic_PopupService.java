...
@Service
public class ZBasic_PopupService extends DefaultServiceHandler {
	final Logger logger = LoggerFactory.getLogger(this.getClass());


	public SqlResultMap getBasic_Popup(HttpRequestArgument arg , int page , int rows) throws Exception {
		DataMessage data = null;
		String gubun = (String)arg.getParameter("title_id");
		if( gubun.equals("alpha_0001") ){
			data = getChainStore(arg);		// 체인점 조회
		}else if ( gubun.equals("alpha_0002") ){
			data = getRestoreDt(arg);
		}
	...
	//체인점 조회
	public DataMessage getChainStore( HttpRequestArgument arg )  throws Exception {
		DataMessage data = arg.newStorage("POS");
		data.param
		.total("select count(1) as maxsize " );
		data.param
		.query(" select 							")
		.query(" 	stor_id as code , 			")
		.query(" 	stor_nm as text 				")
		.where(" from store 						")
		.where(" where 1 = 1 					")
		.where(" and stor_nm like %:search_txt%  	",arg.getParameter("search_txt" ));
		return data;
	}

	public DataMessage getRestoreDt( HttpRequestArgument arg )  throws Exception {
		DataMessage data = arg.newStorage("POS");
		data.param
		.total("select count(1) as maxsize " );
		data.param
		...
		.where(" cdate || seq as code ,											")
		.where(" cdate ||'-'|| seq as text ,											")
		.where(" cdate ,seq 										")
		.where(" from																	")
		.where(" ITEM_STORE_BACKUP											")
		 .where(" where  1=1														")
		.where(" and stor_id = :stor_id  	",arg.getParameter("stor_id" ))
		.where("order by 3 desc ,4 desc											")
		.where(" ) 																		");
	...
	public SqlResultMap getSpotPopup(HttpRequestArgument arg , int page , int rows) throws Exception {
		...
		.where(" and stor_nm like  %:class_nm% "                       ,arg.getParameter("class_nm" ))
