본 프로시져는 검사성적을 작성한 후 입고대장에 등록하도록 개발되었으나, 
검사성적 입력 시 입고 대장에 직접 등록하고 있어 본 기능이 불필요 함.
즉, 입고 접수 시 미검사 상태로 입고대장에 등록하며, 검사성적을 입력하면
검사 성적 관련 사항만을 update 함. 그러므로 수불대장에만 등록하면 된다. 
...
from   optn_mast
where  optn_idcd = '수입검사여부'
;	
select code_idcd into _insp_drtr_idcd
from   optn_mast
where  optn_idcd = '무검사입고담당'
;	
