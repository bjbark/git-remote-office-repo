...
drop procedure if exists `fn_seq_gen_v3`;
/*
 이 프로시져는 기본적으로 fn_seq_gen_v2과 일치되는 내용으로 구성된다. 
 fn_seq_gen_v2는 다른 프로시져 내에서 호출하여 그 값을 받을 수 없어서 출력변수를 지정하여 
 사용하도록 하였다....
 그러므로 fn_seq_gen_v2 내용을 수정할 경우 동시에 함께 수정해 주어야 한다. 
 
 변경된 내용 요약 ;
   1. 전달 변수들에 out _seqn_code 를 추가하였다. 
   2. 프로시져 수행 후 멘 마지막 부분에 단순 select 문장을 out  변수에 결과를 대입하도록 하였다. 

*/


create  procedure `fn_seq_gen_v3`(
    in _stor  varchar(50),
    in _table varchar(50),
    in _invc_numb varchar(50)
	out _seqn_code varchar(50)
...
--    declare _last_seqn     integer;
   ...
   select  ifnull(_code_bfhd_fill,concat(_hqof_idcd,_table)) into _code_bfhd_fill;
   ...
   */
   ...
   if  _seqn_type = "INVOICE ITEM" then begin
    ...
    select  last_idcd into _seqn_code
	from    seqn_dflt
    where   upper(hqof_idcd) = upper(_hqof_idcd)
	and     upper(tabl_name) = upper(_table);
